...
from imgmisc.imgmisc import _validate_connectivity, _validate_inputs, _compute_neighbours, merge, autocrop, rand_cmap, flatten, get_resolution
...
#from phenotastic.mesh import create_mesh, label_mesh
#from phenotastic.misc import rand_cmap
#from phenotastic.mesh import repair, make_manifold
#from phenotastic.misc import flatten
#import phenotastic.domains as boa
#from phenotastic import mesh as mp
...
def find_junctions_all(image, background=0, threshold=3, merge_adjacent=True, resolution=None, include_set='l1'):
    ...
    image, mask, resolution = _validate_inputs(image, mask=None, resolution=resolution)
...
def internal_angles(j_indices, j_neighs, edges, cells=None, background=0):
    poly = create_polydata(j_indices, edges)
    neighs = get_connected_vertices_all(poly, include_self=False)
    
    if cells is None:
        # write convenience function for this
