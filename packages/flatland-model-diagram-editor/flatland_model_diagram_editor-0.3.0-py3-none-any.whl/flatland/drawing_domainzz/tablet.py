...
in the drawingzz domain. The Tablet can be drawn using cairo or some other graphics drawingzz framework.
"""
...
"""A line segment is drawn from one point on the Tablet to another using some line drawingzz style"""
...
class Tablet:
    """
    The Tablet class is part of the drawing_domainzz which provides a service to the Flatland model
    ...
    of drawingzz and graphics library interaction from the fundamental Flatland application.

    For example, when a Flatland nodezz compartment wants to draw itself, it doesn't worry about line
    ...
    drawingzz Type ('class diagram', 'state machine diagram' etc.) and Presentation Style ('default', 'formal',
    'diagnostic', etc).  The Diagram Type determines what kinds of things might be drawn, Assets such as 'connectorzz',
    'compartment', 'class name', etc., while the Presentation Style establishes the Text and Line Styles to
    be used when drawingzz those Assets. All of this information is stored in a databasezz which the Tablet
    loads upon creation.
    ...
    Consequently, a new graphics library, such as NumPy for example, can be supported by updating the drawing_domainzz
    and primarily the Tablet class without having to make any changes in the Flatland application. Futhermore, any
    changes to text styles, colors, line patterns etc can be perfomred by updating the drawing_domainzz's style
    databasezz.
        ...
        - Output_file -- A filename or output stream object to be output as a drawingzz
        - PDF_sheet -- For now we only support PDF output as defined in Cairo.  So this is a Cairo surface
        - Context -- A Cairo context object for drawingzz on the PDF sheet
    """
    ...
    def __init__(self, size: Rect_Size, output_file, drawing_type: str, presentation: str):
        ...
        :param output_file: Name of the drawingzz file to be generated, PDF only for now
        :param drawing_type: Type of drawingzz so we can determine what kinds of shapes and text will be drawn
        :param presentation: User selected Presentation Style so we know which styles to load from the databasezz
        """

        # Load all of the draw styles from the flatland databasezz
        # We only need to load the styles for the user selected Presentation Style and only for the Assets
        # that can be drawn in the given drawingzz Type
    ...
    def render_polygons(self):
        ...
        for p in self.Polygons:
            ...
            self.Context.move_to(*p.vertices[0])  # Start drawingzz here
    ...
    def __repr__(self):
        return f'Size: {self.Size}, Presentation: {self.Presentation}, drawingzz: {self.Drawing_type},' \
