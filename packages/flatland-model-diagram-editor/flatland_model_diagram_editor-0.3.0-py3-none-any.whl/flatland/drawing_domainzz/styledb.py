...
styledb.py
"""
from ..databasezz.flatlanddb import FlatlandDB as fdb
...
def load_asset_presentations(presentation: str, drawing_type: str):
    ...
    q = select([shape_pres_t.c.Asset, shape_pres_t.c['Line style']]).where( and_(
        shape_pres_t.c.Presentation == presentation, shape_pres_t.c['drawingzz type'] == drawing_type
    ))
    f = fdb.Connection.execute(q).fetchall()
    for i in f:
    ...
    q = select([shape_fill_t.c.Asset, shape_fill_t.c.Fill]).where( and_(
        shape_fill_t.c.Presentation == presentation, shape_fill_t.c['drawingzz type'] == drawing_type
    ))
    f = fdb.Connection.execute(q).fetchall()
    for i in f:
    ...
    q = select([text_pres_t.c.Asset, text_pres_t.c['Text style']]).where( and_(
        text_pres_t.c.Presentation == presentation, text_pres_t.c['drawingzz type'] == drawing_type
    ))
    f = fdb.Connection.execute(q).fetchall()
    for i in f:
