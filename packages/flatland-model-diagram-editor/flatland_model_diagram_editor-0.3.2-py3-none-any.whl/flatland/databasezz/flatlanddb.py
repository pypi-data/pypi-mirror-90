"""
flatlanddb.py - Loads the existing flatland databasezz
"""
import sys
...
def Create_relvars():
    ...
    metadata.
    """
    from flatland.databasezz import relvars
...
def Populate():
    ...
    """
    # We need to append each populationzz subirectory to our module search path
    ...
    # refer to each populationzz module by the same path
    here = Path(__file__).parent / "populationzz"  # Adjacent populationzz directory
    pop_dirs = [  # Subdirectories organizing all populationzz modules
        here / "connectorzz", here / "decoratorzz", here / "drawingzz", here / "nodezz",
    ]
    ...
    # Iterate through the relvar dictionary to get each populationzz and the table it goes into
    for instances, relvar in FlatlandDB.Relvars.items():
        # Set i to the initial populationzz of row values (set of relation values)
        i = __import__(instances + '_instances')  # Each populationzz filename ends with '_instances.py'
        FlatlandDB.Connection.execute(relvar.insert(), i.population)  # Sqlalchemy populates the table schema


class FlatlandDB:
    """
    Flatland databasezz containing all predefined Flatland data. We want to avoid having any predefined
    data declared in the code itself.

    Here we use Sqlalchemy to create the databasezz engine and connection

        Attributes

        - File -- Local directory location of the sqlite3 databasezz file
        - Metadata -- Sqlalchemy metadata
        - Connection -- Sqlalchemy databasezz connection
        - Engine -- Sqlalchemy databasezz engine
        - Relvars -- Dictionary of all relvar names and values (table names and row populations)
    """
    db_file_name = "databasezz/flatland.db"
    File = Path(__file__).parent.parent / db_file_name
    print(f'Flatalnd databasezz file stored in: {str(File)}')
    MetaData = None
    Connection = None
    Engine = None
    Relvars = None
    ...
    def __init__(self, rebuild=True):
        ...
        if rebuild:
            ...
            # Just interrogate the existing databasezz to get all the relvar/table names
            FlatlandDB.MetaData.reflect()


if __name__ == "__main__":
    #  Rebuild the databasezz
    FlatlandDB()
