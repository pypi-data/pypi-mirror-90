...
from quant_vnpy.db.orm import TradeDataModel, PositionStatusModel, StrategyStatus, \
    StrategyStatusEnum, LatestTickPriceModel, PositionDailyModel, TradeDateModel
...
class StrategyPositionMonitor(Thread):
    ...
    def get_sleep_seconds(self, minutes=5) -> (datetime.datetime, float):
    ...
    def refresh_positions_of_strategy(
        ...
        """按策略计算相关合约的持仓、收益计算"""
        next_trade_date_dic = TradeDateModel.get_next_trade_date_dic()
        # 获取交易记录
    ...
    def refresh_positions_by_trade_data_list(
            ...
            next_trade_date_dic: Optional[Dict[datetime.date, datetime.date]] = None,
    ):
        """根据交易记录更新相应策略及symbol的持仓状态"""
        if trade_data_list is None or len(trade_data_list) == 0:
            return
        if next_trade_date_dic is None:
            next_trade_date_dic = TradeDateModel.get_next_trade_date_dic()
        ...
        if pos_status is not None:
            ...
            trade_dt_last: Optional[datetime.datetime] = pos_status.trade_dt
        else:
            offset_acc_gl = 0
            pos_status_dict = create_empty_pos_status_dic()
            trade_dt_last: Optional[datetime.datetime] = None
        ...
        for trade_data in trade_data_list:
            ...
            if trade_dt_last is None or trade_dt_last.date() != curr_trade_date:
                offset_daily_gl = 0
    ...
    def output_report():
        ...
        for strategy_name, symbol_pos_daily_dic in strategy_symbol_pos_daily_dic.items():
            for symbol, pos_daily in symbol_pos_daily_dic.items():
                ...
                holding_list.append([
                    ...
                    pos_daily.avg_price,
                    pos_daily.holding_gl, pos_daily.offset_gl, pos_daily.offset_acc_gl,
                    pos_daily.latest_price, pos_daily.trade_dt,
                ])

        if len(holding_list) > 0:
            df = pd.DataFrame(
                [_ for _ in holding_list if trade_date == _[-1].date()],
                columns=['策略名称', '合约', '方向', '手数', '持仓均价',
                         '持仓盈亏', '平仓盈亏', '累计平仓盈亏', '最新价格', '最近成交时间'])
            df.to_csv(f"各策略持仓状态_{date_2_str(trade_date)}.csv", encoding='GBK')
        ...
        if len(trade_date_list) > 0:
            ...
            if len(trade_list) > 0:
                ...
                df.to_csv(f"各策略交易明细_{date_2_str(trade_date)}.csv", encoding='GBK')
...
def _test_position_status_monitor():
    ...
    user_name = "19510002"
    broker_id = "5118"
    from quant_vnpy.db.orm import set_account
    ...
    def run(self) -> None:
        ...
        while self.run_task:
            ...
            try:
                ...
                try:
                    status = StrategyStatusEnum(status_int_db)
                    if sleep_seconds != 2:
                        sleep_seconds = 2
                except ValueError:
