...
class MLP:
    ...
    def generate_document(self, raw_text: str, analyzers: List[str], json_object: dict = None, doc_paths="text", lang=None):
        ...
        words = self._get_stanza_tokens(lang, processed_text) if processed_text else []
        entities = self._get_stanza_ner(lang, processed_text) if processed_text and ("ner" in analyzers) else []
