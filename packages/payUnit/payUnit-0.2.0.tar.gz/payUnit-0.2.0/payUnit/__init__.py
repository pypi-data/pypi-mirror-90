...
class payUnit:
    ...
    def __init__(self,data):      
        ...
        if data_keys.count('notify_url') == 0:
            raise Exception('You did not provide a notify url')
    ...
    def makePayment(self,amount): 
        ...
        test_url = 'https://app-payunit.sevengps.net/sandbox/gateway/initialize'
        # test_url = 'http://192.168.100.70:5000/sandbox/gateway/initialize'
        # test_url = "http://localhost:5000/notify"
        test_url_2 = 'https://app-payunit.sevengps.net/api/gateway/initialize'
        ...
        try: 
            if(str(environment.lower())=="test"):
                response = requests.post(test_url,data = json.dumps(test_body),headers = headers)
            else:
                response = requests.post(test_url_2,data = json.dumps(test_body),headers = headers) 
