...
#    STARTS=RawPars.start.unique()
    ...
    for Nobs in NOBSS:
        ...
        for par in pars:
            ...
            expected=ptmp.iloc[0]['init_'+par]
...
# log-slopes of estd
    ...
    res.to_csv(NAME+'_convergence.csv',index=False, float_format="%.6f")
...
# gnuplot+latex output:
    ...
    gnuplot_convergeplots(NAME,NOBSS,pars,statsfilename,gnuplotfilename)
    gnuplotfilename=NAME+'_paramdist.plt'
    gnuplot_paramdistplots(NAME,NOBSS,pars,statsfilename,gnuplotfilename)
    ...
    latex_documentwrite(NAME,plotpaths,NOBSS,pars,latexfilename)
...
def VPaths(Offset,SERIES,NOBSS,STARTS,RawPars,TimeSeries,VPATHS,VSERIES,VarianceSeries):
    ...
    for ca in range(0,len(SERIES)):
        ...
        TMP[asset+'_logret'] = np.log( TMP[asset] / TMP[asset].shift(-1) ) 
