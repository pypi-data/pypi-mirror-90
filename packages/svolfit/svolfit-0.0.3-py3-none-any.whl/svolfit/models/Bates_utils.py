...
def Bates_condassetprob_limit(y,u_prev,u_this,dt,rho,sigma,coeff_dt,coeff_iu,coeff_du,lamb,gamm,omeg):
...
#    condprob=np.exp(-0.5*yy_tmp*yy_tmp)/(vol_tmp*np.sqrt(2.0*np.pi))
    ...
    NumberTerms=11
    j_probs=np.array([ np.exp(-lamb*dt)*np.power(lamb*dt,cj)/factorial(cj) for cj in range(0,NumberTerms)])
    ...
    j_yy_tmp=( np.subtract.outer(y-coeff_dt-coeff_iu*iu_tmp-coeff_du*du_tmp,gamm*j_nj) )/j_vol_tmp
