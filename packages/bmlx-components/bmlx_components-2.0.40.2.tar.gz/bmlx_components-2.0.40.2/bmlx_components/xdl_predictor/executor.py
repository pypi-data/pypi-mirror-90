...
class XdlPredictorExecutor(XdlExecutor):
    def execute_as_worker(
        ...
        if exec_properties["enable_trace"]:
            ...
            # 重写artifact存储路径
            changed_output_path = reconstruct_path(meta_output_path)
            updated_dict.update({"hooks.trace.output_dir": changed_output_path})
        ...
        logging.info("[xdl_predict output_dict] %s", output_dict)
