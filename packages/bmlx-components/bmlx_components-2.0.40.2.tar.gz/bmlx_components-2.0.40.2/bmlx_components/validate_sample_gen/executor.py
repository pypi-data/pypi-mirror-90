...
class SampleGenExecutor(Executor):
        ...
        with open(local_origin_samples_path, "w") as origin_sample_fp:
            ...
            for msg in consumer:
                ...
                if "dispatcher_id" not in origin_pb.scenario.feature:
                    ...
                    outline = fg_main.compress_originpb_to_line(origin_pb)
    ...
    def upload_to_remote(self, local_path, remote_dir):
        ...
        assert(fs.__class__ in (CephFileSystem, HadoopFileSystem))
        if not io_utils.exists(remote_dir):
    ...
    def execute(
        ...
        fg_conf_path = os.path.join(
            pushed_model.origin_model_path, "fg/fg.yaml",
        )

        fg_lib_path = input_dict["fg_py_lib"][0].meta.uri
        logging.info("pushed_model.origin_model_path: %s", pushed_model.origin_model_path)
