...
class XdlRunner(object):
    def __init__(
        ...
        logging.info("[_model]%s", self._model)
        logging.info("model_module: %s", model_module)
    ...
    def run(self, reader: xdl.DataReader):
        ...
        while not sess.should_stop():
            if timeline_hook:
                ...
                logging.info("[xdl_runner]14")
            else:
                logging.info("[xdl_runner]15")
                run_option = xdl.RunOption()
                run_option.timeout_second = self._parameters[
                    "timeout"
                ].as_number()
                sess.run(train_ops, run_option=run_option)
                logging.info("[xdl_runner]16")
