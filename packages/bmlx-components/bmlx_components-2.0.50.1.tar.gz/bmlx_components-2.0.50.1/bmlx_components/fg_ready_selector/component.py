...
import logging
from bmlx_components.fg_ready_selector.executor import FgReadySelectorExecutor


class FgReadySelectorSpec(ComponentSpec):
    PARAMETERS = {
    ...
    INPUTS = {
        "deps": ChannelParameter(type=standard_artifacts.Schema,
                                 optional=True),
    }

    OUTPUTS = {
        "ready_uris": ChannelParameter(type=standard_artifacts.Schema,
                                       description="selected ready uris"),
    }


class FgReadySelector(Component):
    SPEC_CLASS = FgReadySelectorSpec

    EXECUTOR_SPEC = ExecutorClassSpec(FgReadySelectorExecutor)
    ...
    def __init__(
        ...
        super(FgReadySelector, self).__init__(
