...
from bmlx_components.fg_operator import submit_template

class FgOperatorSpec(ComponentSpec):
    ...
    INPUTS = {
        "original_data": ChannelParameter(type=standard_artifacts.Schema, optional=True),
    }

    OUTPUTS = {
        "output_path": ChannelParameter(type=standard_artifacts.Schema, optional=True),
    }

class FgOperatorExecutor(Executor):
    ...
    def append_output_path(self, output_dict, output_path):
        artifact = Artifact(type_name=standard_artifacts.Schema.TYPE_NAME)
...
class FgOperator(Component):
    SPEC_CLASS = FgOperatorSpec

    EXECUTOR_SPEC = ExecutorClassSpec(FgOperatorExecutor)
    ...
    def __init__(
        ...
        super(FgOperator, self).__init__(
