...
:LastEditTime: 2021-01-06 13:58:53
...
class TopBaseHandler(SevenBaseHandler):
    ...
    def get_taobao_order(self, open_id, access_token, start_created="", end_created=""):
        ...
        try:
            ...
            self.logger_error.error("get_dead_date:" + str(ex))
            return []
    ...
    def instantiate_new(self, description, icon):
        """
        :description: 实例化
        ...
        try:
            ...
            if "crm_member_joinurl_get_response" in resp.keys():
                if "result" in resp["crm_member_joinurl_get_response"].keys():
                    if "result" in resp["crm_member_identity_get_response"]["result"].keys():
                        join_member_url = resp["crm_member_identity_get_response"]["result"]["result"]
        except Exception as ex:
