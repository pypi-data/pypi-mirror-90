...
:LastEditTime: 2020-12-30 18:38:31
        ...
        for act_prize in act_prize_page_list:
            try:
                if act_prize["prize_detail"]:
                    act_prize["prize_detail"] = self.json_loads(act_prize["prize_detail"])
                else:
...
class SubmitPrizeOrderHandler(SevenBaseHandler):
    ...
    def get_async(self):
        ...
        update_sql = "prize_order_id=%s,prize_order_no=%s", "act_id=%s and open_id=%s and prize_order_id=0"
        params = [prize_order.id, prize_order.order_no, act_id, open_id]
        prize_roster_model.update_table(update_sql, params=params)
