...
def index():
    ...
    pd.DataFrame({'a': [1, 2, 3]}).to_csv('test_ret.csv')
    return f'版本号：{__version__}'
...
def run():
    ...
    if not os.environ.get("WERKZEUG_RUN_MAIN"):
        ...
        # vm = VersionManager()
        # if vm.has_updated():
        #     print(f'发现新版本，开始更新({VersionManager.m_current_version})')
        #     vm.start_update()
        #     print(f'更新完成({VersionManager.m_current_version})')
...
#     try:
#         import pdcoder_start_update
#     except ImportError:
#         return
#     os.system(f'start pdcoder_start_update -dir {os.getcwd()}')
#     sys.exit()
...
def main():
    ...
    # has, _ = find_arg(args, '-u', return_value=False)
    # if has:
    #     start_update()
