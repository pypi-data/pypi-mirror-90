...
@main.command("release", help="Create a github release")
@click.option("--deploy", is_flag=True, help="Trigger deploy after release creation")
@click.pass_obj
def cli_release(obj, deploy):
    Release(obj["project"], deploy, obj["config"], obj["tokens"]).run()
...
@main.command("rollback", help="Rollback in production")
@click.pass_obj
...
@click.pass_obj
def cli_secret(obj, create_action, grant_action):
    Secret(obj["project"], obj["config"], create_action, grant_action).run()
