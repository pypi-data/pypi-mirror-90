...
using_tzfinder = sys.version_info.minor >= 6
if using_tzfinder:
...
def get_timezone_from_lat_lon(lat: float, lon: float) -> str:
    # timezonefinder is much faster than tzwhere, but does not import/work
    # properly with python version < 3.6
    if using_tzfinder:
        return tzfinder.timezone_at(lng=lon, lat=lat)
    else:
        return tzwhere.tzwhere().tzNameAt(lat, lon)


def localize_timestamp(timestamp: str, lat: float, lon: float) -> datetime:
    """Returns a datetime object representing the local time at coordinates
    (lat, lon)

    :param timestamp: a timestamp in the following format: 2011-09-28T15:14:11.824385+04:00
    :param lat: latitude
    :param lon: longitude

    :returns: a datetime object representing the local time at coordinates (lat, lon)
    """
    colon_index = -3  # must remove the colon between the timezone offset because datetime lib can't parse that
    assert timestamp[colon_index] == ":", "Unexpected metadata timestamp format {}".format(
        timestamp)
    timestamp = timestamp[:colon_index] + timestamp[colon_index + 1:]
    timestamp = datetime.strptime(timestamp, "%Y-%m-%dT%H:%M:%S.%f%z")

    timezone_str = get_timezone_from_lat_lon(lat, lon)

    if timezone_str is None:
        raise ValueError(
            "ValueError: coordinates {}, {} do not correspond to a timezone".format(
                lat, lon))
