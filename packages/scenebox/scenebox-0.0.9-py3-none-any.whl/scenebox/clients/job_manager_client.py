...
def create_jobs_map(asset_ids: List[str],
                    job_type: str,
                    asset_type: str,
                    auth_token: str,
                    asset_manager_url: str,
                    job_description_fn: Callable = lambda asset_id: "Job for {}".format(asset_id),
                    queue_jobs: bool = False) -> Dict[str,
                                                      str]:
    """Create a map of job IDs keyed by asset_ids."""
    # import is done here since some services use python < 3.5 and there are
    # lots of type hints in this module
    from shared.tools.authentication import identity_manager_client

    user = identity_manager_client.get_user_by_token(auth_token)

    o = {}

    for asset_id in asset_ids:
        job = JobManagerClient(
            job_type=job_type,
            asset_id=asset_id,
            asset_type=asset_type,
            username=user.username,
            description=job_description_fn(asset_id),
            asset_manager_url=asset_manager_url,
            auth_token=user.token
        )
        o[asset_id] = job.job_id
        if queue_jobs:
            job.queue()
