...
def create_file_chunks(bucket_name, file_pattern, job_uuid, file_headers, integration_settings, chunk_number=0,
    ...
    idx = None
    file_path = None
    for row, idx, file_path in read_remote_csvs_as_stream(bucket_name=bucket_name, file_pattern=file_pattern,
                                                          job_uuid=job_uuid,
                                                          delimiter=delimiter):
        ...
        if 0 < integration_settings['chunk_size'] <= chunk_records:
            ...
            update_file_checkpoint(job_uuid=job_uuid, file_pattern=file_pattern, chunk_number=chunk_number,
                                   line_number=idx, file_path=file_path)
            start = datetime.utcnow().timestamp()
...
def update_file_checkpoint(job_uuid, file_pattern, file_path, chunk_number, line_number):
    ...
    return cache_data(key=key, value=dict(file_path=file_path, chunk_number=chunk_number, line_number=line_number), expiration_seconds=86400)
...
def create_parallel_chunks(job_uuid, bucket_name, file_pattern, platform_name, destination, file_headers, segments,
    ...
    for row, line_number, file_path in read_remote_csvs_as_stream(
        ...
        if 0 < chunk_size <= chunk_records:
            ...
            chunk_pointer, chunk_number, chunk_records = push_chunks_to_distribute(
                chunk_pointer=chunk_pointer, chunk_multiplier=chunk_multiplier, chunk_path=chunk_path,
                bucket_name=bucket_name, platform_name=platform_name,
    ...
    if chunk_records > 0:
        total_chunks = 1 if chunk_size == 0 else chunk_number + chunk_records / (chunk_size / chunk_multiplier)
...
def push_chunks_to_distribute(chunk_pointer, chunk_path, chunk_multiplier, job_uuid, bucket_name,
    ...
    file_path = CHUNKED_FILE_GCS_PATH_FORMAT.format(
        ...
        hour=now.hour,
        minute=now.minute)
    upload_file_to_gcs(local_path=chunk_path, file_path=file_path)
    deliver_data_task.delay(bucket_name=bucket_name, platform_name=platform_name, file_pattern=file_path,
    ...
    try:
        ...
        pass
    return None, chunk_number * chunk_multiplier, 0
