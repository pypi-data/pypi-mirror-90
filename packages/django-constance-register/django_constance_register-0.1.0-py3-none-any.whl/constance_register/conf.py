...
def import_attributes(module_path: str, attributes: List[str]):
    imported = {}

    for attribute in attributes:
        try:
            imported[attribute] = import_string(f'{module_path}.{attribute}')
        except ImportError as e:
            msg = "Could not import '%s' for CONFIG setting '%s'. %s: %s." % (module_path, attribute, e.__class__.__name__, e)
            logging.warning(msg)
