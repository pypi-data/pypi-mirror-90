...
class Client(object):
    ...
    def _put_file_chunked(self, remote_path, local_source_file, **kwargs):
        ...
        def progress(chunk):
            global downloaded
            downloaded += chunk
            done = int(console_size * downloaded / float(size))
            percent = 100 * downloaded / float(size)
            remain_time = (int(time.time()) - transfer_id) * \
                ((100 - percent) / percent)
            if percent >= 100:
                sys.stdout.write("\rUploading " + local_source_file + " [" + '#' * done + '.' * (
                    console_size-done) + "] " + '{:.2f}'.format(percent) + "% Time: " + utils.format_time(int(time.time()) - transfer_id))
            else:
                sys.stdout.write("\rUploading " + local_source_file + " [" + '#' * done + '.' * (
                    console_size-done) + "] " + '{:.2f}'.format(percent) + "% ETA: " + utils.format_time(remain_time))
            sys.stdout.flush()
        ...
        for chunk_index in range(0, int(chunk_count)):
            ...
            if kwargs.get('show_progress_bar', True):
                progress(chunk_size if chunk_count > 1 else size)
