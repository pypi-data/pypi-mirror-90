...
from .DecoratorFuns import CusFunsWrapper, HandleWrapper
...
def _register_module(module_path: str):
    '''
    module_path : str 
    'src.cusFuns.columnNameChanger.index'
    '''
    pkg = HandleWrapper.__module__.split('.')[0]
    module = importlib.import_module(f'.{module_path}', pkg)
    # [('name',obj)]
...
def auto_register(package_path: Union[str, pathlib.PurePath], base_folder: Union[str, pathlib.PurePath]):
    org_dir = os.getcwd()
