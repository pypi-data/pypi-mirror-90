...
def parse(page,domain=''):
    ...
    if not addresses and result['aboutLink']:
      try:
        res = requests.get(result['aboutLink'], timeout=20, headers=headers)
    ...
    if result['title'] and namepattern:
      ...
      try:
        aboutres = requests.get(result['aboutLink'], timeout=20, headers=headers)
...
if __name__ == '__main__':
  res = requests.get('http://fastsoft.net', timeout=20, headers=headers)
  result = parse(res.content,domain='fastsoft.net')
  print(result)