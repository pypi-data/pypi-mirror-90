...
class Draw:
    ...
    def draw(self, src, src_point):
        ...
        for i in range(src_point.shape[0]):
            cv2.circle(
                src,
                (int(src_point[i, 0]), int(src_point[i, 1])),
                5,
                color=(b, g, r),
                thickness=2,
            )
    ...
    def draw_match(self, src, dst, src_point, dst_point):
        ...
        pic = np.zeros(
            [max(src.shape[0], dst.shape[0]), src.shape[1] + dst.shape[1], 3],
            dtype=np.uint8,
        )
        ...
        for i in range(0, src_point.shape[0], 20):
            cv2.line(
                pic,
                (src_point[i, 0], src_point[i, 1]),
                (dst_point[i, 0], dst_point[i, 1]),
                (
    ...
    def draw_line(self, img, lines):
        for line in lines:
            cv2.line(
                ...
                (np.int(line.end[0]), np.int(line.end[1])),
                (0, 0, 255),
                1,
            )
        return img