from CVG.feature.feature_point import FeaturePoint
import cv2
import numpy as np
...
class Match:
    ...
    def getInitialFeaturePairs(self):
        ...
        if len(match) > 4:
            psta = np.array([src_point.feature_point[i].pt for (_, i) in match])
            pstb = np.array([dst_point.feature_point[i].pt for (i, _) in match])
