...
cadet_location = "C:\\Users\\kosh_000\\cadet_build\\cadet_3\\cadet\\MS_SMKL_RELEASE\\bin\\cadet-cli.exe"
...
def main(args):
    ka, nu, sigma = args
    #filename = "%.2g_%.2g_%.2g.h5" % (ka, nu, sigma)

    filename = hashlib.md5(str(args).encode('utf-8','ignore')).hexdigest() + '.h5'

    createSimulation(filename, ka, nu, sigma)
    runSimulation(filename)
    plotSimulation(filename)
    return "Completed Ka: %s  Nu: %s Sigma: %s" % (ka, nu, sigma)

def createSimulation(filename, ka, nu, sigma):
    with h5py.File(filename, 'w') as f:
        createInput(f, ka, nu, sigma)

def createInput(f, ka, nu, sigma):
    input = f.create_group("input")
    createModel(input, ka, nu, sigma)
    createReturn(input)
    createSolver(input)

def createModel(input, ka, nu, sigma):
    ...
    createInlet(model)
    createColumn(model, ka, nu, sigma)
    createOutlet(model)
...
def createColumn(model, ka, nu, sigma):
    ...
    createAdsorption(column, ka, nu, sigma)
    createDiscretization(column)

def createAdsorption(column, ka, nu, sigma):
    ads = column.create_group('adsorption')

    set_int(ads, 'IS_KINETIC', 0)
    set_double(ads, 'SMA_KA', [0.0, ka, 1.59, 7.7])
    set_double(ads, 'SMA_KD', [0, 1000, 1000, 1000])
    set_double(ads, 'SMA_LAMBDA', 1200)
    set_double(ads, 'SMA_NU', [0.0, nu, 5.29, 3.7])
    set_double(ads, 'SMA_SIGMA', [0, sigma, 10.6, 10.0])
...
def plotSimulation(filename):
    with h5py.File(filename, 'r') as h5:
        ...
        #f, (ax1, ax2) = fig.subplots(1, 2, figsize=[16, 8])
        plotInlet(ax1, h5)
        plotOutlet(ax2, h5)
        fig.tight_layout()
        #plt.show()
        plt.savefig(filename.replace('h5', 'png'), dpi=100)
        plt.close()
...
if __name__ == "__main__":
    ...
    ka = [30, 35, 40]
    nu = [4.6, 4.7, 4.8]
    sigma = [11.7, 11.8, 11.9]

    settings = itertools.product(ka, nu, sigma)

    results = futures.map(main, settings)
