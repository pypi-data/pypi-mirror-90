...
class MissingValueError(ClientError, ValueError):
    def __init__(self, field, error=None):
        super().__init__("Value for {} is not provided.".format(field), error, error_code=HTTPStatus.BAD_REQUEST.value)

class InvalidInputError(ClientError, ValueError):
    def __init__(self, field, error=None):
        super().__init__("The value provided for {} is invalid.".format(field), error, error_code=HTTPStatus.BAD_REQUEST.value)

class UnexpectedTypeError(ClientError, ValueError):
    def __init__(self, field, expected_type, actual_type):
        super().__init__("Unexpected type of '{}' is specified, expected: {}, actual: '{}'.".format(field, expected_type, actual_type), error_code=HTTPStatus.BAD_REQUEST.value)
...
class UnsupportedOperationError(ClientError):
    ...
    def __init__(self, operation_name, error=None):
        super().__init__("The requested operation({}) is not supported.".format(operation_name), error=error, error_code=HTTPStatus.BAD_REQUEST.value)

class NotImplementedError(ClientError):
    """Exception raised for operation requested is not yet implemented."""
    def __init__(self, operation_name, error=None):
        super().__init__("The requested operation({}) is not yet implemented.".format(operation_name), error=error, error_code=HTTPStatus.NOT_IMPLEMENTED.value)
...
class ServiceUnavailableError(ClientError):
    ...
    def __init__(self, response, error=None):
        message = "Sufficient resources are not available to process the request ({} {}).".format(response.request.method, response.request.url)
        error = response.text if response.apparent_encoding is not None else "[binary content, " + str(len(response.content)) + " bytes]"
