...
class TaskHandler:
    ...
    def execute(self):
        with Popen([self.next.command], shell=True, stdin=None, stdout=None, stderr=None, close_fds=True) as proc:
...
class TaskQHelper:
    ...
    def abort_task(cls, task_id):
        ...
        if task.is_running:
            with Popen(['kill -9 {}'.task.pid], shell=True, stdin=None, stdout=None, stderr=None, close_fds=True) as proc:
                proc.wait()
