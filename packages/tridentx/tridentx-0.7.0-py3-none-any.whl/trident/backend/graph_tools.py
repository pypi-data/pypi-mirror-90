...
from trident.backend.onnx_ops import find_opset,make_name, port_name,are_shapes_equal,map_numpy_to_onnx_dtype,is_onnx_domain,ONNX_UNKNOWN_DIMENSION
from trident.backend.common import make_sure
...
class Node(object):
    ...
    def output(self, val):
        ...
        for o in self._output:
            make_sure(o not in self.graph._output_to_node_name, "output %s already in output mapping", o)
    ...
    def is_nhwc(self):
        ...
        make_sure('D' not in self.data_format, "is_nhwc called on %s with spatial=2 but data_format=%s",
    ...
    def _graph_check(self):
        make_sure(self.graph is not None, "Node %s not belonging any graph",
                        self.name)
...
class Graph(object):
    ...
    def __init__(self, nodes, output_shapes=None, dtypes=None, target=None, opset=None, extra_opset=None,
        ...
        if extra_opset is not None:
            make_sure(isinstance(extra_opset, list), "invalid extra_opset")
        ...
        if not is_subgraph:
            ...
            for o in self.outputs:
                ...
                if n.is_graph_input():
                    ...
                    continue
                new_output_name = port_name(n.name + "_" + make_name("raw_output_"))
    ...
    def make_consts(self, values, np_type=np.int64, skip_conversion=False, raw=True):
        ...
        for value in values:
            ...
            consts.append(self.make_const(make_name("const"), np_val, skip_conversion, raw))
        return consts
    ...
    def make_const(self, name, np_val, skip_conversion=False, raw=True):
        ...
        if raw:
            ...
            onnx_tensor = helper.make_tensor(name, map_numpy_to_onnx_dtype(np_val.dtype),
        ...
        self.set_dtype(name, map_numpy_to_onnx_dtype(np_val.dtype))
        return node
    ...
    def make_node(self, op_type, inputs, attr=None, output_count=1, outputs=None, skip_conversion=True,
                  op_name_scope=None, name=None, shapes=None, dtypes=None, domain=ONNX_DOMAIN,
        ...
        make_sure(n is None, "name %s already exists in node: \n%s", name, n)
        for o in outputs:
            n = self.get_node_by_output_in_current_graph(o)
            make_sure(n is None, "output tensor named %s already exists in node: \n%s", o, n)
    ...
    def remove_node(self, node_name):
        ...
        make_sure(node_name in self._nodes_by_name, "node %s not in current graph, cannot remove", node_name)
    ...
    def update_node_shape_dtype(self, node, override=False):
        ...
        if not is_onnx_domain(node.domain):
            return
        ...
        for output, shape, dtype in zip(node.output, shapes, dtypes):
            ...
            if shape is None:
                ...
                if existing_shape is not None and not are_shapes_equal(existing_shape, shape):
                    if override:
    ...
    def change_node_name(self, node, new_name):
        """Remove node in current graph."""
        make_sure(new_name not in self._nodes_by_name, "node %s not unique ", new_name)
    ...
    def add_graph_output(self, name, dtype=None, shape=None):
        ...
        make_sure(name in self._output_to_node_name, "output %s not exist in the graph", name)
        ...
        if name not in self.outputs:
            make_sure(shape is not None, "shape for output %s should not be None", name)
            make_sure(dtype is not None, "dtype for output %s should not be None", name)
    ...
    def get_shape(self, name):
        """Get shape for node."""
        make_sure(isinstance(name, six.text_type), "get_shape name is invalid type: %s", name)
        node = self.get_node_by_output(name, search_in_parent_graphs=True)
    ...
    def set_shape(self, name, val):
        ...
        if isinstance(val, tuple):
            val = list(val)
        node = self.get_node_by_output(name, search_in_parent_graphs=True)
        make_sure(node is not None, "cannot find node by output id %s", name)
    ...
    def topological_sort(self, ops):
        ...
        for i, op in enumerate(ops):
            ...
            for inp in sorted(all_input):
                ...
                make_sure(j is not None, "Cannot find node with output %r", inp)
    ...
    def make_graph(self, doc, graph_name=None, external_tensor_storage=None):
        ...
        for op in placeholder_ops:
            if op.type == "PlaceholderWithDefault":
                make_sure(op.inputs[0] is not None, "Cannot find node with output {}".format(op.input[0]))
                make_sure(op.inputs[0].is_const(),
    ...
    def make_model(self, graph_doc, optimize=False, graph_name="tf2onnx", external_tensor_storage=None, **kwargs):
        ...
        make_sure(self.opset in OPSET_TO_IR_VERSION,
                        "Opset %s is not supported yet. Please use a lower opset" % self.opset)

        # set the IR version based on opset
        try:
            model_proto.ir_version = OPSET_TO_IR_VERSION.get(self.opset, model_proto.ir_version)
    ...
    def make_onnx_graph_io(self, ids):
        ...
        for name in ids:
            ...
            make_sure(dtype is not None, "missing output dtype for " + name)
            # TODO: allow None output shape or not? e.g. shape=(?,)
            #make_sure(shape is not None, "missing output shape for " + name)
            if shape is None: logger.warning("missing output shape for %s", name)

            v = make_onnx_inputs_outputs(name, dtype, shape)
    ...
    def insert_new_node_on_output(self, op_type, output_name=None, name=None, inputs=None, domain=None, **kwargs):
        ...
        Returns:
            node that was inserted
        """
        make_sure(isinstance(output_name, six.text_type), "output_name's type is not expected: %s",
                        type(output_name))
        make_sure(isinstance(op_type, six.text_type), "op_type's type is not expected: %s",
                        type(op_type))
        make_sure(output_name is not None, "output_name cannot be None for op_type=%r.", op_type)
...
class GraphUtil(object):
    ...
    def create_graph_from_onnx_model(onnx_model_proto):
        ...
        make_sure(opset_version is not None, "opset version is not specified for onnx domain")
