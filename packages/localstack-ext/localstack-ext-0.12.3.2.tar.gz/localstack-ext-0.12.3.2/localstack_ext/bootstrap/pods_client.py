...
class CloudPodManager(wsYyp):
 BACKEND='_none_'
 CONFIG_FILE='.localstack'
 def __init__(self,pod_name=wsYyO,config=wsYyO):
  self.pod_name=pod_name
  self.config=config
 def push(self):
  raise wsYya('Not implemented')
 def pull(self):
  raise wsYya('Not implemented')
 ...
 def get(cls,pod_name,config=wsYyO):
  ...
  raise wsYya('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @wsYyo
 def load_config(cls):
  try:
   return yaml.load(to_str(load_file(cls.CONFIG_FILE)))
  except wsYya:
   raise wsYya('Unable to find and parse config file "%s"'%cls.CONFIG_FILE)
 @wsYyo
 def data_dir(cls):
  ...
  if not config.DATA_DIR:
   raise wsYya('Working with local cloud pods requires $DATA_DIR configuration')
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b'HEAD' not in repo or repo.refs.allkeys()==wsYyb([b'HEAD'])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=wsYyc)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise wsYya('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get('url')
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=wsYyR(self,'clone_dir',wsYyO)
...
def pull_state(pod_name,args):
 backend=CloudPodManager.get(pod_name=pod_name)
 backend.pull()
def _pod_config(pod_name,config=wsYyO):
 config=config or CloudPodManager.load_config()
 pod_config=config.get('pods',{}).get(pod_name)
 if not pod_config:
  raise wsYya('Unable to find config for pod named "%s"'%pod_name)
 return pod_config
