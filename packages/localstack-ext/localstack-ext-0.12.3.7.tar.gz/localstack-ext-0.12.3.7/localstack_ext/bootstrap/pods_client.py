...
class CloudPodManager(bUPyh):
 BACKEND='_none_'
 CONFIG_FILE='.localstack'
 def __init__(self,pod_name=bUPyB,config=bUPyB):
  self.pod_name=pod_name
  self.config=config
 def push(self):
  raise bUPyC('Not implemented')
 def pull(self):
  raise bUPyC('Not implemented')
 ...
 def get(cls,pod_name,config=bUPyB):
  ...
  raise bUPyC('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @bUPyW
 def load_config(cls):
  try:
   return yaml.load(to_str(load_file(cls.CONFIG_FILE)))
  except bUPyC:
   raise bUPyC('Unable to find and parse config file "%s"'%cls.CONFIG_FILE)
 @bUPyW
 def data_dir(cls):
  ...
  if not config.DATA_DIR:
   raise bUPyC('Working with local cloud pods requires $DATA_DIR configuration')
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b'HEAD' not in repo or repo.refs.allkeys()==bUPyS([b'HEAD'])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=bUPyQ)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise bUPyC('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get('url')
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=bUPyI(self,'clone_dir',bUPyB)
...
def pull_state(pod_name,args):
 backend=CloudPodManager.get(pod_name=pod_name)
 backend.pull()
def _pod_config(pod_name,config=bUPyB):
 config=config or CloudPodManager.load_config()
 pod_config=config.get('pods',{}).get(pod_name)
 if not pod_config:
  raise bUPyC('Unable to find config for pod named "%s"'%pod_name)
 return pod_config
