...
np.random.seed(0)
n_x = np.random.randn(1, 1, 4, 4).astype(np.float32)
n_y = np.random.randn(1, ).astype(np.float32)
n_weight = np.random.randn(3, 1, 3, 3).astype(np.float32)
...
t_bias.retain_grad()
# function
t_out1 = torch.nn.functional.conv2d(t_x, t_weight, t_bias)
t_out1.retain_grad()
...
# function
x_out1 = xs.nn.functional.conv2d(x_x, x_weight, x_bias)
