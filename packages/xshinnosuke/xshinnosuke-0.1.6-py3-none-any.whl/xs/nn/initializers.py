from xs.utils.common import copy, Tuple
from xs.core import __global as GLOBAL
from xs.core.base import Tensor
...
class Uniform(Initializer):
    ...
    def __call__(self, shape, **kwargs):
        return Tensor(GLOBAL.np.random.uniform(-self.scale, self.scale, size=shape), **kwargs)
...
class Normal(Initializer):
    ...
    def __call__(self, shape, **kwargs):
        return Tensor(GLOBAL.np.random.normal(loc=self.mean, scale=self.std, size=shape), **kwargs)
...
class Zeros(Initializer):
    def __call__(self, shape, **kwargs):
        return Tensor(GLOBAL.np.zeros(shape), **kwargs)
...
class RandN(Initializer):
    def __call__(self, shape: Tuple, **kwargs):
        return Tensor(GLOBAL.np.random.randn(*shape), **kwargs)
...
class RandInt(Initializer):
    def __call__(self, low, high=None, shape=None, **kwargs):
        return Tensor(GLOBAL.np.random.randint(low=low, high=high, size=shape), **kwargs)
...
def ones_like(a, **kwargs):
    ...
    return Tensor(GLOBAL.np.ones_like(a.data), requires_grad=requires_grad, **kwargs)
...
def zeros_like(a, **kwargs):
    requires_grad = kwargs.pop('requires_grad', a.requires_grad)
    return Tensor(GLOBAL.np.zeros_like(a.data), requires_grad=requires_grad, **kwargs)
