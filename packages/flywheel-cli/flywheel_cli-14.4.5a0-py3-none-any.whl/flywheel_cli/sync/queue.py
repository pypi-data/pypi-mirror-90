...
class StoreTask(Task):
    ...
    def execute(self):
        ...
        if self.queue.unpack and self.src_file.is_packed:
            ...
            log.debug(
                "%s Unpacking %s to %s, exists=%s",
                this_thread.name,
                self.src_file.name,
                out_path,
                out_exists,
            )
            members = self.src_file.get_members(not out_exists)
            for member in members:
                self.execute_one(member)
            log.debug(
                "%s Done unpacking %d zip members", this_thread.name, len(list(members))
            )
        else:
            log.debug("%s Storing %s", this_thread.name, self.src_file.name)
