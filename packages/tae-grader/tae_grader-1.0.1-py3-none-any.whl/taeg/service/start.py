...
class Application(tornado.web.Application):
    """
    Taeg Service tornado application
    """
    def __init__(self):
        """
        Initialize tornado server for receiving/grading submissions
        """
        endpoint = ARGS.endpoint or os.environ.get("TAEG_ENDPOINT", None)
        assert endpoint is not None, "no endpoint address provided"
        assert os.path.isdir(TAEG_SERVICE_DIR), "{} does not exist".format(TAEG_SERVICE_DIR)
        settings = dict(
            google_oauth={
            ...
            notebook_dir = os.path.join(TAEG_SERVICE_DIR, "submissions"),
...
def main(cli_args):
    """
    Starts Taeg Service tornado server
    ...
    #     raise ImportError(
    #         "Missing some packages required for taeg service. "
    #         "Please install all requirements at "
    #         "https://raw.githubusercontent.com/taecoding/tae-grader/master/requirements.txt"
    #     )
    ...
    if not os.path.isdir(TAEG_SERVICE_DIR):
        os.makedirs(os.path.join(TAEG_SERVICE_DIR))
