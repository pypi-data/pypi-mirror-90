...
class GLPKSolver(Solver):
    ...
    def supports(cls, footprint):
        ...
        return footprint in cls.SUPPORTED
    ...
    def test_availability(cls):
        ...
        assert_import_exists("swiglpk")

    @classmethod
    def names(cls):
        """Implement :meth:`~.solver.Solver.names`."""
        return "glpk", "GLPK", "GNU Linear Programming Kit"
    ...
    def _import_constraint(self, picosCon):
        ...
        rows = picosCon.sparse_Ab_rows(self._glpkVarOffset)
        for localConIndex, (glpkVarIndices, coefs, rhs) in enumerate(rows):
