    ...
    def unit_ball_form(self):
        ...
        if len(self.ne.mutables) != 1:
            raise NotImplementedError("Unit ball membership form is only "
                ...
                "expression depends on exactly one mutable. Found {}.".format(
                    " and ".join(mtb.name for mtb in self.ne.mutables)
                    if self.ne.mutables else "none"))
        ...
        try:
            ...
            raise ValueError("The linear operator applied to {} to form the "
                "linear part of {} is not bijective, so no unit ball form "
                "exists for {}.".format(X.name, self.ne.string, self))
