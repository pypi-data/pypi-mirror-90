...
"""Whether to add all non-free solvers to :data:`SOLVER_BLACKLIST`."""
...
for key, value in os.environ.items():
    ...
    setting = key.split(ENV_PREFIX, 1)[1]

    if setting not in __all__:
        raise AttributeError(
            "The setting {} referenced by the environment variable {} is not "
            "known to PICOS.".format(setting, key))

    try:
        value = literal_eval(value)
    except Exception as error:
        raise ValueError(
            "The value {} of the environment variable {} is not a valid Pyhton "
            "literal: {}".format(value, key, error))

    setattr(THIS, setting, value)
