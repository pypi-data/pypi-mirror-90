...
class Expression(ABC):
    ...
    value = property(
        ...
        r"""Value of the expression, or :obj:`None`.

        It is defined if the expression is constant or if all mutables involved
        in the expression are valued. Mutables can be valued directly by writing
        to their :attr:`value` attribute. Variables are also valued by PICOS
        when an optimization solution is found.
    ...
    value_as_matrix = property(
        ...
        lambda self, x: self._set_value(x),
        lambda self: self._set_value(None),
        r"""Value of the expression as a CVXOPT matrix type, or :obj:`None`.
    ...
    safe_value_as_matrix = property(
        ...
        lambda self, x: self._set_value(x),
        lambda self: self._set_value(None),
        r"""Value of the expression as a CVXOPT matrix type, if defined.
    ...
    def __invert__(self):
        ...
        A = Constant(glyphs.frozen(self.string), self._get_value(), self.shape)
        A._origin = self
        return A
    ...
    def __div__(self, other):
        """Denote division by another expression."""
        # Python 2 uses this instead of __truediv__.
        return self.__truediv__(other)

    def __rdiv__(self, other):
        """Denote division of another expression by the expression."""
