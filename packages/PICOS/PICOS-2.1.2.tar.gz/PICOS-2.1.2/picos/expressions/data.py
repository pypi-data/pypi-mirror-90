...
from ..compat import MAJOR, long, lru_cache
...
def load_data(value, shape=None, typecode=None, sparse=None,
    ...
    def load_sparse(V, I, J, shape, typecode):
        ...
        try:
            ...
            raise TypeError("Failed to create a CVXOPT sparse matrix of shape "
                "{} and type '{}': {}.".format(glyphs.shape(shape), typecode,
    ...
    def load_dense(value, shape, typecode):
        ...
        try:
            ...
            raise TypeError("Failed to create a CVXOPT dense matrix of shape "
                "{} and type '{}': {}.".format(glyphs.shape(shape), typecode,
                str(error).capitalize()))
    ...
    # Allow Python 3 range types.
    if MAJOR >= 3 and isinstance(value, range):
        value = list(value)
    ...
    if isinstance(value, (int, long, float, complex, numpy.number)):
        # Convert NumPy numbers to Python ones.
        if isinstance(value, numpy.number):
            if isinstance(value, numpy.integer):
                value = int(value)  # Result can be a Python 2 long.
        ...
        if isinstance(value, (int, long)):
            if value > sys.maxsize or value < -sys.maxsize - 1:
                raise ValueError(
                    "The number {} is too large to be loaded by PICOS."
                    .format(value))

            if isinstance(value, long):
                value = int(value)
                assert isinstance(value, int), "Failed to convert from long to"\
                    " int despite being smaller than sys.maxsize."
        ...
        try:
            tokens = re.findall(regex, value)[0]
            if len(tokens) != 3:
        ...
        if inShape:
            ...
            if blend_shapes(shape, inShape) != inShape:
                raise ValueError("Inconsistent shapes for matrix given as '{}' "
                    "with expected shape {}."
                    .format(value, glyphs.shape(shape)))
...
def convert_and_refine_arguments(*which, **kwargs):
    ...
    """
    # HACK: Python 2 does not allow individual keyword arguments following
    #       starred positional ones, so map them manually.
    refine    = kwargs.pop("refine")    if "refine"    in kwargs else True
    allowNone = kwargs.pop("allowNone") if "allowNone" in kwargs else False
    if kwargs:  # Mimic Python 3's usual exception.
        raise TypeError("'{}' is an invalid keyword argument for "
            "convert_and_refine_arguments()".format(next(kwargs.keys())))
            ...
            for key, arg in callargs.items():
                if key not in which:
                    ...
                    try:
                        ...
                        raise TypeError("Failed to convert argument '{}' of {} "
                            "to a PICOS constant: {}".format(key, name(), e))
    ...
    *not* have arbitrary dimensions, then it must validate the element's shape
    ...
    def decorator(operator):
        ...
        def wrapper(lhs, rhs, *args, **kwargs):
            def error(reason="Operand shapes do not match."):
            ...
            if lhsIsExpOrSet and rhsIsExpOrSet:
                ...
                error("The operand shapes of {} and {} do not match."
                    .format(glyphs.shape(lhsShape), glyphs.shape(rhsShape)))
            elif lhsIsExpOrSet:
                ...
                try:
                    ...
                    error("Failed to load right hand side as a constant of "
                        "shape {}: {}".format(glyphs.shape(rhsShape), exc))
            elif rhsIsExpOrSet:
                ...
                try:
                    ...
                    error("Failed to load left hand side as a constant of "
                        "shape {}: {}".format(glyphs.shape(rhsShape), exc))
            else:
...
def cvxopt_inverse(matrix):
    ...
    try:
        ...
        raise ValueError(
            "Failed to invert CVXOPT matrix: {}".format(error))
