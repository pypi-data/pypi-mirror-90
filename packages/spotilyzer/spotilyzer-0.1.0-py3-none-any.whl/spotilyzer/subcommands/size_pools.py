"""
spotilyzer size-pools subcommand
"""
...
# constants
_SUBCMD = 'size-pools'
_DESCRIPTION = "size pools based on a request"
_BUFFER_OPT = 'buffer'
_BUFFER_DEFAULT = 20.0
...
_TABLE_HEADER = ('Group', 'Pool', 'Instance Count', 'Total Cost')
_TABLE_FORMAT = 'grid'
_TABLE_COL_ALIGN = ('left', 'left', 'right', 'right')


class SizePools(SubCommand):

    name = _SUBCMD

    @classmethod
    def add_parser(cls, subparsers):
        parser = subparsers.add_parser(_SUBCMD, description=_DESCRIPTION,
                                       help=_DESCRIPTION)
    ...
    def run(self):
        ...
        results = self._size_pools(groups, requirements)
    ...
    def _get_groups(self):
        ...
        for group in candidates[GROUPS_KEY]:
            pools = groups[group[GROUP_NAME_KEY]] = {}
            for candidate in group[CANDIDATES_KEY]:
                pools[candidate[CANDIDATE_NAME_KEY]] = {
    ...
    def _size_pools(self, groups, requirements):
        results = {}
        buffer = 1 + self.getarg(_BUFFER_OPT) / 100
        for group_name, pools in groups.items():
            ...
            for pool_name, pool_data in pools.items():
                resources = pool_data[RESOURCES_KEY]
                ...
                group_data[pool_name] = {
                    _INSTANCE_COUNT: count,
                    _TOTAL_COST: count * pool_data[PRICE_KEY]
                }
        return results
    ...
    def _display_results(self, results, candidates):
        sorted_pools = {
            group_name: sorted(
                group_data.keys(), key=lambda k: group_data[k][_TOTAL_COST]
            )
        ...
        table = [
            [
                ...
                '\n'.join(pool_names),
                '\n'.join(
                    f"{results[group_name][p][_INSTANCE_COUNT]}"
                    for p in pool_names
                ),
                '\n'.join(
                    f"{results[group_name][p][_TOTAL_COST]:.3f}"
                    for p in pool_names
                )
            ]
            for group_name, pool_names in sorted_pools.items()
        ]
