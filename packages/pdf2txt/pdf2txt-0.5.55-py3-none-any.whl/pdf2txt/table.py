        ...
        for row in self.textlines:
            ...
            for token in row:
                col = self.vertically_in_line_with(
                    token, self.textlines, tolerance=5
                )
                col_exists=False
 #               for c in cols:
 #                   if bool(c & col):
...
#        cols=self._fix_cols(cols, self.textlines)
    ...
    def _fix_cols2(self, cols, elements):
        """
        ...
        for element in [token for line in elements for token in line]:
 ...
 #           return cols
        ...
        for line in tokenlines:
            for token in line:
                ...
                cols_with_token = [col for col in cols if token in list(col)]
...
class TableFinder(object):
    ...
    def words_to_edges_h(self, word_threshold=DEFAULT_MIN_WORDS_HORIZONTAL):
        ...
        """
        by_top = utils.cluster_objects(self.page.tokens, "bottom", 1)
        if by_top is None:
            return []
    ...
    def guess_table_lines(self,token_threshold=3, pruning_cut_off=2):
        ...
        if not isinstance(self.page, Region):
