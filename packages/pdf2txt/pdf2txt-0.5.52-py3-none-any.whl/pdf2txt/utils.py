...
def are_in_same_Line(word1, word2, tol=2):
...
def is_ovarlaping_with_objects(bbox, objects):
    for o in objects:
        if isinstance(o, dict):
            o = BoundingBox(left=o["left"], top=o["top"], right=o["right"], bottom=o["bottom"])
        if has_overlap_with_bbox(o, bbox):
...
def cluster_list_multi(xs, tolerance=0):
    ...
    last = xs[0]
    for x in xs[1:]:
        if x[0] <= (last[0] + tolerance) and  x[1] <= (last[1] + tolerance):
            current_group.append(x)
        else:
            groups.append(current_group)
            current_group = [x]
...
def cluster_objects_multi(objs, attr1, attr2, tolerance):
    ...
    values = map(lambda p: (getattr(p, attr1), getattr(p, attr2)), objs)
...
def extract_chart_and_data():
    ...
    for component in edge['components']:
        ...
        for i in range(10):
            ...
            if len(value) > 0:
                ...
                if value_f < 0:
                    data.append(value_f)
                    bbox.top = value[0].bottom + 1
                    all_values += 1
                    continue
                elif value_f > 0 or isinstance(value_f, str):
    ...
    # go left until you touch text
    bbox_left = BoundingBox(left=edge['bbox'].left, top=edge['bbox'].top, bottom=edge['bbox'].bottom,
                            right=edge['bbox'].left)
    bbox_bottom = BoundingBox(left=edge['bbox'].left, top=edge['bbox'].bottom, bottom=edge['bbox'].bottom,
                              right=edge['bbox'].right)
    while not is_ovarlaping_with_objects(bbox_left, words):
        edge['bbox'].left -= 1
        bbox_left.left -= 1

    # once you touch text continue until you passed it
    while is_ovarlaping_with_objects(bbox_left, words):
        edge['bbox'].left -= 1
        bbox_left.left -= 1
        bbox_left.right -= 1

#        edge['bbox'].bottom=max(l.bottom for l in labels)+3
