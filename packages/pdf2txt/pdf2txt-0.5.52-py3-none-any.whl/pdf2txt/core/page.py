...
#        self._tokens=None
    ...
    def detect_semantic_structure_bk(self):
        self.regions.sort(key=lambda te: ( te.top, te.left))
        ...
        for region_number, region in enumerate(self.regions):
            ...
            if len(region.graphs):
                ...
                for graph in region.graphs:
                    top_paragraph=extract([tl for tl in region.text_lines if (tl[0].bottom<=graph.top or tl[0].left>=graph.right or tl[0].right<=graph.left) and tl[0].top>top])
                    if len(top_paragraph[0])>0:
                        paragraphs.extend(top_paragraph)
                    graph_data=graph.extract_chart_data()
                    if len(top_paragraph[0])>0 and len(graph_data['text'])>0:
                        top_paragraph.extend(graph_data['text'])
                    paragraphs.append(graph_data['dataframe'])
                    top=graph.bottom
            ...
            for n, p in enumerate(paragraphs):
                #try to combine paragraphs that do not have titles
                #also if there is only one font and one font size we should combine with the former paragraph
                if len(p)==0:
                    continue
                if isinstance(p, DataFrame):
                    ...
                    paragraph.add_content(p, ContentType.Graph)
                    continue
                elif n>0 and (p[0][0].font == paragraph_text_font or title_found or (len(font_statistics["size_by_frequency"])==1 and len(font_statistics["name_by_frequency"])==1)):
                    paragraph=self.document.paragraphs.get_last_paragraph()

                else:
                    paragraph = self.document.paragraphs.create_paragraph("paragraph")

                has_table=utils.get_partially_overlapping_objects(utils.get_BoundingBox_from_objects([t for l in p for t in l]), region.tables)

                lines_for_table = []
                for i, l in enumerate(p):
                    if (not title_found or (region_number==0 and n==0)) and self.is_title(i, region_number, l, font_statistics, title_font):
                        paragraph.title=l
                        if not (region_number==0 and n==0 and i<3):
                            title_found=True
                        continue
                    if has_table:
                        if len(p[i])<2:
                            if len(lines_for_table) > 0:
                                table = Table(region, textlines=lines_for_table)
                                paragraph.add_content(table.extract().to_pandas(), ContentType.Table)
                                lines_for_table = []
                                title_found = False
                            paragraph.add_content(l, ContentType.Text)
                            paragraph_text_font = l[0].font
                            title_found = False
                            continue
                        elif len(p[i])>0:
                            lines_for_table.append(p[i])
                        else:
                            paragraph.add_content(l, ContentType.Text)
                            paragraph_text_font = l[0].font
                            title_found = False
            ...
            for p in paragraphs:
                ...
                for n, tl in enumerate(p):
                    if isinstance(tl, DataFrame):
                        ...
                        if len(columns)>0:
                            first=columns[0]
                            t=self.get_token_wit_value(first)
                            if check_title and t is not None and self.is_title2([t], font_statistics, page_font_statistics,title_font):
...
#                        existing_title=self.document.paragraphs.filter_by_title_equal(tl)
    ...
    def extract_paragraph(self, region):
        ...
        if len(region.graphs):
            ...
            paragraphs = []
            for graph in region.graphs:
                top_paragraph = extract([tl for tl in region.text_lines if tl[0].bottom <= graph.top])
                if len(top_paragraph[0]) > 0:
                    paragraphs.extend(top_paragraph)
                graph_paragraph=[]
                graph_data = graph.extract_chart_data()
        ...
        for p in paragraphs:
            ...
            lines_for_table = []
            pp=[]
            if utils.get_partially_overlapping_objects(utils.get_BoundingBox_from_objects([t for l in p for t in l]), region.tables):
                for l in p:
                    if len(l)>=2 or (len(l)==1 and utils.get_type(l[0].Text.strip('%'))=="Numeric"):
                        lines_for_table.append(l)
                    elif len(lines_for_table) > 2:
                            table = Table(region, textlines=lines_for_table).extract()
...
#                    print(table.to_string())
    ...
    def is_title2(self, textline, font_stat, page_font_stat, title_font={}, font_body={}):
        ...
        if utils.get_type(textline[0].Text.replace('M€', '').replace('€', '').replace('/', '').strip())== "Numeric":
            return False
        if title_font and textline[0].font+str(textline[0].is_bold)==title_font['name']:
            return True
        elif title_font and textline[0].font_size>title_font['size']:
            return True
        elif title_font:
            return False
        if (len(font_stat["name_by_frequency"]) > 1 or len(font_stat["size_by_frequency"]) > 1) and textline[0].font_size > font_stat["most_frequent"]:
    ...
    def find_header(self, region):
        font_stat= region.font_statistics
        ...
        for l in region.text_lines[:3]:
            if first_horizontal_separator and l[0].top > first_horizontal_separator.top:
                return

            if l[0].font_size == font_stat["largest"] or l[0].font_size > font_stat["most_frequent"]:
                self.title.append(l)
    ...
    def get_token_wit_value(self, val):
    ...
    def graphs(self):
        ...
        self._graphs=extract_graphs(page=self)
        return self._graphs
    ...
    def text_lines(self):
        ...
        if doctop_clusters is not None:
            self.__text_lines = [line for line in doctop_clusters]
        else:
            self.__text_lines=[]
        return self.__text_lines
