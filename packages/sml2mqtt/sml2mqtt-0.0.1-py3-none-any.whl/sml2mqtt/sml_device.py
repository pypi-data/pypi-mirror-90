...
class Device:
    @classmethod
    async def create(cls, url: str, skip_values: List[str]):
        c = cls(url, set(skip_values))
        c.serial = await sml2mqtt.sml_serial.SmlSerial.create(url, c)
        ALL[url] = c
        return c
    ...
    async def read(self):
        analyze = sml2mqtt._args.ARGS.analyze
        try:
            while True:
                await sleep(0.4)

                # If the port is closed we have to restart the application so we might as well cancel the reader
                if self.status == DeviceStatus.PORT_CLOSED:
                    return None
