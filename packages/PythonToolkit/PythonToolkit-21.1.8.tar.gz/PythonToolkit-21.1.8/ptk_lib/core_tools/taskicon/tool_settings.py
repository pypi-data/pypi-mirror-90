...
class ToolManagerPanel(wx.Panel):
    ...
    def Update(self, hide_core=False):
        ...
        tools = [t for t in zip( avail.keys(), avail.values() )]
        tools.sort()
        self.items = {}
...
class ToolItem(wx.Panel):
    def __init__(self,parent,tool):
        ...
        hbox.Add(self.info,0,wx.ALL|wx.ALIGN_CENTER_VERTICAL,5)

        #enable tool checkbox
        self.cb = wx.CheckBox(self, -1, 'Enable: ',style=wx.ALIGN_RIGHT)
        hbox.Add(self.cb,0,wx.ALL|wx.ALIGN_CENTER_VERTICAL,5)
...
class ToolInfoDialog(wx.Dialog):
    def __init__(self, parent, tool):
        ...
        hbox.Add(grid,1,wx.EXPAND|wx.LEFT|wx.RIGHT,5)
