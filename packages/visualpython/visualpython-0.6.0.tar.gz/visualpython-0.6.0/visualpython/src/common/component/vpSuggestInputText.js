define([
    ...
    vpSuggestInputText.prototype.toTagString = function() {
        ...
        sbTagString.appendFormatLine(`<input type='text' class='{0} {1} {2}' {3} placeholder='{4}' value="{5}"/>`
            , that._UUID, vpConst.VP_SUGGEST_INPUT_UNINIT, that._additionalClass, that._compID == "" ? "" : vpCommon.formatString("id='{0}'", that._compID), that._placeholder, that._value);
        ...
        $(document).on(vpCommon.formatString("focus.init-{0}", that._UUID), vpCommon.formatString(".{0}.{1}", that._UUID, vpConst.VP_SUGGEST_INPUT_UNINIT), function() {
            ...
            $(vpCommon.formatString(".{0}", that._UUID)).autocomplete({
                    ...
                    if (that._normalFilter) {
                        for (var idx = 0; idx < srcList.length; idx++) {
                            ...
                            if (typeof srcList[idx] == "object") {
                                ...
                                if (srcList[idx].label.toLowerCase().includes(req.term.trim().toLowerCase())) {
                                    returlList.push(srcList[idx]);
                                }
                            } else if (srcList[idx].toLowerCase().includes(req.term.trim().toLowerCase()))
                                returlList.push(srcList[idx]);
                        }
                    } else {
                        returlList = srcList;
                ...
                , select: function(evt, ui) { if (typeof that._selectEvent == "function") that._selectEvent(ui.item.value); }
            }).focus(function() {
