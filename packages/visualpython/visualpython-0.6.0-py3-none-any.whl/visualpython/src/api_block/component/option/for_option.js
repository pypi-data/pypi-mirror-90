define([
    ...
    const { GenerateForParam
            , GenerateListforConditionList  } = blockRenderer;
    ...
    var InitForBlockOption = function(thatBlock, optionPageSelector) {
        ...
        $(document).on(STR_CHANGE, VP_ID_PREFIX + VP_ID_APIBLOCK_OPTION_FOR_TYPE_SELECT + uuid, function(event) {
            ...
            var forParamStr = STR_NULL;
            if ( selectedVal == FOR_BLOCK_TYPE.FOR ) {
                forParamStr = GenerateForParam(thatBlock);
                $(VP_CLASS_PREFIX + VP_CLASS_APIBLOCK_BLOCK_HEADER + uuid).html(forParamStr);
                thatBlock.getBlockHeaderDom().find(STR_STRONG).css(STR_DISPLAY, STR_FLEX);
            } else {
                forParamStr = GenerateListforConditionList(thatBlock);
                $(VP_CLASS_PREFIX + VP_CLASS_APIBLOCK_BLOCK_HEADER + uuid).html(forParamStr);
                thatBlock.getBlockHeaderDom().find(STR_STRONG).css(STR_DISPLAY, STR_NONE);
            }
        ...
        var renderThisComponent = function() {
            ...
            /** For Type 설정 */
            var forName = STR_FOR;
            var forBlockOptionType = thatBlock.getState(STATE_forBlockOptionType);
            ...
            var selectedListfor = forBlockOptionType == FOR_BLOCK_TYPE.LIST_FOR
                                    ? STR_SELECTED 
                                    : STR_NULL;  
            var sbforNameSelectType = new sb.StringBuilder();
            sbforNameSelectType.appendFormatLine("<div class='{0} {1}'>", VP_CLASS_STYLE_FLEX_ROW_END, VP_CLASS_STYLE_MARGIN_TOP_5PX);
            sbforNameSelectType.appendFormatLine("<div class='{0}'>", 'vp-apiblock-tab-navigation-node-block-title');

            sbforNameSelectType.appendFormatLine("<select id='{0}'>", VP_ID_APIBLOCK_OPTION_FOR_TYPE_SELECT + uuid);
            sbforNameSelectType.appendFormatLine("<option value='{0}' '{1}'>", STR_FOR, selectedFor);
            sbforNameSelectType.appendFormatLine("{0}", STR_FOR);
            sbforNameSelectType.appendLine("</option>");
            sbforNameSelectType.appendFormatLine("<option value='{0}' '{1}'>", 'listfor', selectedListfor);
            sbforNameSelectType.appendFormatLine("{0}", 'listfor');
            sbforNameSelectType.appendLine("</option>");
            sbforNameSelectType.appendLine("</select>");
            sbforNameSelectType.appendLine("</div>");
            sbforNameSelectType.appendLine("</div>");
            optionContainerDom.append(sbforNameSelectType.toString());
    

            var forParamState = thatBlock.getState(STATE_forParam);  
            ...
            //     var sbforParamArg7 = MakeVpSuggestInputText_apiblock(VP_ID_APIBLOCK_OPTION_FOR_ARG_3_INPUT_STR + uuid
            //                                                         , arg3InputStr
            //                                                         , loadedVariableNameList_arg7
            //                                                         , VP_CLASS_STYLE_WIDTH_40PERCENT
            //                                                         , STR_VALUE
            //                                                         , function(selectedValue) {
            //                                                             bindSelectValueEventFunc_for(selectedValue, 
            //                                                                 FOR_BLOCK_SELECT_VALUE_ARG_TYPE.ARG3_INPUT_STR);
            //                                                         });
            //     sbforArgContainer.appendLine(sbforParamArg7);
            // }
            ...
            if (arg3 == FOR_BLOCK_ARG3_TYPE.INPUT_STR) {
                $(VP_ID_PREFIX + VP_ID_APIBLOCK_OPTION_FOR_ARG_3 + uuid).val(STR_NULL);
            } else {
