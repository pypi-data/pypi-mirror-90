...
# @Last Modified time: 2021-01-04 18:35:36
...
class LocalServer(object):
	def __init__(self,alreadyRecord,JavaCoverageReportUrl="",
		...
		self.mergeFile = JAVA_COV["mergeFile"].format(week=self.alreadyRecord.week_num,
			service_name=self.alreadyRecord.service_name)
		self.destfile = JAVA_COV["destfile"].format(week=self.alreadyRecord.week_num,service_name=self.alreadyRecord.service_name,recordID=self.alreadyRecord.id)
		self.mdestfile = JAVA_COV["mdestfile"].format(week=self.alreadyRecord.week_num,service_name=self.alreadyRecord.service_name)
	...
	def cpFile(self):
		...
		for jcIndex in eval(self.alreadyRecord.sn_id):
			command = "cp {jacocoIt} {mergeFile}/{index}.exec".format(jacocoIt=self.jacocoIt.format(
				week=self.alreadyRecord.week_num,service_name=self.alreadyRecord.service_name,index=jcIndex),mergeFile=self.mergeFile,
				index=jcIndex)
