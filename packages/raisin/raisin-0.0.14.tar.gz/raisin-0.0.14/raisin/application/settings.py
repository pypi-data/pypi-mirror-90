...
from ..tools import Printer, id_, get_temperature
    ...
    def get_settings(self):
        ...
        if not self._is_complete(settings):
            with Printer("Account settings...") as p:
                ...
                settings["account"]["username"] = str(settings["account"].get("username", id_().username))
            ...
            with Printer("Server settings...") as p:
                ...
                settings["server"]["network_name"] = settings["server"].get("network_name", "main")
        ...
        if "cluster_work" in settings:
            if "recording_directory" in settings["cluster_work"]:
                if not os.path.isdir(os.path.join(settings["cluster_work"]["recording_directory"], "results")):
                    _make_tree(
                        os.path.join(settings["cluster_work"]["recording_directory"], "results")
                        )
...
def _make_tree(tree_path, indentation=0):
    ...
    if os.path.isfile(tree_path):
        with Printer("Deleted '%s'..." % tree_path):
            os.remove(tree_path)
    dirname = os.path.dirname(tree_path)
    with Printer("Make '%s'..." % tree_path):
