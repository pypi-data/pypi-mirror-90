...
def decipher_aes(data, passphrase):
    ...
    with Printer("Deciphering bloc with AES..."):
        ...
        try:
            ...
            raise DecryptError("La passphrase n'est pas la bonne.") from e
...
def decipher_rsa(data, private_key_pem):
    ...
    with Printer("Deciphering block with RSA..."):
        ...
        try:
            ...
            raise DecryptError("La clef privee n'est pas la bonne.") from e
...
def _decipher_file(path, **kwargs):
    """
    |=======================|
    ...
    >>> _decipher_file("monfichier.txt.crais")
    'monfichier.txt'
    >>>
    """
    ...
    with Printer("Decryption of the file %s..." % repr(path)):
        if filename[-6:] == ".crais":
            new_file = filename[:-6]
        else:
            new_file = "deciphered_" + filename
        new_file = filename + ".crais"
        try:
...
def authenticity(gen):
    ...
    with Printer("Verify authenticity..."):
        ...
        for is_end, pack in anticipate(deconcat_gen(gen=gen, pack=pack)):
            if is_end:
                if hash_ != pack:
                    raise DecryptError("Les donnees ne sont pas authentiques!")
                break
