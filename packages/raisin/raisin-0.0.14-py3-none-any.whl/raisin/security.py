...
def rsa_keys(*, psw=None):
    ...
    with Printer("Generation of RSA keys (it can take a time)..."):
...
def change_private_key(private_key_pem, old_psw, new_psw, *, signature=None):
    """
    |====================================================|
    ...
    with Printer(sentence, signature=signature):
...
def get_private_key(*, force=False, signature=None):
    ...
    | Le clef retournee est dechiffree.    |
    |======================================|
    ...
    >>>
    """
    assert type(force) is bool, "'force' doit etre un booleen, pas %s." % type(force)

    with Printer("Recuperation de la clef privee raisin...", signature=signature):
        settings = load_settings()
        psw = request_psw(force=force, check=False)
        try:
            key_pem = settings["account"]["security"]["private_key"]
        except KeyError: # si il n'y a pas de clef privee
            settings["account"] = settings.get("account", {})
            settings["account"]["security"] = settings["account"].get("security", {})
            if "public_key" in settings["account"]["security"]:
                raise RuntimeError("Il y a une clef publique mais pas privee! "
                                    "Vous avez touches a quelque chose!")
            key_pem, public_key = rsa_keys(psw=psw, signature=signature)
            settings["account"]["security"]["private_key"] = key_pem
            settings["account"]["security"]["public_key"] = public_key
...
def get_public_key(*, signature=None):
    r"""
    |======================================|
    | Recupere la clef publique de raisin. |
    | Si elle n'existe pas, elle est cree. |
    |======================================|
    ...
    sortie
    ------
    :return: la clef publique serialisee au format PEM.
    :rtype: bytes

    exemple
    -------
    :Example:
    >>> from raisin.security import get_public_key
    >>> get_public_key()
    b'-----BEGIN PUBLIC KEY-----\nMIIEIjANBgkqhkiG9w0BAQEFAAOCBA8AMIIECgKC
    BAEA2ZNhutNMYZ1BdGSFRlE7\nL7pnOiXxrcoj6xdp2bnEEkfPUl61630KwqEW5w1jIka3
        ...
    jspNlQ/qs3Zq4ukkSRfcc3+TFoKHYJlHc/QORv62\nsezGe249budYyoYaOK+hBsfG4xWR4
    vkipC323ptMYDL4jYd3iQj33nOWIZUtv/S\nkQIDAQAB\n-----END PUBLIC KEY-----'
    >>>
    """
    with Printer("Recuperation de la clef publique raisin...", signature=signature):
        settings = load_settings()
        try:
            return settings["account"]["security"]["public_key"]
        except KeyError: # si il n'y a pas de clef publique
            settings["account"] = settings.get("account", {})
            settings["account"]["security"] = settings["account"].get("security", {})
            if "private_key" in settings["account"]["security"]:
                raise RuntimeError("Il y a une clef privee mais pas publique! "
                                    "Vous avez touches a quelque chose!")
            psw = request_psw(force=False, check=False)
            private_key, public_key = rsa_keys(psw=psw, signature=signature)
            settings["account"]["security"]["private_key"] = private_key
            settings["account"]["security"]["public_key"] = public_key
            dump_settings(settings)
            return public_key
