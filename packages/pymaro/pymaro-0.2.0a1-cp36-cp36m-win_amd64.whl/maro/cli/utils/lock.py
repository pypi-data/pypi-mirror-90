...
def acquire_lock(cluster_name: str, operation: str):
    logger.debug(f"Acquire lock at {cluster_name} with operation {operation}")

    # Load details
    lock_details = load_lock_details(cluster_name=cluster_name)
    ...
    save_lock_details(cluster_name, lock_details)


def release_lock(cluster_name: str, operation: str):
