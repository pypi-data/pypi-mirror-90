...
class StatusBar:
    def __init__(self):
        self.distance = 0
        self.hp = 2
        with open("resource/score.json", "r") as self.f:
    ...
    def draw_top_score(self):
        ...
        draw_text(f"最高分：{self.top_score}", pos_x, pos_y, color.BLUE, font_name=("simhei", "PingFang"))
    ...
    def draw_hp(self):
        ...
        for i in range(self.hp):
            heart = Sprite("resource/heal.png")
...
class MyCar(Window):
    ...
    def setup(self):
        self.road1 = Road("resource/road.png")
        self.road2 = Road("resource/road.png")
        self.road2.center_y = SCREEN_HEIGHT // 2 + SCREEN_HEIGHT
        self.small_car_list = [f"resource/small_car{str(i)}" for i in range(1, 5)]
    ...
    def create_carts(self):
        cart_list = ["resource/big_car1.png", "resource/big_car2.png", "resource/big_car3.png"]
        num = 2
    ...
    def judge_game_status(self):
        if self.status_bar.hp <= 0:
            ...
            if self.status_bar.distance > self.status_bar.top_score:
                ...
                with open("resource/score.json", "w") as f:
