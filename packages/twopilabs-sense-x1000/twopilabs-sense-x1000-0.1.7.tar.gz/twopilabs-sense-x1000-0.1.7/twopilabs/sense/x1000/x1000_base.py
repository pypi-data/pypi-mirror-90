...
class SenseX1000Base(object):
    ...
    class AcqData(object):
        ...
        def __init__(self, data: bytes, header: 'SenseX1000Base.AcqHeader', seq_num: int, n_sweeps: int) -> None:
            ...
            # Calculate slopes (alternating sign if ALTERNATING flag is set, otherwise constant sign)
            self.sweep_slope = 1 if header.acq_flags & SenseX1000Base.AcqFlags.SLOPE else -1
            self.sweep_slopes = [-self.sweep_slope if seq_num % 2 else self.sweep_slope
                                 if header.acq_flags & SenseX1000Base.AcqFlags.ALTERNATING
                                 else self.sweep_slope for seq_num in self.seq_nums]
