...
class GeventWorker(AsyncWorker):
    ...
    def handle_request(self, listener_name, req, sock, addr):
        try:
            content_length = dict(req.headers).get("CONTENT-LENGTH", 0)
            setattr(req, "body", BytesIO(req.body.read(int(content_length))))
            gevent.spawn(super(GeventWorker, self).handle_request, listener_name, req, sock,
                                                     addr)
