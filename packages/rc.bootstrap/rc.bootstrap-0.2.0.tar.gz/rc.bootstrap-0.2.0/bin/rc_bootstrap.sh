...
out "# -------------------------------------------------------------------\n"
out "#\n# install miniconda\n"
if test -x "miniconda/bin/python3.8"
then
...
out "# -------------------------------------------------------------------\n"
out "#\n# create env $RC_NAME\n#\n"
conda create -y -p "$ENV_LOC" python=3.8 2>&1 | progress
check "$?" "miniconda failed"

out "# \n"
out "# -------------------------------------------------------------------\n"
out "#\n# activate env $env_loc\n#\n"
conda activate "$ENV_LOC" 2>&1
...
pip install 'git+https://github.com/radical-cybertools/radical.utils.git@am/rc' \
    | progress
