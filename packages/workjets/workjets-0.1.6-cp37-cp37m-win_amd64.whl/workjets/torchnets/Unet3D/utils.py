...
def save_checkpoint(state, is_best, checkpoint_dir, logger=None):
    ...
    log_info("Saving last checkpoint to '{last_file_path}'")
    torch.save(state, last_file_path)
    if is_best:
        best_file_path = os.path.join(checkpoint_dir, 'best_checkpoint.pytorch')
        log_info("Saving best checkpoint to '{best_file_path}'")
...
def adapted_rand(seg, gt, all_stats=False):
    ...
    ----------
    seg : np.ndarray
        the groundtruth to score, where each value is the label at that point
