...
class UNet3D(nn.Module):
    ...
    Args:
        ...
        out_channels (int): number of output groundtruth masks;
            Note that that the of out_channels might correspond to either
            different semantic classes or to different binary groundtruth mask.
    ...
    def forward(self, x):
        ...
        # logits and it's up to the user to normalize it before visualising with tensorboard
        # or computing validation metric
        #  TODO:  这个不正确  不管怎么样都输出sigmoid/softmax之前的 logits ！！
...
class ResidualUNet3D(nn.Module):
    ...
    def forward(self, x):
        ...
        # apply final_activation (i.e. Sigmoid or Softmax) only for prediction. During training the network outputs
        # logits and it's up to the user to normalize it before
        # visualising with tensorboard or computing validation metric
        #  TODO:  这个不正确  不管怎么样都输出sigmoid/softmax之前的 logits ！！
        # if not self.training:
        #     x = self.final_activation(x)
        # x = self.final_activation(x)
...
if __name__ == '__main__':
    ...
    net = ResidualUNet3D(in_channels=4, out_channels=3, final_sigmoid=True)
    net.to(device=torch.device('cpu'))
    # from torch import optim
    # optimizer = optim.Adam(list(net.parameters()), 0.002, (0.8, 0.99))

    # data = torch.rand(size=(1, 1, 32, 32, 32))
    # pred = net(data)
    # print(pred.type())
    # print(pred.detach().numpy().dtype)
