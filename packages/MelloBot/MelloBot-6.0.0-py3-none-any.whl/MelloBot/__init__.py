...
"""
“Commons Clause” License Condition Copyright Pirxcy/Mello 2019-2020 / 2020-2020
...
try:
    ...
    print(intro)
    print("YOU FOOL PIRACY IS NO PARTY\nNext Time Dont Modify ")
...
if True: #Classes
    ...
    class Client(fortnitepy.Client):
        def __init__(self, email: str, emote: str, **kwargs: Any) -> None:
            self.email = email
            self.status_ = "Made By Pirxcy and Mello"
            self.hides = []
            self.eid = emote
            self.boot_time = None
            ...
            self.discord = False
            self.web = True
            self.whisperperfect = True
            self.partychatperfect = True
            self.discordperfect = True
            self.joinmessageenable = True
            self.randommessageenable = True
            self.outfitmimic = False
            self.backpackmimic = False
            self.pickaxemimic = False
            self.emotemimic = False
            self.outfitlock = False
            self.backpacklock = False
            self.pickaxelock = False
            self.emotelock = False
        ...
        async def ng_cosmetic_check(self, member: fortnitepy.PartyMember) -> None:
            if (member.id in (otherbotlist + [i.user.id for i in loadedclients])
                    or member.id in [i.id for i in self.owner]):
                return
            checks = [
                "outfit",
                "backpack",
            ...
            for check in checks:
                asset = member_asset(member, check)
                if asset and asset.lower() in [i.lower() for i in data['fortnite']['ng_cosmetics']]:
                    if self.party.me.leader:
                        if data['fortnite']['ng_cosmetic_kick']:
                            await member.kick()
                        elif data['fortnite']['ng_cosmetic_chatban']:
                            await member.chatban()
                        friend = self.get_friend(member.id)
                        if friend is not None:
                            await friend.send(data['fortnite']['ng_cosmetic_reply'])
                        else:
                            if not data['fortnite']['ng_cosmetic_kick'] and not data['fortnite']['ng_cosmetic_chatban']:
                                await self.party.send(data['fortnite']['ng_cosmetic_reply'])
                        break
        ...
        async def event_party_member_join(self, member: fortnitepy.PartyMember) -> None:
            ...
            await self.ng_cosmetic_check(member)
            
            if not self.party.me.leader and (data['fortnite']['hide-user'] or data['fortnite']['hide-blacklist']):
                if data['fortnite']['hide-user']:
                    if (not (member.id in [owner.id for owner in self.owner] and data['fortnite']['show-owner'])
                        ...
                        self.hides.append(member.id)
                        self.party.update_hide_users(self.hides)
            ...
            if self.joinmessageenable:
                ...
                try:
                    mes = eval_formatf("Hey {client.display_name}",var)
        ...
        async def event_party_member_confirm(self, confirmation: fortnitepy.PartyJoinConfirmation) -> None:
            ...
            if data['fortnite']['blacklist-autokick'] and confirmation.user.id in blacklist:
                ...
                if data['fortnite']['hide-user'] or data['fortnite']['hide-blacklist']:
                    if data['fortnite']['hide-user']:
                        if (not (confirmation.user.id in [owner.id for owner in self.owner] and data['fortnite']['show-owner'])
                            and not (confirmation.user.id in whitelist and data['fortnite']['show-whitelist'])
                            and not (confirmation.user.id in (otherbotlist + [i.user.id for i in loadedclients]) and data['fortnite']['show-bot'])
                            and confirmation.user.id != self.user.id):
                            self.hides.append(confirmation.user.id)
                            self.party.update_hide_users(self.hides)
                    elif data['fortnite']['hide-blacklist']:
                        if confirmation.user.id in blacklist:
                            self.hides.append(confirmation.user.id)
                            self.party.update_hide_users(self.hides)
...
if True: #Functions
        ...
        if data['discord-log']:
            if data['hide-email']:
                try:
                    ...
                    print(__import__('traceback').format_exc());input()
    ...
    def confiig(client: Optional[fortnitepy.Client] = None) -> bool:
        global data
        global commands
        global replies
        try:
    ...
    def load_config(client: Optional[fortnitepy.Client] = None) -> bool:
        ...
        try:
            ...
            send('ボット','info.json ファイルの読み込みに失敗しました。正しく書き込めているか確認してください',red,add_d=lambda x:f'>>> {x}')
            send('Bot','Failed to load info.json file. Make sure you wrote correctly',red,add_d=lambda x:f'>>> {x}')
            return False
        except FileNotFoundError:
            send('ボット',traceback.format_exc(),red,add_d=lambda x:f'>>> {x}')
            send('ボット','info.json ファイルが存在しません',red,add_d=lambda x:f'>>> {x}')
            send('Bot','info.json file does not exist',red,add_d=lambda x:f'>>> {x}')
            return False
        ...
        if error_config:
            send('ボット',f'info.json ファイルの読み込みに失敗しました。キーの名前が間違っていないか確認してください。アップデート後の場合は、最新のinfo.jsonファイルを確認してください\n{", ".join(error_config)} がありません',red,add_d=lambda x:f'>>> {x}')
            send('Bot',f'Failed to load info.json file. Make sure key name is correct. If this after update, plase check latest info.json file\n{", ".join(error_config)} is missing',red,add_d=lambda x:f'>>> {x}')
    ...
    def get_item_data(lang: str) -> dict:
        res = requests.get("https://fortnite-api.com/v2/cosmetics/br/", params={"lang": lang})
    ...
    if args[0] in commands['restart']:
        ...
        try:
            ...
            if len(args) > 4:
                ...
                background_colors = getattr(fortnitepy.KairosBackgroundColorPreset, args[2].upper())
            ...
            if rawcontent == '':
                ...
                if len(users) == 1:
                    ...
                    if not member:
                        await reply(message, client, l('user_not_in_party'))
                        return
                    await client.party.hide(member)
            ...
            if rawcontent == '':
                ...
                if len(users) == 1:
                    ...
                    if not member:
                        await reply(message, client, l('user_not_in_party'))
                        return
                    await client.party.show(member)
        ...
        try:
            async with aiohttp.ClientSession() as session:
                res = await session.get("https://fortnite-api.com/v2/cosmetics/br/new")
                res = await res.json()
            flag = False
            items = res["items"]
            for item in items:
        ...
        try:
            ...
            for item in (store.featured_items
                ...
                for grant in item.grants:
                    type_ = convert_backend_type(grant["type"])
                    if type_ is None or type_ in ignoretype:
                        continue
                    item = {
                        "id": grant["asset"],
                        "type": convert_to_asset(convert_to_old_type(type_)),
        ...
        try:
            ...
            variants = client.party.me.create_variants(**variantdict)
            type_ = convert_to_new_type(type_)
            if type_ == "Back Bling" and (id_.startswith("pet_carrier_") or id_.startswith("pet_")):
                type_ = "Pet"
...
config_tags={
    ...
    "['fortnite']['emote']": [str],
    "['fortnite']['ng_cosmetics']": [str,"can_be_multiple"],
    "['fortnite']['ng_cosmetic_kick']": [bool_,"select_bool"],
    "['fortnite']['ng_cosmetic_chatban']": [bool_,"select_bool"],
    "['fortnite']['ng_cosmetic_reply']": [str],
