import os, sys, sanic, types, asyncio, importlib, traceback, MelloBot
    ...
    def __init__(self, client, cogs, *args, **kwargs):
        ...
        for proc in process_iter():
            ...
            for location in cogs:
                for path in Path(f"{MelloBot.__path__[0]}/{location}").rglob('*.pyc'):
                    if not str(path).endswith('__init__.pyc'):
                        path = str(path).split(f"{MelloBot.__path__[0]}/{location}")[1]
                        try:
                            self.load_extension(f"MelloBot.{location}{str(path).replace('/', '.')[:-4]}")
