from src.application.utils import execute, execute_sync
...
def setup_receive_wal_service(name, host, port, version, username, password, slot, destination):
    ...
    execute_sync('chown postgres:postgres {}'.format(destination))
...
def take_basebackup(name, host, port, version, username, password, destination):
    command = '/usr/lib/postgresql/{version}/bin/pg_basebackup -h {host} -p {port} -D {destination} -U {username} -v --checkpoint=fast'.format(
        host=host,
        port=port,
        version=version,
        username=username,
...
def config_recovery_file(name, host, port, version, username, password, base_destination, wal_destination, slot, delay):
    ...
    execute_sync('chown postgres:postgres {}/recovery.conf'.format(base_destination))
