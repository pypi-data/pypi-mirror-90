        ...
        if msg.type == 'text':
            ...
            if msg_dict['Event'] == 'unsubscribe':
                ...
                if queryset:
                    ...
                    if 'bluedot_event_' in msg_dict['EventKey']:
                        event_id = msg_dict['EventKey'].split("_")[2]
                        default_subscribe_text = f"<a href='https://cpa-global-wechat.bluewebonline.com/html-dev/activity_details?id={event_id}'>点击此处查看活动详情>>></a>"
                    elif 'bluedot_pc_login_' in msg_dict['EventKey']:
                        requests.get(
                            f"http://cpa-global-wechat.bluewebonline.com/api-dev/wechat/login-websocket?scene_str={msg_dict['EventKey']}&openid={msg_dict['FromUserName']}")
                        default_subscribe_text = default_subscribe_text
