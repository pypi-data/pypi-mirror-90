...
from bluedot_rest_framework.utils.jwt_token import jwt_get_wechatid_handler, jwt_get_openid_handler
...
class FrontendView:
        ...
        if page is not None:
            ...
            for item in data['data']:
                ...
                if EventRegister.objects.filter(
                        wechat_id=wechat_id, event_id=data['event_id']):
                    item['is_register'] = 1
    ...
    def share_qrcode_officialaccount(self, request, *args, **kwargs):
        ...
        _id = request.query_params.get('id', '')
        wechat_id = jwt_get_wechatid_handler(request.auth)
        openid = jwt_get_openid_handler(request.auth, appid)
