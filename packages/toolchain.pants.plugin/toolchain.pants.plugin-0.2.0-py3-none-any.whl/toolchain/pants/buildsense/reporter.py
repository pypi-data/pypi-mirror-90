...
from toolchain.pants.buildsense.common import RunTrackerBuildInfo, WorkUnitsMap
from toolchain.pants.buildsense.state import BuildState
from toolchain.pants.common.toolchain_setup import ToolchainSetup

try:
    from pants.engine.streaming_workunit_handler import (  # pylint: disable=ungrouped-imports
    ...
    from pants.reporting.streaming_workunit_handler import (  # pylint: disable=ungrouped-imports
        StreamingWorkunitContext,
        WorkunitsCallback,
        WorkunitsCallbackFactory,
        WorkunitsCallbackFactoryRequest,
...
class ReporterCallback(WorkunitsCallback):
    ...
    def __init__(
        ...
        if not repo_name:
            ...
            return
        if not auth_store.get_auth_state().is_ok:
            logger.warning("Auth failed - BuildSense plugin is disabled.")
            return
        client = BuildSenseClient.from_options(
            client_options=options, get_access_token_callback=auth_store.get_access_token, repo=repo_name
        )
    ...
    def handle_workunits(
        ...
        *,
        ...
        if self._call_count == 0 and not finished:
            # If there first call is also the last call (finished=True) then we don't the initial report to buildsense.
            self._enqueue_initial_report(context.run_tracker)
        if finished:
            self._on_finish(context.run_tracker, self._call_count, work_units_map)
        else:
            self._build_state.queue_workunits(self._call_count, work_units_map)
        self._call_count += 1

    def _enqueue_initial_report(self, run_tracker: RunTracker) -> None:
        run_tracker_info = self._get_run_tracker_info(run_tracker)
        logger.debug(f"enqueue_initial_report {run_tracker_info.run_id}")
        self._build_state.queue_initial_report(run_tracker_info)

    def _on_finish(self, run_tracker: RunTracker, call_count: int, work_units_map: WorkUnitsMap) -> None:
        run_tracker_info = self._get_run_tracker_info(run_tracker)
    ...
    def _get_run_tracker_info(self, run_tracker: RunTracker) -> RunTrackerBuildInfo:
        ...
        # Support for https://github.com/pantsbuild/pants/pull/11382
        # We can eventually remove support/use of run_tracker.v2_goals_rule_names
        goals = getattr(run_tracker, "goals", []) or run_tracker.v2_goals_rule_names
        if goals:
            # We already have this in the service side, maybe migrate it to use a better name. but the name is internal anyway right now.
            run_info["computed_goals"] = list(goals)
        ...
        if has_ended:
            # https://github.com/pantsbuild/pants/pull/11385 keeping both options for backward compatibility
            if hasattr(run_tracker, "get_cumulative_timings"):
