...
class APIClient(object):
    ...
    def __init__(self, url=None, proxies=None, timeout=10.0,
        max_retries=2, headers=None):
    ...
    def doLogInfo(self, msg):
        msg = '%s %s: %s' % (self.__class__.__name__, self.cName,
            msg)
        LOGGER.info(msg)

    def doLogError(self, msg):
        msg = '%s %s: %s' % (self.__class__.__name__, self.cName, msg)
        LOGGER.error(msg)
    ...
    def doRequest(self, method, url, headers=None, params=None, data=None,
        ...
        headers = None
        if method == 'POST':
            # use content type for POST requests
            headers = {'Content-Type': 'application/json'}
        # log request
        self.doLogInfo("Request: %s, Data: %s" % (url, jData))
        resp = self.session.request(method, url, headers=headers,
            params=params, data=data, json=jData, proxies=self.proxies,
            verify=False)
        self.doLogInfo("Response: %s, Data: %s" % (url, resp.json()))
        return resp
