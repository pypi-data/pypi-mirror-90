...
class LoginTimes:
    def __init__(self, data):
        self.first = datetime.date.fromtimestamp(data.get("player", {}).get("firstLogin", 0) / 1000)
        self.last = datetime.date.fromtimestamp(data.get("player", {}).get("lastLogin", 0) / 1000)
