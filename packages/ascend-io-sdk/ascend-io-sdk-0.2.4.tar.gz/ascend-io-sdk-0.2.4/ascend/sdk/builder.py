...
from ascend.sdk.definitions import (Dataflow, ReadConnector, Transform, WriteConnector, ComponentGroup, ComponentUuidToIdMap, DataFeed, DataFeedConnector,
                                    DataService)
...
def data_service_from_proto(client: Client, proto: api_pb2.DataService) -> 'DataService':
  ...
  """
  dataflow_protos = client.list_dataflows(proto.id).data
  return DataService(
      id=proto.id,
      name=proto.name,
      description=proto.description,
      dataflows=[dataflow_from_proto(client, proto.id, dataflow_proto) for dataflow_proto in dataflow_protos],
  )
...
def dataflow_from_proto(client: Client, data_service_id: str, proto: api_pb2.Dataflow) -> 'Dataflow':
  ...
  """
  component_protos = client.list_dataflow_components(data_service_id, proto.id, deep=True).data
  ...
  for c in component_protos:
    if c.type == "sub":
      ...
      component_ids = [uuid_map[input.uuid] for input in c.content]
