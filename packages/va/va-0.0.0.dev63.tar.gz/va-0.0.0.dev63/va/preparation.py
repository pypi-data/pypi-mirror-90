...
class PreParation:
    ...
    def __init__(self):
        ...
        if self.emdid:
            ...
            self.fscfile, self.method, self.resolution, self.masks, self.run, self.platform = self.read_json(self.json)
    ...
    def read_json(self, injson):
        ...
        if injson:
            ...
            return (map, workdir, models, cl, evenmap, oddmap, fscfile, method, resolution, masks, runs, platform)
        else:
    ...
    def read_para():
        ...
        checkpar = (isinstance(args.m, type(None)) and isinstance(args.f, type(None)) and
                    ...
                    isinstance(args.mscl, type(None)) and isinstance(args.p, type(None)))
        ...
        for (key, value) in chaindict.items():
            ...
            for chainfile in value:
                ...
                try:
                    ...
                    if sys.version_info[0] == 2:
                        modelmap = emda_methods.model2map(chainfile, dim, float(self.resolution), unit_cell, maporigin=origin)
                    else:
            ...
            if self.method == 'sp' and self.resolution:
                ...
                for model in self.model:
                    try:
                        ...
                        modelmap = emda_methods.model2map(self.dir + model, dim, float(self.resolution), unit_cell, maporigin=origin)
                        emda_methods.write_mrc(modelmap, modelmapname, unit_cell, map_origin=origin)
                        modelmaps.append(modelmapname)
                    except:
                        err = 'Simulating model({}) map error:{}.'.format(model, sys.exc_info()[1])
                    ...
                    if errlist:
                        try:
                            modelmapname = '{}{}_modelmap.map'.format(self.dir, model)
                            modelmap = emda_methods.model2map(self.dir + model, dim, float(self.resolution), unit_cell, lig=True, maporigin=origin)
                            emda_methods.write_mrc(modelmap, modelmapname, unit_cell, map_origin=origin)
                            modelmaps.append(modelmapname)
                            print('Model map produced without the ligand description.')
                        except:
                ...
                print("No model map is produced as this is either not a single particle entry or nothing specified for method"
                      "-m (please use: -m sp) or resolution not specified -s (please use: -s <value>).")
