...
class BaseOpThread(threading.Thread):
    ...
    async def _bind_to_socket(self, socket: BaseSocket = None) -> bool:
        ...
        try:
            await socket.bind()
            _log.debug(f"Socket bound for {self._op_type.value}ing")
            return True
        except BindFailedException:
            _log.warning(f"Socket binding failed for {self._op_type.value}ing", exc_info=1)
            return False
    ...
    async def _unbind_from_socket(self, socket: BaseSocket = None, bound: bool = False):
        ...
        try:
            if bound:
                await socket.close()
                _log.debug(f"Socket unbound for {self._op_type.value}ing")
    ...
    async def _runner(self):
        ...
        try:
            ...
            while self._running or operating:
                try:
                    _log.debug(f"{self._op_type.value}ing thread about to block on op")
                    operating = await self._operate(socket=socket, op_timeout=None)
                except asyncio.CancelledError:
                    _log.warning(
                        f"{self._op_type.value}ing thread is interrupted by handler forceful shutdown",
                        exc_info=1,
                    )
                    self._running = False
                    operating = False
                    ...
                    # Stop thread
                    _log.warning(
                        f"{self._op_type.value}ing thread is interrupted by user or system exit",
                        exc_info=1,
                    )
                    self._running = False
                    operating = False
                    ...
                    # Stop thread
                    _log.warning(
                        f"{self._op_type.value}ing thread is interrupted by unknown error",
                        exc_info=1,
                    )
                    self._running = False
                    operating = False
    ...
    def shutdown(self, timeout: float = None):
        ...
        _log.debug(
            f"{self._op_type.value}ing thread shutdown requested with{ f' {timeout}s' if timeout is not None else 'out' } timeout. State: running ({self._running}, loop running: {loop_running})"
        )
        ...
        async def _force_shutdown(loop):
            ...
            if len(pending_tasks) == 0:
                ...
                if not block:
                    _log.warning(
                        f"{self._op_type.value}ing thread took longer than {timeout}s to shutdown"
                    )
        ...
        if is_loop_running(self._loop):
            ...
            _log.warning(f"{self._op_type.value}ing thread is now terminated: {future}")

    def _log_thread_status(self, running, operating, dirty_shutdown, shutdown_duration):
        msg = f"{self._op_type.value}ing thread is "
        msg += "running." if running else "shutting down."
        msg += f"Operating on log records: {operating}."
        msg2 = (
            f"(Shutdown duration: {shutdown_duration}s, dirty shutdown: {dirty_shutdown})"
            if shutdown_duration is not None
            else ""
        )
        _log.debug(f"{msg}{msg2}")
    ...
    def _log_thread_stopped(self, duration):
        _log.debug(
            f"{self._op_type.value}ing thread stopped {str(duration) + 's after requested shutdown' if duration is not None else '' }"
        )
