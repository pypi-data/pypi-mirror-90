...
_log.debug(f"pyzmq v{zmq.__version__} & libzmq v{zmq.zmq_version()}")
...
class ZmqResilientSocket(BaseSocket):
    ...
    def _poller_register(self, socket: zmq.Socket = None, flags: int = 0):
        _log.debug(f"Register socket {socket} with poller {self._poller} with flags:{flags}")
        self._poller.register(socket=socket, flags=flags)

    def _poller_unregister(self, socket: zmq.Socket = None):
        _log.debug(f"Unregister socket {socket} from poller {self._poller}")
    ...
    async def send_json(self, data, timeout=None):
    ...
    def _destroy_context(self):
        ...
        if self._context is not None and not self._context.closed:
            _log.debug(f"Destroy context: {self._context}")
            self._context.destroy()
        return None

    def _create_socket(self, context=None, socket_type=None):
        _log.debug(f"Create socket on context {context} of socket type {socket_type}")
    ...
    def _close_socket(self):
        ...
        if self._socket is not None and not self._socket.closed:
            self._poller_unregister(socket=self._socket)
            _log.debug(f"Closing socket: {self._socket}")
            self._socket.close()
        return None
