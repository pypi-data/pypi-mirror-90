...
class ZMQSocket(BaseSocket):
    ...
    async def read_payload(self, op_timeout=None):
        try:
            return await self._zmq_socket.recv_json(timeout=op_timeout)
        except zmq.ZMQError as error:
            if error.errno == errno.EAGAIN:
                raise ReadTimeoutException() from error
            raise error

    async def write_payload(self, payload, op_timeout=None):
        try:
            future = self._zmq_socket.send_json(payload, timeout=op_timeout)
            return await future
        except zmq.ZMQError as error:
            if error.errno == errno.EAGAIN:
