...
def setup_server_handlers(
    ...
    for i, orig_handler in enumerate(list(logger.handlers)):
        handler_name = (
            str(orig_handler.name) if orig_handler.name is not None else f"narration-_handler-{i}"
        )
        kwargs = {
            ...
            "message_dispatching": DispatchMode.SYNC,
        }
...
def _shutdown_handler_resources(handler: BaseHandler = None) -> threading.Event:
    ...
    try:
        ...
        _log.debug(
            f"{name} {'fully' if shutdown_completed is not None and shutdown_completed.is_set() else 'not yet'} shutdown"
        )
...
def _closed_forked_handler(logger: logging.Logger = None, handler_id: str = None):
    ...
    forked_handler = next(filter(lambda h: h.uuid == handler_id, logger.handlers), None)
    if forked_handler is None:
        return

    # This handler was created in parent process
    ppid = os.getppid()
    if forked_handler.pid == ppid:
        logger.removeHandler(forked_handler.name)
        _log.debug(f"Fork server handler closing: {forked_handler}")
