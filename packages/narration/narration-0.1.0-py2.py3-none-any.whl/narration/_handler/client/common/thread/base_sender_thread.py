...
from narration._handler.common.payload.serialization.record_payload import to_raw_record_payload
...
class BaseSenderThread(BaseOpThread):
        ...
        try:
            ...
            if not has_payload:
                ...
                if raw_payload is not None:
                    result = await self._write_payload_on_socket(
                        socket=socket,
    ...
    async def _write_payload_on_socket(
        self,
        ...
        _log.debug(
            f"{'Retry' if retrying else 'New'} record to send ({op_timeout + 's' if op_timeout is not None else 'no timeout'}) {raw_payload}"
        )
        return await socket.write_payload(raw_payload, op_timeout=op_timeout)
        ...
        try:
            while True:
                ...
                try:
                    ...
                    _log.debug(f"Try cancelling dispatch status {dispatch_status.payload}")
                    cancelled = future.cancel()
                    if not cancelled:
                        try:
                            future.result(timeout=None)
                        except BaseException:
                            _log.warning(f"{dispatch_status.payload} raised exception.", exc_info=1)
                            pass
                finally:
