...
from narration._handler.common.payload.serialization.record_payload import to_record_payload
...
class BaseReceiverThread(BaseOpThread):
    ...
    async def _operate(self, socket: BaseSocket = None, queue=None, op_timeout=None) -> bool:
        ...
        try:
            ...
            _log.debug(
                f"Record received with handler id {handler_id} {f'discarded' if record_emitter is None else f'to be dispatched to {record_emitter}'}: {record}"
            )
    ...
    async def _read_payload_from_socket(
        ...
        raw_payload = await socket.read_payload(op_timeout=op_timeout)
        payload = to_record_payload(raw_payload=raw_payload)
        _log.debug(f"New record received from {payload.handler_id}. Record: {payload.record}")
        return payload
