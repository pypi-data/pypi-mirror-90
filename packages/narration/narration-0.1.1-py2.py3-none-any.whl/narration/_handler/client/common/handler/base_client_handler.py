    ...
    def emit(self, record):
        try:
            ...
            if self._record_emitter is None:
                _log.error(
                    "Not record emitter set for sending message to shared processor. Handler id: %s. Record: %s",
                    self._uuid,
                    record,
                )
            ...
            for receiver, dispatch_status in self._record_emitter.send(payload):
                ...
                _log.debug("handler to send message %s with timeout=%s", payload.record, timeout)
                dispatch_status.future.result(timeout=timeout)
        except concurrent.futures.TimeoutError:
            # Record may still be sent later. no error
            _log.warning(
                "Record sending timeout. Record may not have been dispatch yet. Record: %s", record
            )
