...
from bin.simulate_datasets import make_all_datasets
from bin.mom_clust import cluster_omes, write_ome_results, plot_ome_results
from bin.simulate_datasets import process_dir
from bin.common_functions import write_table
from bin.mom_cat import get_contingency_table
...
def get_real_random_feat_stats(selected_features, all_features, iteration, method = "MOMClust"):
...
def get_contingency_table_stats(ground_truth, cluster_vect, out_file):
    ...
    cont_df.columns = col_ids
    index_dict={key:value for key,value in enumerate(row_ids)}
...
def get_mi_stats(all_feat_mi, 
    ...
    ## make the plot
    ax = sns.violinplot(x="method", y="purity", data = mi_df, 
                        scale = "width",
                        size = size)
    ax.set_title("t="+str(paired_t_test_t)+"\np="+str(paired_t_test_p))
    out_dir = process_dir(out_dir)
...
def do_feat_select_plot(in_stats, var, out_dir, feature_type):
    ...
    mom_row = in_stats["method"]=="MOMClust"
    random_results = in_stats[var][rand_row]
    momclust_results = in_stats[var][mom_row]
    momclust_results[momclust_results==np.nan]=0
    random_results[random_results==np.nan]=0
    paired_t_test_t, paired_t_test_p = ttest_rel(momclust_results, random_results)
    print(momclust_results)
    print(random_results)
    print(var,"paired t-test:",paired_t_test_t, paired_t_test_p)

    plt.clf()
    ax = sns.violinplot(x="method", y=var,# hue="method",
                        scale = 'width', 
...
def main():
    ...
    for i in range(len(all_datasets)):
        ...
        temp_clustered_ome = cluster_omes(all_datasets[i]["cat"],all_datasets[i]["num"])
