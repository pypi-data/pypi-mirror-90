...
class DashApp(DashComponentBase):
    ...
    def run(self, port=None, width=1000, height=800, **kwargs):
        """Run the dash app

        Args:
            port (int): port to run server on
            width (int): width of display for mode='inline' or 'jupyterlab'
            height (int): height of display for mode='inline' or 'jupyterlab'
            """
        ...
        if self.mode=='dash':
            ...
            if not self.is_colab:
                print(f"Starting DashApp on http://localhost:{port}\n"
                      "You can terminate the dashboard with "
                      f"DashApp.terminate({port})", flush=True)
            self.app.run_server(port=port, mode=self.mode, **kwargs)
        elif self.mode in ['inline', 'jupyterlab']:
            print(f"Starting DashApp inline (terminate it with "
                  f"ExplainerDashboard.terminate({port}))", flush=True)
            self.app.run_server(port=port, mode=self.mode, width=width, height=height, **kwargs)

    @staticmethod
    def terminate(port, token=None):
        """
        terminate any JupyterDash dashboard (so started with
        mode='inline',  mode='external' or mode='jupyterlab') from any
        DashApp by specifying the right port.

        Example:
            DashApp(dash_component, mode='inline').run(port=8050)

            DashApp.terminate(8050)
        ...
        """
        if token is None:
            token = jupyter_dash.JupyterDash._token
