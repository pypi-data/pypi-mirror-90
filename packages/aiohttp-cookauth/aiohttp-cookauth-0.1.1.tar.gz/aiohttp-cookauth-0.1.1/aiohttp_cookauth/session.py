...
async def delete_all_sessions(request, identity):
    if identity is not None:
        session_or_identity = identity
    else:
        session_or_identity = await get_session(request)
    storage = request.get(STORAGE_KEY)
    await storage.delete_all_sessions(session_or_identity)
    return session_or_identity
...
class AbstractStorage(metaclass=abc.ABCMeta):
    ...
    def _get_session_identity(self, session):
        return list(session._mapping.values())[0]
