...
async def forget_all(request, identity=None):
    """Forget previously remembered identity.
    ...
    if identity_policy is None:
        ...
        # *text* arguments.
        raise web.HTTPInternalServerError(reason=text, text=text)
    await identity_policy.forget_all(request, identity)
...
def setup_security(app, identity_policy, autz_policy):
