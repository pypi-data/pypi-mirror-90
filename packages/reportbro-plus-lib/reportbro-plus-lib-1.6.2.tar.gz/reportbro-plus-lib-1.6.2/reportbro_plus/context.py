...
class Context:
    ...
    def build_data(self, parameters, object_id, field, pattern=None):
        ret = {}

        for item in parameters:
            parameter_name = re.search(r'\{(.*?)\}', item)

            if parameter_name is not None:
                param_ref = self.get_parameter(parameter_name.group(1))
                ...
                if param_ref is not None:
                    ...
                    if not value_exists:
                        ...
                        formated_value = self.get_formatted_value(value, param_ref.parameter, object_id, pattern=pattern)
                        ret.update({item: formated_value})
            else:
                ret.update({item: ''})
        return ret
    ...
    def fill_parameters(self, expr, object_id, field, pattern=None):
        ...
        parameters = list(filter(lambda item: item.find('}') != -1 or item.find('()') != -1, expr.split(' ')))
        ret = self.build_data(parameters=parameters, object_id=object_id, field=field, pattern=pattern)

        for key in ret.keys():
            val = re.findall(r'\b[A-Z\s]+\b', key)

            if (len(val) > 0):
                for action in val:
                    ret[key] = actions[action](ret[key])

        for key in ret.keys():
            expr = expr.replace(key, ret[key])
    ...
    def get_formatted_value(self, value, parameter, object_id, pattern=None, is_array_item=False):
        ...
        if value_type == ParameterType.string:
            ...
            if used_pattern:
                try:
                    if used_pattern == FilterParams.capitalize_all.value:
                        value = value.title()
                        rv = value
                    elif used_pattern == FilterParams.capitalize.value:
                        value = value.capitalize()
                        rv = value
                    elif used_pattern == FilterParams.lower_case.value:
                        value = value.lower()
                        rv = value
                    elif used_pattern == FilterParams.upper_case.value:
                        value = value.upper()
                        rv = value
                    elif used_pattern == FilterParams.time_current.value:
                        value = datetime.date.today()
                        rv = value
                    elif used_pattern == FilterParams.fixed.value:
                        fixedNumber = parameter.number_fixed
                        value = round(value, fixedNumber)
                        rv = value
                except ValueError:
