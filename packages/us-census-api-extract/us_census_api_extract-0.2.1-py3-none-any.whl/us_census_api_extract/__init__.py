...
def extract_by_var(vars, state_codes,
                   request_url_pre, request_url_post_1, request_url_post_2, api_key,
                   tmp_folder, tmp_filename_prefix='', reset=False, s3=False) -> DataFrame:
    """
    ...
    Args: 
        ...
        s3(Boolean): Default False.  Use S3 to store tmp_extract file(s)?
    ...
    if not s3 and reset and os.path.exists(progress_fp):
        os.remove(progress_fp)
    # TODO: delete tmp extract file on s3
    ...
    # If there is already progress save in the tmp_extract metadata then set the Progress object with the correct values
    if s3 and wr.s3.does_object_exist(export_tmp_fp):
        progress.set_int('batches_in_state_processed', int(
            wr.s3.describe_objects([export_tmp_fp])[0]['Metadata']['states_processed']))
    if s3 and wr.s3.does_object_exist(export_tmp_state_fp):
        progress.set_int('states_processed', int(wr.s3.describe_objects([export_tmp_fp])[0]['Metadata']['batches_in_state_processed']))
    ...
    if not s3 and not os.path.exists(export_tmp_fp):
        results_df.to_parquet(export_tmp_fp, compression='gzip')
    if s3 and not wr.s3.does_object_exist(export_tmp_fp):
        try:
            wr.s3.to_parquet(
                df=results_df,
                dataset=True,
                path=export_tmp_fp,
                mode='overwrite',
                s3_additional_kwargs={'Metadata': {
                    'batches_in_state_processed': 0}},
            )
        except Exception as x:
            print(f"Error sending data to S3: {x}")
    ...
    for states_processed in range(states_processed, states_to_process):
        ...
        for result_df, i in api_state_extract(batches_in_state_processed, batches_left_in_state_to_process,
            ...
            elif s3 and not wr.s3.does_object_exist(export_tmp_fp):
                try:
                    wr.s3.to_parquet(
                        df=results_df,
                        dataset=True,
                ...
                if s3:
                    try:
                        wr.s3.to_parquet(
                            ...
                            mode='overwrite',
                            s3_additional_kwargs={'Metadata': {
                                'batches_in_state_processed': progress.get_int('batches_in_state_processed') + 1}},
                        )
                    except Exception as x:
                        print(f"Error sending data to S3: {x}")
                else:
        ...
        # save result locally in a temp file, if temp file exists then join result to existing results
        if not s3 and not os.path.exists(export_tmp_state_fp):
            tmp_df.to_parquet(export_tmp_state_fp, compression='gzip')
        elif s3 and not wr.s3.does_object_exist(export_tmp_state_fp):
            try:
                wr.s3.to_parquet(
                    df=tmp_df,
                    dataset=True,
                    path=export_tmp_state_fp,
                    mode='overwrite',
                    s3_additional_kwargs={'Metadata': {
                        'states_processed': progress.get_int('states_processed')}},
                )
            ...
            if s3:
                try:
                    wr.s3.to_parquet(
                        ...
                        mode='overwrite',
                        s3_additional_kwargs={'Metadata': {
                            'states_processed': progress.get_int('states_processed') + 1}},
                    )
                except Exception as x:
                    print(f"Error sending data to S3: {x}")
            else:
        ...
        if tmp_df.empty:
            ...
            if s3:
                tmp_df = wr.s3.read_parquet(path=export_tmp_fp)
            else:
    ...
    if results_df.empty:
        ...
        if s3:
            results_df = wr.s3.read_parquet(path=export_tmp_state_fp)
        else:
