...
class TFWriter(object):
    ...
    def __init__(
        ...
        if tag == "train":
            ...
            n_files = 1
            self.all_files = ["{}_{}_{}_{}".format(file_name, i, tag, n_files) for i in range(n_files)]
            if overwrite is False:
                is_check(self.all_files)
            self.all_writer = [tf.io.TFRecordWriter(file_) for file_ in self.all_files]
...
class TFReader(object):
    ...
    def auto_batch_for_training(self,
                            tf_dataset, batch_size, x_keys=None, y_keys=None, shuffle=False, drop_remainder=False, shuffle_buffer_size=10000, prefetch_buffer_size=100):
        element_spec = tf_dataset.element_spec
        padded_values = {}
        for k,v in element_spec.items():
            padded_values[k] = tf.constant(0, dtype=v.dtype)
        dataset = tf_dataset.padded_batch(padding_values=padded_values,
    ...
    def read_record(self, keys=[], auto_batch = False, **kwargs):
        ...
        if auto_batch:
            dataset = self.auto_batch_for_training(dataset, **kwargs)
        return dataset
