...
class TextDecoderSerializableSeq2Seq(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        self.embedding_size, self.decoder_num_attention_heads,self.decoder_num_hidden_layers, self.decoder_attention_state = self.auto_infer_config(decoder_config)
    ...
    def greedy(self):
        ...
        def call_greedy(inputs):
            ...
            if "decoder_input_type_ids" in self.decoder_input_name_list:
                model_inputs["decoder_input_type_ids"] = tf.ones_like(model_inputs['decoder_input_ids']) * self.decoder_input_type_ids
    ...
    def beam(self):
        ...
        def body(
            ...
            # Add length penalty
            length_penalty = tf.pow(((5. + (tf.cast(i, tf.float32) + 1.0)) / 6.), self.alpha)
            ...
            if self.do_sample:
                ...
                topk_log_probs, topk_indices = tf.nn.top_k(flat_log_probs, k=beams_to_keep) # (batch_size x k (beams_to_keep))
    ...
    def top_k_top(self):
        ...
        def call_top_k_top_p(inputs):
            ...
            # Skip -1 initial ids
            results_dict["predicted_ids"] = results[-1][:, 1:]
            results_dict["predicted_ids"] = tf.cast(tf.reshape(
