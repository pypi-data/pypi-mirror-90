...
from tf_transformers.text import (_gather_beams, _log_prob_from_logits,
                           assign_zeros_to_K_V, top_k_logits, top_p_logits)
...
class TextDecoder(object):
    ...
    def __init__(
        ...
        if isinstance(model, tf.keras.Model):
            ...
            self.embedding_size, self.num_attention_heads,self.num_hidden_layers, self.attention_state = self.auto_infer_config(model_config, saved_model=False)
        ...
        #     self.model_fn = self.model
            ...
            self.embedding_size, self.num_attention_heads,self.num_hidden_layers, self.attention_state = self.auto_infer_config(model_config, saved_model=True)
    ...
    def validate_decoder_type_ids(self,inputs):
        if "input_type_ids" in inputs:
            if self.input_type_ids < 0:
                raise ValueError("Seems like you model has `decoder_input_type_ids`, but it hasn't set yet. Please provide a valid positive index for `decoder_input_type_ids`")
    ...
    def greedy_decode(self,
        ...
        if self.input_type_ids > -1:
            tokenized_input_dict["input_type_ids"] = tf.ones_like(tokenized_input_dict["input_ids"]) * self.input_type_ids
    ...
    def beam_decode(
        ...
        if self.input_type_ids > -1:
            tokenized_input_dict_ragged["input_type_ids"] = tf.ones_like(tokenized_input_dict_ragged["input_ids"]) * self.input_type_ids
        ...
        for i in range(max_iterations):
            ...
            # Add length penalty
            length_penalty = tf.pow(((5. + (tf.cast(i, tf.float32) + 1.0)) / 6.), alpha)
    ...
    def decode(
        ...
        if mode == "greedy":
            result = self.greedy_decode(tokenized_input_dict, max_iterations=max_iterations, temperature=sampling_temperature, do_sample=do_sample, eos_id=eos_id)
        if mode == "beam":
