...
class TransformerBERT(LegacyLayer):
    ...
    def call_cross_attention_in_encoder(self,
            inputs, mode, cache_key=None, cache_value=None):
        """
        inputs: dict keys:
                if self.is_training:
        ...
        if mode == 'decoder':
            decoder_attention_output, key, value,  = calculate_self_attention(input_tensor, attention_mask)
    ...
    def call(self, inputs, mode = 'encoder', cache_key=None, cache_value=None):
        ...
        if self._is_decoder:
            ...
            outputs = self.call_cross_attention_in_encoder(inputs, mode=mode, cache_key=cache_key, cache_value=cache_value)
        else:
