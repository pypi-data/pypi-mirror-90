...
from tf_transformers.layers import (
    OnDeviceEmbedding,
...
class T5Encoder(LegacyLayer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._last_layer_norm = T5LayerNormalization(
            name="last_layer_norm", axis=-1, epsilon=1e-6, dtype=tf.float32
        )
        self._last_layer_dropout = tf.keras.layers.Dropout(
    ...
    def call_training(self, inputs):
        ...
        if self.mask_mode == "prefix":
            attention_mask = tf.map_fn(
                prefix_mask, input_mask, fn_output_signature=tf.float32
            )
        ...
        for i in range(self.num_hidden_layers):
            ...
            embeddings, position_bias, k, v = layer(
                [embeddings, attention_mask], position_bias=position_bias
            )
    ...
    def call_decoder_predict(self, inputs):
        ...
        all_cache_key = [
            ...
            for item in tf.split(
                all_cache_key, num_or_size_splits=self.num_hidden_layers, axis=0
            )
        ]
        all_cache_value = [
            tf.squeeze(item, axis=0)
            for item in tf.split(
                all_cache_value, num_or_size_splits=self.num_hidden_layers, axis=0
            )
        ]
