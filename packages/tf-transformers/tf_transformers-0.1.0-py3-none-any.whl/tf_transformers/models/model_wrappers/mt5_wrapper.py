...
from tf_transformers.models import mT5Encoder, EncoderDecoder
...
def mT5Model(model_name, is_training=False, use_dropout = False, pipeline_mode=None, model_checkpoint_dir=None, encoder_sequence_length=None):
    """Wrapper for Model
    ...
    decoder_layer, decoder_model, decoder_config = encoder_class(
        model_name, **decoder_kwargs
    )
    del decoder_model
    tf.keras.backend.clear_session()
    model_layer = EncoderDecoder(
        encoder=encoder_layer, decoder=decoder_layer, is_training=is_training, name=model_name, encoder_sequence_length=encoder_sequence_length
    )
    config = {}
