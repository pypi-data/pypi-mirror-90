...
class ParamValueMixin:
    ...
    def get_spec(
        ...
        if not self.is_literal:
            ...
            if len(value_parts) == 1 and value_parts[0] not in refs.CONTEXTS:
                raise ValidationError(
                    "Received an invalid value `{}` for param `{}`. "
                    "Value must be one of `{}`".format(self.value, name, refs.CONTEXTS)
                )
            # Check the case of current DAG, it should not allow to use outputs
            if (
                len(value_parts) == 1
                and self.is_dag_ref
                and value_parts[0] == refs.OUTPUTS
            ):
                raise ValidationError(
                    "Received an invalid value `{}` for param `{}`. "
                    "You can not use `{}` of current dag".format(
                        self.value, name, refs.OUTPUTS
                    )
                )
            if (
                len(value_parts) == 2
                and value_parts[0] not in refs.CONTEXTS_WITH_NESTING
            ):
                raise ValidationError(
                    "Received an invalid value `{}` for param `{}`. "
                    "Value `{}` must be one of `{}`".format(
                        self.value, name, value_parts[0], refs.CONTEXTS_WITH_NESTING
                    )
                )
            if len(value_parts) == 3 and value_parts[0] != refs.ARTIFACTS:
                raise ValidationError(
                    "Received an invalid value `{}` for param `{}`. "
                    "Value `{}` must can only be equal to `{}`".format(
                        self.value, name, value_parts[0], refs.ARTIFACTS
                    )
                )
        if self.is_ref:
            if self.is_join_ref:
                if not is_list and iotype != types.ARTIFACTS:
                    raise ValidationError(
                ...
                refs.validate_ref(ref=self.ref, name=name)
...
class V1Param(BaseConfig, refs.RefMixin, ParamValueMixin, polyaxon_sdk.V1Param):
