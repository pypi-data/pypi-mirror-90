...
class V1EventKind(polyaxon_sdk.V1EventKind):
    events_statuses_mapping = {
        ...
        polyaxon_sdk.V1EventKind.RUN_STATUS_INITIALIZING: V1Statuses.INITIALIZING,
...
class V1EventTrigger(BaseConfig, refs.RefMixin, polyaxon_sdk.V1EventTrigger):
    ...
    Events can be attached to an operation in the context of a DAG to extend the simple trigger process,
    this is generally important when the user defines a dependency between two operations,
    and needs an operation to start as soon as
    ...
    Events can be attached as well to a single operations
    to wait for an internal alert or an external events,
    ...
    ```yaml
    >>> event:
    >>>   kinds: [V1EventTrigger.RUN_STATUS_RUNNING, V1EventTrigger.RUN_STATUS_DONE]
    ```
