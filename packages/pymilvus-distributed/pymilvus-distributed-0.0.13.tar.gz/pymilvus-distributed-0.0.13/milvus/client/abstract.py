...
from ..grpc_gen import service_msg_pb2
    ...
    def __pack(self, raw):
        ...
        for type_param in raw.type_params:
            if type_param.key == "params":
                import ast
                self.params[type_param.key] = ast.literal_eval(type_param.value)
            else:
        ...
        for index_param in raw.index_params:
            if index_param.key == "params":
                import ast
                index_dict[index_param.key] = ast.literal_eval(index_param.value)
            else:
...
class CollectionSchema:
    ...
    def dict(self):
        ...
        _dict["statistics"] = self.statistics
