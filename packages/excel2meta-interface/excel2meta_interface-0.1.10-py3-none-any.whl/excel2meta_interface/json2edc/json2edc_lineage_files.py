...
class JSON2EDCLineageFiles:
    ...
    def process_files(self, excel=None):
        module = __name__ + "." + currentframe().f_code.co_name
        if excel is not None:
            self.excel = excel
            self.json_directory += excel + "/"
            self.output_directory += excel + "/"
        ...
        try:
            with open(configuration_file) as config:
                ...
                if self.excel is not None:
                    self.json_directory += self.json_directory + self.excel + "/"
                    self.output_directory += self.output_directory + self.excel + "/"
