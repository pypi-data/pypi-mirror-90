    ...
    def create_mlflow_experiment(self, experiment_name):
        for retry in range(2):
            ...
            url = 'https://mlflow.' + builtins.service +\
                    '.com/Prod/2.0/mlflow/experiments/get-by-name?experiment_name=' +\
        ...
        for retry in range(2):
            ...
            url = 'https://mlflow.' + builtins.service + '.com/Prod/2.0/mlflow/experiments/create'
            try:
                response = requests.post(url, data=payload, headers=headers)
                resp_json = response.json()
                response.raise_for_status()
...
class DeletePeriodicRunHandler(APIHandler):  # pylint: disable=abstract-method
    ...
    def post(self, path=""):
        ...
        for retry in range(2):
            ...
            url = 'https://api.' + builtins.service + '.com/deleteperiodrun'
            try:
                response = requests.post(url, data=payload, headers=headers)
                resp_json = response.json()
                response.raise_for_status()
