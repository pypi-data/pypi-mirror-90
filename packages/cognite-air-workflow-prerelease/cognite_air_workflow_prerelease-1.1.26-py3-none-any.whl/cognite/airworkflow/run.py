...
from cognite.airworkflow.master_config import get_master_config, get_parallel_deployment
...
from cognite.airworkflow.util import cdf, env, functions, github


def handle_asset_hierarchy():
    _, project_to_config = get_master_config()
    assert len(sys.argv) < 3, "Expected not more than two arguments"
    ...
    for project, config in project_to_config.items():
        ...
        ModelAssetHierarchy(config, project, airInfra).create_or_update()
...
def handle_call_schedulemanager():
    ...
    for project in project_to_config:
        ...
        client.functions.call(external_id="cognitedata/air-functions/schedulemanager:latest", wait=False)
...
if __name__ == "__main__":
    assert len(sys.argv) > 0, "Expected at least one argument."
