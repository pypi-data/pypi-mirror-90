...
class NaverCheckScraper() :
    ...
    def get_data(self, data_list, start_index, end_index, save_list) :
        ...
        driver = webdriver.Chrome('../driver/chromedriver (2).exe')#, options=options)
        ...
        for idx, data in enumerate(data_list) :
            ...
            try :
                ...
                if re.search(pattern='no-product', string=current_url) is not None :
                    ...
                    extracted_data = self.data_from_outlet_window(data=data, driver=driver, current_index=idx,data_length=data_length)
                    save_list.append(extracted_data)

                elif re.search(pattern='smartstore', string=current_url) is not None :
                    extracted_data = self.data_from_smart_store(data=data, driver=driver, current_index=idx, data_length=data_length)
                    save_list.append(extracted_data)
