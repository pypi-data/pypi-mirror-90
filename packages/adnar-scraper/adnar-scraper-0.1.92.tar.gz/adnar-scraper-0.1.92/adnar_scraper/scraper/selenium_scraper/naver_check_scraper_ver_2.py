    ...
    def multi_get_data(self, data_list, num_of_process):
        with Manager() as manager :
            ...
            if num_of_process == 2 :
                ...
                process_2 = Process(target=self.get_data, args=(data_list, basis * 1, basis * len(data_list), save_list))
    ...
    def get_data(self, data_list, start_index, end_index, save_list) :
        ...
        for i in range(len(data_list)//30) :
            if (i+1) * 30 <= len(data_list) :
                ...
                #options.add_argument('--proxy-server={}'.format(proxies[randint(0, len(proxies))]))

                profile = webdriver.FirefoxProfile()
                profile.set_preference('webdriver.load.strategy', 'unstable')

                driver = webdriver.Firefox(profile) #executable_path="../driver/geckodriver.exe",
