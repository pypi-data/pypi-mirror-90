...
def build_and_schedule(ganttTasksCsvFilePath: str,
                       resourceFileCsvFilePath: str = None,
    ...
    gantt = Gantt.gantt_from_listdict(name=ganttTasksCsvFilePath, tasks=tasks, resources={r.name: r for r in resources})

    start_date = datetime.datetime.today().date()  # - datetime.timedelta(days=15)
    gantt.calculate_schedule(start_date,
...
if __name__ == "__main__":
    ...
    chart = build_and_schedule(ganttTasksCsvFilePath = 'C:/Users/tburns/Documents/GitHub/coopgantt/tests/testdata/gantt_items.csv',
                               ...
                               project_priority= project_priority,
                               schedule_method=ScheduleMethod.EARLIEST_DUE_DATE,
                               output_dirs = [
                                   './gantt_items_scheduled.csv'
                               ])

    build_svgs(chart, output_dirs=['./rendering/svgs'])

    html = create_html(chart, True)
