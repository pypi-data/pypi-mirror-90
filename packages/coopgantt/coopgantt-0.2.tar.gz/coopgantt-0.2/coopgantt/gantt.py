...
class Gantt:
    ...
    @classmethod
    def gantt_from_listdict(cls, name: str, tasks: List[Dict], resources: Dict[str, Resource]=None):
        chart = Gantt(name)
        ...
        tasks = cls._clean_input_data(tasks)
        ...
        for row in tasks:
            ...
            for r in row['resources']:
                if r.name not in resources:
                    resources[r.name] = r
                task_resources.append(resources[r.name])
    ...
    def _clean_input_data(data: List[Dict]) -> List[Dict]:
        ...
        for row in data:
            ...
            # Duration
            feature_duration = math.ceil(float(row['duration'])) if row['duration'] is not '' else 1
            us_duration = sum([math.ceil(float(sub_activity.get('duration', 0)) if sub_activity.get('duration', 0) is not '' else 0)
    ...
    def as_dataframe(self) -> pd.DataFrame:
        ...
        df['perc_completed'] = [task.percent_done for task in self.tasks]
        ...
        df['scheduled_start'] = [task._start_date for task in self.tasks]
        df['scheduled_end'] = [task.end_date for task in self.tasks]
        df['status'] = [task.calculated_status.name for task in self.tasks]
        ...
        df.sort_values(by=['scheduled_start'], ascending=True, inplace=True)
...
class ScheduledActivity(ABC):
    ...
    def schedule(self, start_date, max_resources_by_type_for_project: Dict[str, int]):
        ...
        logging.debug(f'activity {self} scheduled')
    ...
    def extrapolated_priority(self):
        return min(self.priority, min(x.priority for x in self.successors))
