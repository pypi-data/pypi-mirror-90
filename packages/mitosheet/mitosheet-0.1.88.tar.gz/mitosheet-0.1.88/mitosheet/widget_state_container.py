...
class WidgetStateContainer():
    ...
    def handle_edit_event(self, edit_event):
        ...
        for new_step in STEPS:
            if edit_event['type'] == new_step['event_type']:
                ...
                # If this event says to overwrite the previous step, then we delete the step event before executing this one. 
                # However, if this event execution fails, we undelete the previous step.
                overwrite = edit_event['overwrite'] if 'overwrite' in edit_event else False
                if overwrite:
    ...
    def handle_update_event(self, update_event):
        ...
        if update_event['type'] == 'df_names_update':
            ...
            self.curr_step['df_names'] = deepcopy(df_names)
    ...
    def _create_and_checkout_new_step(self, step_type):
        ...
        new_step['df_names'] = deepcopy(self.steps[new_step_id - 1]['df_names'])
