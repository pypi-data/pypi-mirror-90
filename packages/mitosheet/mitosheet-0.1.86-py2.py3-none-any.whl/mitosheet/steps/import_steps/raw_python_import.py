...
"""
from mitosheet.utils import get_header_renames
...
def execute_raw_python_import(
    ...
    for df_name in new_df_names:
        if df_name in wsc.curr_step['df_names']:
            raise make_df_exists_error(df_name)

    # Create a new step and save the inputs
    wsc._create_and_checkout_new_step(RAW_PYTHON_IMPORT_STEP_TYPE)
    wsc.curr_step['python_code'] = python_code
    wsc.curr_step['new_df_names'] = new_df_names
