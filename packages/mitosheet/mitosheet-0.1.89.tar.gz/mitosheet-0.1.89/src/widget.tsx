...
export class ExampleView extends DOMWidgetView {
  ...
  */
  send(msg: Record<string, unknown>) : void{
    ...
    // If this is a message that makes a change (anything but a log_event, api_call or tutorial_mode_edit),
    // then we make the system as loading, as we wait for a response
    if (msg['event'] !== 'log_event' && msg['event'] !== 'api_call' && msg['event'] !== 'tutorial_mode_event') {
  ...
  render() : void {  
    ...
    */
    if (!this.getHasRendered()) {
      ...
      // Log that this view was rendered, from a page refresh
      window.logger?.track({
      ...
      // renders therefor must be page refreshes
      this.send({
        'event': 'update_event',
