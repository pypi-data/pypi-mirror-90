...
class MPNN(ReaxFF):
  def __init__(self,libfile='ffield',direcs={},
               ...
               cons=['val','vale','rosi','ropi','ropp', # 'ovun3', # 'val8','val9','val10',
                     'bo1','bo2','bo3','bo4','bo5','bo6','Depi','Depp','hbtol'],
               optmol=True,opt=None,optword='nocoul',
               nanv={'boc1':-2.0},
      ...
      ReaxFF.__init__(self,libfile=libfile,direcs=direcs,
                      dft=dft,atoms=atoms,cons=cons,opt=opt,optword=optword,optmol=optmol,
  ...
  def supervise(self):
      ...
      for bd in self.bonds: 
          ...
          if self.nbd[bd]>0:
             ...
             diffa_ = tf.reduce_sum(input_tensor=tf.nn.relu(self.botol-self.bo0[bd])*fao_)
             l_atol = tf.add(diffa_,l_atol)
             
             if self.bopSV:
                fbo = tf.where(tf.less_equal(self.rbd[bd],self.rcuta[bd]),1.0,0.0) ##### r> rcut that bo < botol
                diffb_  = tf.reduce_sum(input_tensor=tf.nn.relu(self.atol - self.bop_si[bd])*fbo)
                l_atol  = tf.add(diffb_,l_atol)
                diffb_  = tf.reduce_sum(input_tensor=tf.nn.relu(self.atol - self.bop_pi[bd])*fbo)
                l_atol  = tf.add(diffb_,l_atol)
                diffb_  = tf.reduce_sum(input_tensor=tf.nn.relu(self.atol - self.bop_pp[bd])*fbo)
                l_atol  = tf.add(diffb_,l_atol)
  ...
  def get_bondorder_uc(self,bd):
      ...
      self.bop_si[bd] = fsi_*self.eterm1[bd] # consist with GULP
      self.bop_pi[bd] = fpi_*self.eterm2[bd]
      self.bop_pp[bd] = fpp_*self.eterm3[bd]
