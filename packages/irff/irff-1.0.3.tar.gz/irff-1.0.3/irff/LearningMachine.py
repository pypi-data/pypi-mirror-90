...
class LearningMachine(object):
  ...
  def __init__(self,aimd=None,initConfig='poscar.gen',
      ...
      self.c            = AtomDance(poscar=self.initConfig,nn=self.nn,
                                    rtole=self.rtole-0.05,bondTole=self.bondTole)
  ...
  def run(self):
      ...
      while running:
          ...
          if isfile('md.traj'): 
             ...
             e_gmd,mdsteps = self.mlmd(cwd,run_dir,atoms,10000,i,learnWay) 
          ...
          if bondBroken>=1 and self.CheckBond and (not bkbd is None):  # if bonds have broken
             mdsteps = 10
             images = self.c.stretch(bkbd,atoms=atoms,nbin=mdsteps,st=self.bondTole-0.015,ed=1.0,scale=1.2,traj='md.traj')
             if images is None:
                atoms = self.c.bond_momenta_bigest(atoms)
                e_gmd,mdsteps = self.mlmd(cwd,run_dir,atoms,10000,i,learnWay) 
             else:
          ...
          if bondBroken<1 or (not self.CheckBond): 
             e_gmd,mdsteps = self.mlmd(cwd,run_dir,atoms,10000,i,learnWay) 
         ...
         #           else:                                              #
         #              fe.write(' %d %f \n' %(step+i_,e_gmd[i_]))      #
          self.learninglog(i,loss,accu,mdsteps,e_gulp[-1],e_siesta[-1],dEmax_,d2Emax_,learnWay,
                           bondBroken,bkbd)
  ...
  def learninglog(self,i,loss,accu,mdsteps,eirff,edft,dEmax,d2Emax,learnWay,bondBroken,bkbd):
      with open('learning.log','a') as l:
         ...
         # l.write('Bond Broken :  %d \n' %bondBroken)
         if bondBroken>=1 and (not bkbd is None):
            l.write('Bond between atoms %d and %d was broken during MLMD.\n' %(bkbd[0],bkbd[1]))
         l.write(' \n')
