...
def amz_gen_init(verbose=False):
    ...
    try:
        with open(f'{AMZ_DATA_DIR}/githooks/config/.pre-commit-config.yaml', 'r') as file:
            filedata = file.read()
        filedata = filedata.replace('AMZ_DATA_DIR', AMZ_DATA_DIR)
        with open(f'{AMZ_DATA_DIR}/githooks/config/.pre-commit-config.yaml', 'w') as file:
            file.write(filedata)
    except:
        l.end(1)
