...
def read_linter_config(filename):
    ...
    with open(filename, 'r') as ymlfile:
        parsed_config = yaml.load(ymlfile, Loader=yaml.FullLoader)
    ...
    for changed_file in staged_files:
        ...
        if changed_file.lower().endswith(('.cc', '.h', '.cpp', '.cu', '.cuh')):
            ...
            if error_count > 0:
                ...
                print("-" * 80)
                print("Found {} errors in : {}".format(error_count, changed_file))
                print("-" * 80)
...
def run_clang_format(repo_root, staged_files, list_of_changed_staged_files):
    ...
    clang_format_path = ("/tmp/" + os.path.basename(os.path.normpath(repo_root)) + "_" +
    ...
    run_command_in_folder(
        "git diff -U0 --cached | " + clang_format_diff_executable + " -style=file -p1 > " +
...
def run_yapf_format(repo_root, staged_files, list_of_changed_staged_files, yapf_file=None):
    ...
    for staged_file in staged_files:
        ...
        if staged_file.endswith((".py")):
            ...
            yapf_format_path = ("/tmp/" + os.path.basename(os.path.normpath(repo_root)) + "_" +
                                datetime.datetime.now().isoformat() + ".yapf.patch")
            if yapf_file is not None:
                task = (YAPF_FORMAT_EXECUTABLE + " --style='" + yapf_file + "' -d " + staged_file +
                        " > " + yapf_format_path)
            else:
            ...
            if not os.stat(yapf_format_path).st_size == 0:
                if first_file_formatted:
                    print("=" * 80)
                    print("Formatted staged python files with pep8 style:")
                ...
                if staged_file in list_of_changed_staged_files:
                    ...
                    run_command_in_folder("git apply -p0 " + yapf_format_path, repo_root)
...
def check_python_lint(repo_root, staged_files, pylint_file):
    ...
    is_python2 = True if sys.version[:4] == '2.7.' else False
    ...
    for changed_file in staged_files:
        ...
        if re.search(r'\.py$', changed_file):
            ...
            if os.stat(changed_file).st_size == 0:
                continue

            # Apply the linter only to files using the same python version as of pylint
            with open(changed_file, 'r') as f:
                shebang_line = f.readline()
            if '#!/usr/bin/env' in shebang_line:
                if 'python3' in shebang_line and is_python2:
                    continue
                elif 'python2' in shebang_line and not is_python2:
                    continue
            else:
                raise RuntimeError('Missing shebang line in file: ' + changed_file)

            print("Running pylint on \'{}\'".format(repo_root + "/" + changed_file))
            pylint_output = TextReporterBuffer()
            pylint_args = ["--rcfile=" + pylint_file, "-rn", repo_root + "/" + changed_file]
            from pylint.reporters.text import TextReporter
            pylint.lint.Run(pylint_args, reporter=TextReporter(pylint_output), exit=False)
...
def get_whitelisted_files(repo_root, files, whitelist):
    ...
    for file_name in files:
        for entry in whitelist:
            ...
            # whitelist.
            if (file_name == entry or os.path.commonprefix([file_name, entry]) == entry):
...
def linter_check(repo_root, linter_subfolder, linter_config_file=None):
    """ Main pre-commit function for calling code checking script. """

    if linter_config_file is not None and os.path.isfile(linter_config_file):
    ...
    yapf_file = os.path.join(linter_subfolder, "yapf_config.rc")
    ...
    # print("Using pylint config file: {}".format(pylintrc_file))
    ...
    if not check_if_merge_commit(repo_root):
        ...
        # complaints.
        list_of_changed_staged_files = check_modified_after_staging(whitelisted_files)

        if linter_config['use_clangformat']:
            run_clang_format(repo_root, whitelisted_files, list_of_changed_staged_files)

        if linter_config['use_yapf']:
            run_yapf_format(repo_root, whitelisted_files, list_of_changed_staged_files, yapf_file)

        # Use Google's C++ linter to check for compliance with Google style
        # guide.
        if linter_config['use_cpplint']:
            cpp_lint_success = check_cpp_lint(whitelisted_files, cpplint_file, ascii_art, repo_root)
        else:
        ...
        if linter_config['use_pylint']:
            pylint_success = check_python_lint(repo_root, whitelisted_files, pylintrc_file)
        else:
            pylint_success = True

        if not (cpp_lint_success and pylint_success):
            print("=" * 80)
            print("Commit is not up to standards!")
            ...
            if lucky_commit:
                ...
                # print("=" * 80)
                print("Commit accepted, well done! This is your {}th commit!".format(commit_number))
                ...
                # print("=" * 80)
                print("Commit accepted, well done! This is the {}th commit!".format(commit_number))
                # print("=" * 80)
    else:
