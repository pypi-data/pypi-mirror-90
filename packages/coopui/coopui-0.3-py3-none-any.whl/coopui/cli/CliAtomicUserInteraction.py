...
class CliAtomicUserInteraction(IAtomicUserInteraction):
    ...
    def request_data_from_csv_with_specified_columns(self, column_type_definition: Dict, allow_partial:bool=False, title:str=None) -> pd.DataFrame:
        ...
        types = {k:v for k, v in column_type_definition.items() if v not in (datetime.date, datetime.datetime)}

        df = df.astype(types)

        # handle date conversions
        for col, type in {k: v for k, v in column_type_definition.items() if v in (datetime.date, datetime.datetime)}.items():
