...
class SavviHubClient:
    ...
    def experiment_create(self, workspace, project, image_url, resource_spec_id, git_ref, git_diff_file_path,
                          start_command, datasets, env_vars, **kwargs):
    ...
    def cluster_register(self, workspace, cluster_name,
                         master_endpoint, namespace, service_account_token, ssl_ca_cert_base64_encoded):
        try:
            r = self.post(f'/api/v1/workspaces/{workspace}/custom_clusters/{cluster_name}/register', {
    ...
    def cluster_list(self, workspace):
        try:
            r = self.get(f'/api/v1/workspaces/{workspace}/clusters', raise_error=True)
        except SavviHubError as e:
            typer.echo(e.message())
            sys.exit(1)
    ...
    def cluster_agent_token(self, workspace, cluster_name):
        try:
            r = self.get(f'/api/v1/workspaces/{workspace}/custom_clusters/{cluster_name}/token', raise_error=True)
        except SavviHubError as e:
            typer.echo(e.message())
            sys.exit(1)
