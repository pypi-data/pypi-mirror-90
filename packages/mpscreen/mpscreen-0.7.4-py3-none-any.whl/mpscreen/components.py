...
class line:
    ...
    def _add(self, value, item):
        self.q.put(bytes([self.id, int(self.translation[item]), 1]) + int(value).to_bytes(4, byteorder='little'))
...
class lineServer(Elem):
    ...
    def _receive(self, data):
        if data[2]:
            self.variables[data[1]] += int.from_bytes(data[3:], byteorder='little')
        else:
