...
class line:
    ...
    def __setattr__(self, key, value):
        if key in ["q", "id", "attributes", "lastAdd", "lastSet", "addBuffer"]:
    ...
    def _add(self, value, item):
        t = time.time()
        nv = value + (0 if item not in self.addBuffer else self.addBuffer[item])
        if t - self.lastAdd < 0.05:
            self.addBuffer[item] = nv
        else:
            self.q.put([self.id, item, True, nv])
            self.addBuffer[item] = 0
            self.lastAdd = t

    def _set(self, value, item):
        t = time.time()
        if t - self.lastSet > 0.05:
