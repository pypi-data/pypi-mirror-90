...
class line:
    ...
    def __setattr__(self, key, value):
        if key in ["q", "id", "attributes", "lastUpload","addBuffer"]:
    ...
    def _add(self, value, item):
        t = time.time()
        if t - self.lastUpload < 0.1:
            self.addBuffer[item] = value + 0 if item not in self.addBuffer else self.addBuffer[item]
        else:
            self.q.put([self.id, item, True, value + 0 if item not in self.addBuffer else self.addBuffer[item]])
            self.addBuffer[item] = 0
            self.lastUpload = t

    def _set(self, value, item):
        t = time.time()
        if t - self.lastUpload > 0.1:
            self.q.put([self.id, item, False, value])
            self.lastUpload = t
