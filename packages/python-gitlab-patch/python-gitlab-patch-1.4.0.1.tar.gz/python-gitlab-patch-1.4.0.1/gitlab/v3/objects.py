...
class ProjectMergeRequest(GitlabObject):
    ...
    def get(cls, gl, id, **kwargs):
        ...
        :return:
        """
        _url = '/projects/%(project_id)s/merge_request/%(mr_id)s' % \
               {'project_id': kwargs["project_id"], 'mr_id': id}
        r = gl._raw_get(_url, **kwargs)
        raise_error_from_response(r, GitlabGetError)
        data = utils.decode_response(r)
        data['_from_api'] = True
        return cls(gl, data)
    ...
    def _update(self, **kwargs):
        ...
        _url = '/projects/%(project_id)s/merge_request/%(mr_id)s' % \
               {'project_id': params["project_id"], 'mr_id': params["id"]}

        # build data that can really be sent to server
        data = self._data_for_gitlab(extra_parameters=kwargs, update=True)
        r = self.gitlab._raw_put(_url, data=data, content_type='application/json')
        raise_error_from_response(r, GitlabUpdateError)
        return utils.decode_response(r)
    ...
    def subscribe(self, **kwargs):
        ...
        Raises:
            ...
            GitlabSubscribeError: If the subscription cannot be done
        """
        url = ('/projects/%(project_id)s/merge_request/%(mr_id)s/'
               'subscription' %
    ...
    def cancel_merge_when_build_succeeds(self, **kwargs):
        ...
        u = ('/projects/%s/merge_request/%s/cancel_merge_when_build_succeeds'
             % (self.project_id, self.id))
    ...
    def closes_issues(self, **kwargs):
        ...
        Raises:
            ...
            GitlabGetError: If the server fails to perform the request.
        """
        url = ('/projects/%s/merge_request/%s/closes_issues' %
               (self.project_id, self.id))
    ...
    def commits(self, **kwargs):
        ...
        Raises:
            ...
            GitlabListError: If the server fails to perform the request.
        """
        url = ('/projects/%s/merge_request/%s/commits' %
               (self.project_id, self.id))
    ...
    def changes(self, **kwargs):
        ...
        Raises:
            ...
            GitlabListError: If the server fails to perform the request.
        """
        url = ('/projects/%s/merge_request/%s/changes' %
               (self.project_id, self.id))
        r = self.gitlab._raw_get(url, **kwargs)
    ...
    def merge(self, merge_commit_message=None,
        ...
        """
        url = '/projects/%s/merge_request/%s/merge' % (self.project_id,
                                                        self.id)
        data = {}
    ...
    def todo(self, **kwargs):
        ...
        Raises:
            GitlabConnectionError: If the server cannot be reached.
        """
        url = ('/projects/%(project_id)s/merge_request/%(mr_id)s/todo' %
               {'project_id': self.project_id, 'mr_id': self.id})
        r = self.gitlab._raw_post(url, **kwargs)
        raise_error_from_response(r, GitlabTodoError, [201, 304])
    ...
    def time_stats(self, **kwargs):
        ...
        Raises:
            GitlabConnectionError: If the server cannot be reached.
        """
        url = ('/projects/%(project_id)s/merge_request/%(mr_id)s/time_stats' %
               {'project_id': self.project_id, 'mr_id': self.id})
        r = self.gitlab._raw_get(url, **kwargs)
        raise_error_from_response(r, GitlabGetError)
        return r.json()
