...
def download(cache_dir, language):  # put a try-catch here
    ...
    if not os.path.exists(os.path.join(lang_dir, '{}.downloaded'.format(language))):
        ...
        url = "http://nlp.uoregon.edu/download/merino/{}.zip".format(language)
        print(url)
...
def process_doc(tokenized_doc, conllu_doc):
    num_sents = len(conllu_doc)
    for sent_id in range(num_sents):
        sent = conllu_doc[sent_id]
        out_sent = []
...
def tprocess_sentence(sentence):
    sent = []
    i = 0
...
def normalize_token(treebank_name, token, ud_eval=True):  # adapted from stanza
