...
def bio2_to_bioes(tags):  # Borrowed from stanza.
    """
...
def process_cache(cached_lines):  # borrowed from stanza
    tokens = []
    ner_tags = []
...
def read_ner_data(config, data_fpath, evaluate):
    stanza_examples = load_conllu(data_fpath)
    output_examples = [{'words': x[0], 'entity-labels': bio2_to_bioes(x[1])} for x in stanza_examples]
...
def decode_from_bioes(tags):  # Borrowed from stanza.
    """
