...
from .utils.tbinfo import tbname2training_id, lang2treebank, trainable_langs
...
class TPipeline:
    ...
    def _set_up_config(self, training_config):
        ...
        # lang and data
        self._lang = training_config['lang']
        assert self._lang in trainable_langs, '{} is not supported for training. Currently, we only support the training option for the following languages: {}'.format(
            self._lang, trainable_langs)
        self._task = training_config['task']
        assert self._task in ['tokenize', 'mwt', 'posdep', 'lemma', 'ner']
        self._text_split_by_space = training_config['text_split_by_space']  # deal with languages like Chinese, Japanese
        ...
        self._train_bio2_fpath = training_config['train_bio2_fpath'] if 'train_bio2_fpath' in training_config else None
        self._dev_bio2_fpath = training_config['dev_bio2_fpath'] if 'dev_bio2_fpath' in training_config else None
        ...
        if self._task == 'tokenize':
            ...
            assert self._train_bio2_fpath and self._dev_bio2_fpath, 'Missing one of these files: train/dev BIO2 file containing annotated NER labels'

        # device and save dir
        self._save_dir = training_config['save_dir']
        self._cache_dir = self._save_dir
        self._gpu = training_config['gpu']
        self._use_gpu = training_config['gpu']
        self._ud_eval = True
        ...
        self._config.max_input_length = training_config['max_input_length']  # this is for tokenizer only
        self._config.batch_size = training_config['batch_size']
        self._config.max_epoch = training_config['max_epoch']
    ...
    def _prepare_ner(self):
        self.train_set = NERDataset(
            config=self._config,
            bio2_fpath=self._train_bio2_fpath,
            evaluate=False
        )
        self.train_set.numberize()
        self.batch_num = len(self.train_set) // self._config.batch_size
        ...
        self.dev_set = NERDataset(
            config=self._config,
            bio2_fpath=self._dev_bio2_fpath,
            evaluate=False
        )
        self.dev_set.numberize()
        self.dev_batch_num = len(self.dev_set) // self._config.batch_size + \
        ...
        for j in range(len(paragraphs)):
            ...
            for t, wp_p in zip(raw, wp_pred):
                ...
                if wp_p >= 1:
                    ...
                    current_tok = ''
                    if (wp_p == 2 or wp_p == 4):
                        doc.append(tprocess_sentence(current_sent))
                        current_sent = []
    ...
    def _eval_posdep(self, data_set, batch_num, name, epoch):
        ...
        doc = tprocess_doc(conllu_doc=data_set.conllu_doc)
        CoNLL.dict2conll(doc, pred_conllu_fpath)
