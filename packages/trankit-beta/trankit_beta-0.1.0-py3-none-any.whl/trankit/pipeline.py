...
class Pipeline:
    def __init__(self, lang, cache_dir=None, gpu=True):
        ...
        print('Loading pretrained XLMR, this may take a while...')
    ...
    def tokenize(self, input, is_sent=False):
        ...
        if type(input) == str and input.isspace():
            return []
        if is_sent:
            return self._tokenize_sent(in_sent=input)
        else:
            return self._tokenize_doc(in_doc=input)
        ...
        for j in range(len(paragraphs)):
            ...
            for t, wp_p in zip(para_text, wp_pred):
                ...
                if wp_p >= 1:
                    ...
                    current_tok = ''
                    if (wp_p == 2 or wp_p == 4):
                        tokens += process_sentence(current_sent)
                        current_sent = []
        ...
        for j in range(len(paragraphs)):
            ...
            for t, wp_p in zip(para_text, wp_pred):
                ...
                if wp_p >= 1:
                    ...
                    current_tok = ''
                    if (wp_p == 2 or wp_p == 4):
                        processed_sent = process_sentence(current_sent)
                        doc.append({
                            ID: len(doc) + 1,
    ...
    def _posdep_sent(self, in_sent):  # assuming input is a sentence
        ...
        for batch in DataLoader(test_set, batch_size=tbname2tagbatchsize.get(self._config.treebank_name, self._tagbatchsize),
                                shuffle=False, collate_fn=test_set.collate_fn):
        ...
        tagged_doc = process_doc(posdep_sent, test_set.conllu_doc)
        torch.cuda.empty_cache()
        ...
        for batch in DataLoader(test_set, batch_size=tbname2tagbatchsize.get(self._config.treebank_name, self._tagbatchsize),
            ...
            for bid in range(batch_size):
                for i in range(batch.word_num[bid]):
                    ...
                    # deprel
                    test_set.conllu_doc[sentid][wordid][DEPREL] = pred_tokens[bid][i][1]
        tagged_doc = process_doc(dposdep_doc, test_set.conllu_doc)
        torch.cuda.empty_cache()
        return tagged_doc
    ...
    def lemmatize(self, input, is_sent=False):
        ...
        if is_sent:
            if is_list_strings(input):
                input = [{ID: k + 1, TEXT: w} for k, w in enumerate(input)]
            return self._lemmatize_sent(in_sent=input, obmit_tag=True)
        else:
            if is_list_list_strings(input):
                input = [{ID: sid + 1, TOKENS: [{ID: tid + 1, TEXT: w} for tid, w in enumerate(sent)]} for sid, sent in
                         enumerate(input)]
            return self._lemmatize_doc(in_doc=input, obmit_tag=True)
    ...
    def ner(self, input, is_sent=False):
        ...
        if is_sent:
            if is_list_strings(input):
                input = [{ID: k + 1, TEXT: w} for k, w in enumerate(input)]
            return self._ner_sent(in_sent=input)
        else:
            if is_list_list_strings(input):
                input = [{ID: sid + 1, TOKENS: [{ID: tid + 1, TEXT: w} for tid, w in enumerate(sent)]} for sid, sent in
                         enumerate(input)]
            return self._ner_doc(in_doc=input)
    ...
    def __call__(self, input, is_sent=False):
        ...
        if is_sent:
            ...
            tokenized_doc = self._tokenize_doc(input)
    ...
    def _conllu_predict(self, text_fpath):
        print('{}: Running the pipeline on device={}'.format(datetime.now(), self._config.device))
        with open(text_fpath) as f:
            raw_text = f.read()
        print('{}: Beginning tokenization'.format(datetime.now()))
        tokenized_doc = self._tokenize_doc(raw_text)
        print('{}: Beginning pos tagging and dependency parsing'.format(datetime.now()))
        tagged_doc = self._posdep_doc(tokenized_doc)
        print('{}: Beginning lemmatization'.format(datetime.now()))
