...
def test_to_html_tree(test, root_timestamp, indent=0):
    ...
    lines = []
    lines.append(f'{S}<tr data-depth="{indent}" class="">')
    lines.append(f'{S}  <td class="collapser status {status}">')
    if test["sub_tests"]:
        lines.append(f'{S}<i class="ri-flask-fill"></i>')
    else:
        lines.append(f'{S}<i class="ri-test-tube-fill"></i>')
    lines.append(f'{S}    <span class="tooltip">{test_name}')
    lines.append(f'{S}      <span class="tooltiptext">')
    lines.append(f'{S}        <span class="status {status}">{status}</span>')
    lines.append(f"{S}        <pre>{summary}</pre>")
    if error:
        lines.append(f"{S}        <pre>{error}</pre>")
        lines.append(f"{S}        <pre>{trace}</pre>")

    lines.append(f"{S}        </span>")
    lines.append(f"{S}      </span>")
    lines.append(f"{S}    </td>")
    lines.append(f'{S}    <td class="toggleable info-description">')
    lines.append(f'{S}      <span class="tooltip">{short or ""}')
    if long:
        lines.append(f'{S}      <span class="tooltiptext">')
        lines.append(f"{S}        <pre>{long}</pre>")
        lines.append(f"{S}    </span>")
    lines.append(f"{S}    </td>")
    lines.append(
        f'{S}    <td class="toggleable info-time">start @+{start_time:.02f}, {run_time:.02f}s long.</td>'
    )
    lines.append(f"{S}</tr>")
...
def html_tree(session_json_dumps):
