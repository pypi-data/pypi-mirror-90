...
def test_build_valid_environment(mocker, tuxbuild_config):
    Build = mocker.patch("tuxbuild.Build")
    build = Build.return_value
    wait_for_object = mocker.patch("tuxbuild.cli.wait_for_object")
    runner = CliRunner()
...
def test_invalid_environment_key_value(mocker, tuxbuild_config):
    Build = mocker.patch("tuxbuild.Build")
    build = Build.return_value
    runner = CliRunner()
    result = runner.invoke(
...
def test_build_set_make_targets_vars_argument():
    """ Test calling build with --help to see targets argument"""
    runner = CliRunner()
    result = runner.invoke(tuxbuild.cli.build_set, ["--help"])
    assert result.exit_code == 0
    assert "[VAR=VALUE...] [target ...]" in result.output
    assert "[KEY=VALUE | target] ..." in result.output


def test_build_valid_make_targets(mocker, tuxbuild_config):
    Build = mocker.patch("tuxbuild.Build")
    build = Build.return_value
    wait_for_object = mocker.patch("tuxbuild.cli.wait_for_object")
    runner = CliRunner()
...
def test_build_valid_make_vars(mocker, tuxbuild_config):
    Build = mocker.patch("tuxbuild.Build")
    build = Build.return_value
    wait_for_object = mocker.patch("tuxbuild.cli.wait_for_object")
    runner = CliRunner()
...
def test_build_invalid_make_vars(mocker, tuxbuild_config):
    Build = mocker.patch("tuxbuild.Build")
    build = Build.return_value
    wait_for_object = mocker.patch("tuxbuild.cli.wait_for_object")
    runner = CliRunner()
