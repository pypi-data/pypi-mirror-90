...
def get_tuxbuild_remote():
    cmd = ["git", "config", "--get", "--default", "origin", "tuxbuild.remote"]
...
def push_head(remote, sha1):
    check_call(["git", "push", remote, f"HEAD:refs/tuxbuild/{sha1}"])
...
def get_git_head():
    ...
    Operates on the remote named in `git config tuxbuild.remote`, or "origin"
    if that is unset. The current HEAD is pushed to remote at
    refs/tuxbuild/${SHA1}, to make it available in the public repository.
