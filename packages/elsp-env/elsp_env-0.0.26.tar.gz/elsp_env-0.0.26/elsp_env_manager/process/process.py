...
class Process(ABC):
    ...
    def __init__(self, process_unit):
        ...
        self.obs_inf_k = "process_" + str(self.process_unit.m_id)
...
class Demand(Process):
    ...
    def __init__(self, demand_json: dict, process_unit):
        super().__init__(process_unit)
    ...
    def generate(self):
        for prod, dist in self.demand_dist.items():
            dist: Distribution
            self.demand_dict[prod]["all"] += dist.sample(self.process_unit.get_time())
            self.demand_dict[prod]["not_fit"] += dist.sample(self.process_unit.get_time())
...
class Scheduling(Process):
    ...
    def __init__(self, scheduling_period: Distribution, process_unit):
        super().__init__(process_unit)
        self.scheduling_period = scheduling_period
        self.obs_inf_k = 'scheduling_' + str(self.process_unit.m_id)
        self.prod_feature_dim = 3
        self.env_feature_dim = 2
    ...
    def on_scheduling_finished(self):
        ...
        store: Store = self.process_unit.storage_unit.store
        demand: Demand = self.process_unit.customer_unit.demand
        for p in self.scheduling_valid_prod:
            if p not in demand.demand_dict:
                self.obs.append(0)
                self.obs.append(0)
                self.obs.append(0)
            else:
                d = demand.demand_dict[p]
                self.obs.append(d['fit'])
                self.obs.append(d['not_fit'])
                self.obs.append(store.prod_total[p] if p in store.prod_total else 0)
        self.obs.append(self.action)
        self.obs.append(self.delta_time)
        pass
    ...
    def scheduling(self, action):
        if isinstance(action, int):
            ...
            if self.process_unit.needs_scheduling() or done:
                ...
                return self.obs, self.delta_profit / self.delta_time, done
