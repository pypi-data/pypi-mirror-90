...
class ProductionUnit(ProcessUnit):
    ...
    def reset(self):
        if self.setup is not None:
            self.setup.reset()
            self.setup.last_prod = None
        if self.launch is not None:
            self.launch.reset()
            self.launch.last_prod = None
        if self.conversion is not None:
            self.conversion.reset()
            self.conversion.last_prod = None
        if self.failure is not None:
            self.failure.reset()
            self.failure.last_prod = None
        self.idle = True
...
class CustomerUnit(ProcessUnit):
    ...
    def __init__(self, m_id: MID, model_json, env):
        super().__init__(m_id, model_json, env)
        self.demand: Demand = Demand(model_json["demand"], self)
    ...
    def load(json_or_json_path, env):
        ...
        for u_id, u in json.items():
            ...
            CustomerUnit.customer_units[m_id] = CustomerUnit(m_id, u, env)
