...
use crate::pyutils::{close_pyobject, PyGILState_STATE};
use crate::request::WSGIRequest;
use crate::startresponse::{StartResponse, WriteResponse};
use crate::transport::{Connection, HTTPPersistentConnection};
...
pub struct WSGIResponse<C: Connection> {
    ...
    pub written: usize,
    pub connection: HTTPPersistentConnection<C>,
}

impl<C: Connection> WSGIResponse<C> {
    pub fn new(connection: HTTPPersistentConnection<C>, chunked_transfer: bool) -> WSGIResponse<C> {
    ...
    pub fn write_chunk(&mut self, py: Python, gstate: PyGILState_STATE) -> io::Result<bool> {
            ...
            if let Some(ob) = self.pyobject.as_mut() {
                self.last_chunk_or_file_sent = match ob.extract::<FileWrapper>(py) {
                    Ok(mut fw) => {
                        ...
                        fw.send_file(&mut self.connection, gstate, py)
                    }
                    Err(_) => {
...
mod tests {
    ...
    use crate::startresponse::{StartResponse, WriteResponse};
    use crate::transport::{
        shared_connection_options, Connection, HTTPPersistentConnection, SetBlocking,
    };
    ...
    fn dummy_connection() -> HTTPPersistentConnection<TcpStream> {
    ...
    fn test_write_chunk() {
        ...
        with_gil(|py, gs| match r.write_chunk(py, gs) {
            Err(_) => {
                assert!(false);
            }
            Ok(true) => {
