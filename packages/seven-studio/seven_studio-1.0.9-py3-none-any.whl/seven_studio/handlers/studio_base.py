...
:LastEditTime: 2021-01-06 18:43:31
...
class StudioBaseHandler(BaseCookieHandler):
    ...
    def add_action_log(self, module_code, content, record_time=None):
        ...
        try:
            ...
            self.logger_error.error(f"添加品台行为日志时异常:{traceback.format_exc()}")
            return False
    ...
    def request_product_id(self):
        ...
        product_id = int(
            header_product_id) if header_product_id.strip() != "" else 0
        if product_id == 0:
    ...
    def get_product_info(self):
        ...
        if product_id > 0:
            product_info = ProductInfoModel(
                config.get_value("base_manage_context_key")).get_entity_by_id(
                    product_id)
        return product_info
    ...
    def get_curr_user_info(self):
        ...
        if user_id:
            user_info = UserInfoModel(
                self.get_manage_context_key()).get_entity_by_id(user_id)
        return user_info
    ...
    def get_page_cote_id(self):
        ...
        if not page_id == "":
            page_cote_id = page_id.split("$")[1] if page_id.__contains__(
                "$") else ""
        return page_cote_id
    ...
    def get_condition_by_body(self):
        ...
        for item in body_dict:
            if not item.__contains__("PageIndex") and not item.__contains__(
                    "PageSize"):
    ...
    def logout(self, user_id):
        ...
        if config.get_value("login_type") == "redis":
            self.redis_init().hdel(
                config.get_value("redis_provider_name_user_login"), user_id)
        else:
            UserLoginModel(config.get_value("manage_context_key")).del_entity(
                "UserID=%s", user_id)
    ...
    def check_power_menu(self, api_path, cote_id):
        ...
        menu_info_list = MenuInfoModel(manage_context_key).get_list(
            f"ApiPath LIKE '%{api_path}%'")
        if not menu_info_list:
            return False
        if len([i for i in menu_info_list if i.IsPower == 0]) > 0:
            return True
        role_user_list = RoleUserModel(manage_context_key).get_list(
            "UserID=%s", params=self.request_user_id())
        role_power_list = RolePowerModelEx(
            manage_context_key).get_role_power_list(
                [i.RoleID for i in role_user_list])
    ...
    def check_login(handler):
        def wrapper(self, **args):
            base_manage_context_key = config.get_value(
                "base_manage_context_key")
            # 获取头部信息
            header_token = self.request_header_token()
            user_id = header_token["UserID"] if header_token.__contains__(
                "UserID") else ""
            user_token = header_token[
                "UserToken"] if header_token.__contains__("UserToken") else ""
            user_id_md5 = header_token[
                "UserIDMD5"] if header_token.__contains__("UserIDMD5") else ""
            if user_id.strip() == "" or user_token.strip(
            ) == "" or user_id_md5.strip() == "":
                return self.reponse_common("NoLogin", "未登录")
            if is_need_login and CryptoHelper.md5_encrypt(
                    user_id,
                    config.get_value("login_encrypt_key")) != user_id_md5:
                return self.reponse_common("NoLogin", "未登录")

            # 登陆日志判断，Redis还是MySQL
            if config.get_value("login_type") == "redis":
                redis_user_login = self.redis_init().hget(
                    config.get_value("redis_provider_name_user_login"),
                    user_id)
                user_login = {}
                if redis_user_login:
                    user_login = json.loads(redis_user_login)

                if not user_login or TimeHelper.format_time_to_datetime(
                        user_login["ExpireTime"]
                ) < TimeHelper.get_now_datetime():
                ...
                curr_user_info = UserInfoModel(
                    config.get_value("manage_context_key")).get_entity_by_id(
                        user_id)
                ...
                user_login["ExpireTime"] = TimeHelper.add_hours_by_format_time(
                    hour=config.get_value("login_expire"))
                self.redis_init().hset(
                    config.get_value("redis_provider_name_user_login"),
                    user_id, self.json_dumps(user_login))
            else:
                user_login_model = UserLoginModel(base_manage_context_key)
                user_login = user_login_model.get_entity_by_id(user_id)

                if not user_login or TimeHelper.format_time_to_datetime(
                        user_login.ExpireTime) < TimeHelper.get_now_datetime():
                    if not is_need_login:
                        return
                    return self.reponse_common("UserExpire", "用户已过期，请重新登录")
                if user_login.UserToken != user_token:
                ...
                curr_user_info = UserInfoModel(
                    manage_context_key).get_entity_by_id(user_id)
                ...
                if base_manage_context_key != manage_context_key:
                    curr_user_info = UserInfoModel(
                        base_manage_context_key).get_entity_by_id(user_id)
                    if not curr_user_info:
                        return self.reponse_common("UserNoExist",
                                                   "用户不存在，请重新登录")
                    if curr_user_info.IsLock == 1:
                        return self.reponse_common("UserLimit", "用户已被锁，请联系管理员")
                ...
                user_login.ExpireTime = TimeHelper.add_hours_by_format_time(
                    hour=config.get_value("login_expire"))
                user_login_model.update_table(
                    f"ExpireTime='{user_login.ExpireTime}'", "UserID=%s",
                    user_login.UserID)
        ...
        def wrapper(self, **args):
            ...
            if user_info and base_user_info:
                if is_check_power and user_info.IsSuper != 1 and base_user_info.IsSuper != 1:
                    ...
                    page_id = self.request_manage_page_id()
                    cote_id = page_id.split("$")[1] if page_id.__contains__(
                        "$") else ""
