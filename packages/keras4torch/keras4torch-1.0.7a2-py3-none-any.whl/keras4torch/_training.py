...
class MetricsRecorder():
    def __init__(self, metrics, epoch_metrics, total_count) -> None:
    ...
    def update(self, y_batch_pred, y_batch):
        count = len(y_batch)
        self.__update_batch_metrics(y_batch_pred, y_batch, count)

        y_batch_pred = y_batch_pred.cpu()
        y_batch = y_batch.cpu()

        if self.has_epoch_metrics:
            self.__update_epoch_metrics(y_batch_pred, y_batch, count)
        
        self.current_count += count

    def __update_epoch_metrics(self, y_batch_pred, y_batch, count):
        if self.y_pred is None:
            self.y_pred = torch.empty_like(y_batch_pred)
            self.y_pred.resize_(self.total_count, *y_batch_pred.shape[1:])

            self.y_true = torch.empty_like(y_batch)
            self.y_true.resize_(self.total_count, *y_batch.shape[1:])

        start = self.current_count
        end = start + count
        self.y_pred[start:end] = y_batch_pred
        self.y_true[start:end] = y_batch

    def __update_batch_metrics(self, y_batch_pred, y_batch, count):
        batch_metrics = calc_metrics(y_batch_pred, y_batch, self.metrics)

        for key, value in batch_metrics.items():
            self.accum_metrics[key] += value * count
        
    def average_epoch_metrics(self):
        if not self.has_epoch_metrics:
            return OrderedDict()

        end = self.current_count
        return calc_metrics(self.y_pred[:end], self.y_true[:end], self.epoch_metrics)
    ...
    def average_batch_metrics(self):
        return OrderedDict([(k, (v/self.current_count)) for k,v in self.accum_metrics.items()])

    def average(self, reset=True):
        metrics = self.average_batch_metrics()
        metrics.update(self.average_epoch_metrics())
...
class Trainer(object):
    ...
    def run(self, train_loader, val_loader, max_epochs, verbose, use_amp, accum_grad_steps):
        ...
        metrics_rec = MetricsRecorder(self.metrics, self.epoch_metrics, len(train_loader.dataset))
        ...
        for epoch in range(1, max_epochs+1):
            ...
            # training
            train_metrics = self.train_on_epoch(train_loader, metrics_rec)
            # validation
        ...
        metrics_rec.free_memory()

        self.__fire_event(Events.ON_TRAIN_END)
        return self.logger.history

    def train_on_epoch(self, data_loader, metrics_rec) -> None:
        self.model.train()
    ...
    def valid_on_epoch(self, data_loader, use_amp):
        self.model.eval()
        metrics_rec = MetricsRecorder(self.metrics, self.epoch_metrics, len(data_loader.dataset))
