...
class BaseExperiment(CommonExperiment):
    ...
    def log_code(self, file_name=None, folder=None):
        # type: (Any, Any, Optional[int]) -> Optional[List[Tuple[str, Dict[str, str]]]]
        """
        ...
        if file_name is None and folder is None:
            if _in_ipython_environment():
        ...
        elif file_name is not None and folder is not None:
            LOGGER.warning(LOG_CODE_FILE_NAME_FOLDER_MUTUALLY_EXCLUSIVE)
            return None

        elif file_name is not None:
            log_result = self._log_asset(
                file_data=file_name, file_name=file_name, asset_type="source_code",
            )
