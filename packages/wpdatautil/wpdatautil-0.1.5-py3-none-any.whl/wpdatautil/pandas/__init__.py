...
def dedup_df(df: pd.DataFrame, *, df_description: str = "dataframe") -> float:
    ...
    """
    len_pre_drop = len(df)
    log.debug(f"Dropping duplicate rows from {len_pre_drop:,} rows of the {df_description}.")
    timer = Timer()
    ...
    logger = log.info if len_dropped else log.debug
...
def denull_df(df: pd.DataFrame, *, df_description: str = "dataframe", criteria_description: str = "criteria", **kwargs: Any) -> float:
    ...
    """
    len_pre_drop = len(df)
    log.debug(f"Dropping rows meeting {criteria_description} from {len_pre_drop:,} rows of the {df_description} using: {kwargs}")
    timer = Timer()
    ...
    logger = log.info if len_dropped else log.debug
    logger(
        f"Dropped {len_dropped:,} ({rate_dropped:.2%}) rows in {timer} meeting {criteria_description} from the {df_description}, "
