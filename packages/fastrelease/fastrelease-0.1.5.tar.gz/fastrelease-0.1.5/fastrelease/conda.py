...
__all__ = ['pypi_json', 'latest_pypi', 'pypi_details', 'update_meta', 'conda_output_path', 'write_pip_conda_meta',
           'write_conda_meta', 'anaconda_upload', 'fastrelease_conda_package']
...
def conda_output_path(name,ver):
    ...
    return f"{os.getenv('CONDA_PREFIX')}/conda-bld/noarch/{name}-{ver}-py_0.tar.bz2"
...
# Cell
def anaconda_upload(name, version, user=None):
    "Update `name` `version` to anaconda"
    user = f'-u {user} ' if user else ''
    return run(f'anaconda upload {user} {conda_output_path(name,version)} --skip-existing')
