...
__global__ void kernal_SparseLutN_ForwardTraining
        (
            T   const   *x_buf,
            T           *y_buf,
...
{
    const int WN        = N <= 5 ? N : 5;
    const int SN        = N <= 5 ? 0 : 5;
    ...
    int frame_id   = (threadIdx.x >> 5);
    int frame_step = (blockDim.x >> 5);

    __shared__  T           sbuf[MAX_NODE_UNIT][MAX_FRAME_UNIT];

    __shared__  T           W[MAX_NODE_UNIT][SUB_SIZE][WARP_SIZE];
                T   const   *x_ptr[N];
                T           *y_ptr;
    ...
    if ( node < node_size ) {
        ...
        for ( int i = 0; i < SUB_SIZE; ++i ) {
            T w = W_buf[node * LUT_SIZE + i * WARP_SIZE + tid];
            if ( lut_binarize ) { w = w > 0.5 ? 1.0 : 0.0; }
            W[node_id][i][tid] = w;
        }
    ...
    for (int frame = id; frame < frame_size; frame += id_step) {
        if ( node < node_size ) {
            ...
            T y = StochasticLutN<N, T, MAX_NODE_UNIT>::NodeForward(node_id, x, W);
