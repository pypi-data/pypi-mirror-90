...
class RabbitmqClient(object):
    ...
    def _connect(self):
        if not self._connected:
            try:
                ...
                self._channel.exchange_declare(exchange=self._exchange_name, exchange_type=self._exchange_type, durable=self._durable)
                if self._queue:
    ...
    def publish_msg(self, msg_str, retry=5):
        while retry>0:
