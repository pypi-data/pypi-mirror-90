...
tests to implement:
    - single doy, lat, lon, but list of depths (a single profile), using track, with last depth np.nan. At some point it was returning the last value as 1e20 instead of a masked point.
"""
...
def test_coincident_gridpoint_13():
    ...
    t = db["sea_water_temperature"].extract(
        var="t_mn", doy=136.875, depth=0, lat=17.5, lon=-37.5
    )
    assert np.allclose(t["t_mn"], [24.60449791])

    t = db["sea_water_temperature"].extract(
        var="t_mn", doy=[136.875, 228.125], depth=0, lat=17.5, lon=-37.5
    )
    assert np.allclose(t["t_mn"], [24.60449791, 26.38446426])

    t = db["sea_water_temperature"].extract(
        var="t_mn", doy=136.875, depth=[0, 10], lat=17.5, lon=-37.5
    )
    assert np.allclose(t["t_mn"], [24.60449791, 24.62145996])

    t = db["sea_water_temperature"].extract(
        var="t_mn", doy=136.875, depth=0, lat=[17.5, 12.5], lon=-37.5
    )
    assert np.allclose(t["t_mn"], [25.17827606, 24.60449791])

    t = db["sea_water_temperature"].extract(
        var="t_mn", doy=136.875, depth=0, lat=17.5, lon=[-37.5, -32.5]
    )
    assert np.allclose(t["t_mn"], [24.60449791, 23.98172188])

    t = db["sea_water_temperature"].extract(
        var="t_mn", doy=136.875, depth=[0, 10], lat=[17.5, 12.5], lon=-37.5
    )
    assert np.allclose(
        t["t_mn"], [[25.17827606, 24.60449791], [25.25433731, 24.62145996]]
    )
...
def test_coincident_gridpoint():
    ...
    assert np.allclose(t["t_mn"], [[25.167528, 24.641016], [25.190489, 24.636028]])
...
def test_only_one_coincident_gridpoint():
    ...
    t = db["sea_water_temperature"].extract(
        var="t_mn", doy=136.875, depth=0, lat=-14.03, lon=62.5
    )
    assert np.allclose(t["t_mn"], [27.41223907])

    t = db["sea_water_temperature"].extract(
        var="t_mn", doy=136.875, depth=0, lat=-12.5, lon=62.03
    )
...
def test_lon_cyclic():
    ...
    t1 = db["sea_water_temperature"].extract(
        var="t_mn", doy=136.875, depth=0, lat=17.5, lon=182.5
    )
    t2 = db["sea_water_temperature"].extract(
        var="t_mn", doy=136.875, depth=0, lat=17.5, lon=-177.5
    )
    ...
    t2 = db["sea_water_temperature"].extract(
        var="t_mn", doy=136.875, depth=0, lat=17.5, lon=[322.5, 327.5]
    )
    assert np.allclose(t1["t_mn"], t2["t_mn"])
...
def test_extract_overlimit():
    ...
    t = db["sea_water_temperature"].extract(
        var="t_mn", doy=136.875, depth=5502, lat=17.5, lon=-37.5
    )
    assert ma.is_masked(t["t_mn"])

    t = db["sea_water_temperature"].extract(
        var="t_mn", doy=136.875, depth=[10, 5502], lat=17.5, lon=-37.5
    )
    assert np.all(t["t_mn"].mask == [False, True])
    assert ma.allclose(t["t_mn"], ma.masked_array([24.62145996, 0], mask=[False, True]))
...
def test_get_point():
    ...
    t = db["sea_water_temperature"].extract(
        var="t_mn", doy=90, depth=0, lat=17.5, lon=-37.5
    )
...
def test_get_point_inland():
    ...
    t = db["sea_water_temperature"].extract(
        var="t_mn", doy=90, depth=0, lat=-19.9, lon=-43.9
    )
...
def test_get_profile():
    ...
    t = db["sea_water_temperature"].extract(
        var="mean", doy=10, depth=[0, 10], lat=10, lon=330
    )
    assert np.allclose(t["mean"], [26.07524300, 26.12986183])

    t = db["sea_water_temperature"].extract(doy=10, depth=[0, 10], lat=10, lon=330)
...
def test_profile_maskedDepth():
    ...
    db["sea_water_temperature"].extract(
        var="mean", doy=10, depth=depth, lat=10, lon=330
    )
...
def test_track():
    db = WOA(dbname="WOA18")
    params = [
        [{"doy": 34, "depth": 0, "lat": 10, "lon": 330}, [25.51016]],
        [{"doy": 120, "depth": 0, "lat": 10, "lon": 330}, [25.795078]],
        [{"doy": 34, "depth": 300, "lat": 10, "lon": 330}, [10.52824]],
        [{"doy": 34, "depth": 0, "lat": 12, "lon": -25}, [24.412867]],
        [{"doy": [34, 120], "depth": 0, "lat": 10, "lon": 330}, [25.51016, 25.795078]],
        [{"doy": 34, "depth": [0, 300], "lat": 10, "lon": 330}, [25.51016, 10.52824]],
        [
