...
def evaluate_catalog(y:list, predicted:list):
    ...
    print('Accuracy: ' + str(results['n'].mean()))
    ...
    predict = output.groupby(['predicted']).sum().reset_index()
    predict.pop('actual')
    predict.columns = ['predicted', 'predicted_total'] 
    
    output = output.merge(actual, on='actual').merge(predict, on='predicted') 
    output['% of actual'] = output['n']/output['actual_total']
    output['% of predicted'] = output['n']/output['predicted_total']
    display(output.sort_values(['actual', 'predicted']))
...
def balance(data:pd.DataFrame, key:str):
    ...
    for r in range(1, value.shape[0]):
        d = data[data[key] == value.loc[r, 'index']].reset_index(drop=True)
        output.append(d.loc[np.random.choice(np.arange(len(d)), l)])
...
def add_econ_variables(data:pd.DataFrame, datekey:str, tickers:list, start:datetime = datetime.datetime(1910, 1, 1), change:bool = False):
    ...
    for ticker in tickers:
        econ = web.DataReader(('ticker='+ticker), 'econdb', start).sort_index().reset_index()
