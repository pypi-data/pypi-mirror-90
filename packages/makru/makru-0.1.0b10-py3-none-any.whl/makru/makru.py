...
class Sandbox(object):
    ...
    def set_build_store(self, store: Store):
        if self._build_store:
            panic("build store could not be overrided, old: {}, new: {}".format(self._build_store, store))
...
class OptionDescription(object):
    def __init__(self, *, plugin_name: str=None, description: str=None, multiple=False, bool_value=False, mix_into: str=None) -> None:
        self.plugin_name = plugin_name
        if mix_into and (not description):
            self.description = "alias for {}".format(mix_into)
        else:
            self.description = description
        assert not (multiple and bool_value) # option could not accept multiple bool values
        assert not (mix_into and (multiple or bool_value)) # mix_into means this option just a alias for the mixed variable
...
class Makru(object):
    ...
    def definde_option(self, name: str, description: OptionDescription):
        if name in self.option_descriptions:
            panic("{} have been exists in options. the old one is by {}, the new one is by {}".format(name, self.option_descriptions[name].plugin_name, description.plugin_name))
    ...
    def search_config_path(cls, arguments: List[str]):
        ...
        list(map(arguments.pop, config_indexs))
        return result
    
    def show_helps(self):
        print("{} [:<action> [<action_argument>] [...]] [-F<path>] [-C] [--<key>=<value> --<key> --!<key>...] [-h]".format("makru"))
        print(":<action> [<action_argument>]", "set the <action> you want to call, optionally uses <action_argument> as argument")
        print("-F<path>", "use <path> as makru config file or project folder needed to find makru config")
        ...
        for name in option_keys:
            ...
            if optiond.multiple:
                option_show = "--{name}=<value>/{name}=<value> [--{name}=<value>/{name}=<value>]".format(name=name)
    ...
    def set_var(self, vars, key, value):
        ...
        if option_d:
            if option_d.mix_into:
                self.set_var(self, vars, option_d.mix_into, value)
                ...
                if not isinstance(value, bool):
                    print("WARN variable {}={} is forced to convert to boolean as plugin {} define".format(key, value, option_d.plugin_name))
                    value = not not value
                vars[key] = value
            else:
                vars[key] = value
            ...
            if arg.startswith("--"):
                ...
                if len(parts) == 1:
                    ...
                    if key[0] == "!":
                        ...
                        self.set_var(options['vars'], key[1:], True)
                elif len(parts) == 2:
                    key = parts[0]
                    value = self.str2val(parts[1])
                    self.set_var(options['vars'], key, value)
                else:
                    panic("could not parse command line argument: undefined format {}, use -h option for help".format(arg))
            elif "=" in arg:
                parts = arg.split("=")
                true_or_panic(
                    len(parts) == 2,
                    "could not parse command line argument: undefined format {}, use -h option for help".format(arg)
                )
                ...
                if shell_like_action_arg:
                    ...
                    panic("could not parse command line argument: unknown option {}, use -h option for help".format(arg))
        return options
