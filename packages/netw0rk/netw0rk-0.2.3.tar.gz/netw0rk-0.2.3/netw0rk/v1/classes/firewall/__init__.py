...
class FireWall(object):
	...
	def enable(self):
		...
		if "Firewall is active and enabled on system startup" in output:
			return r3sponse.success_response(f"Successfully enabled the firewall.")
		else:
			return r3sponse.error_response(f"Failed to enable the firewall.")
	...
	def disable(self):
		...
		if "Firewall stopped and disabled on system startup" in output:
			return r3sponse.success_response(f"Successfully disabled the firewall.")
		else:
			return r3sponse.error_response(f"Failed to disable the firewall.")
	...
	def allow(self, port):
		...
		if "Rule updated" in output or "Skipping adding existing rule" in output:
			return r3sponse.success_response(f"Successfully allowed port [{port}].")
		else:
			return r3sponse.error_response(f"Failed to allow port [{port}].")
	...
	def deny(self, port):
		...
		if "Rule updated" in output or "Skipping adding existing rule" in output:
			return r3sponse.success_response(f"Successfully denied port [{port}].")
		else:
			return r3sponse.error_response(f"Failed to deny port [{port}].")
	...
	def allow_all(self):
		...
		for port, info in ports.items():
			...
			if response["error"] != None: return response
		return r3sponse.success_response(f"Successfully allowed all {len(ports)} ports.")
	def deny_all(self):
		response = self.info()
		if response["error"] != None: return response
	...
	def set_default(self, deny=True):
		...
		if "Default incoming policy changed to " in output:
			return r3sponse.success_response(f"Successfully set the default action to [{option}].")
		else:
			return r3sponse.error_response(f"Failed set the default action to [{option}].")
	...
	def info(self):
		...
		# success.
		return r3sponse.success_response("Successfully retrieved the firewall info.", {
			"status":status,
			"ports":ports,
		})
	...
	def __check_os__(self):
		...
		return r3sponse.success_response(f"Supported operating system: [{OS}].")
