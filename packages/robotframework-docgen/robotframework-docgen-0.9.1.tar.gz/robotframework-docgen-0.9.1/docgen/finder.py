...
def rpaframework():
    ...
    for path in sys.path:
        if (Path(path) / "RPA").is_dir():
            paths.update(walk_directory(path, dirs=["RPA"]))
...
def walk_directory(root, libraries=True, resources=True, dirs=None):
    dirs = dirs or ["."]
    paths, stack = set(), [Path(root) / dirname for dirname in dirs]
...
def is_module_library(path, root):
    ...
    try:
        ...
        return hasattr(module, path.stem)
    except ImportError:
        return False
