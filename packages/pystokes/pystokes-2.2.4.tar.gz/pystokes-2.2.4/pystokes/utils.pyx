...
def minDistance(data, ii):
    """
    data: array of size (Nt, 6*Np) where Np is #particles and Nt is #timePoints
            * 0:Np    points are x coordinates
            * Np:2*Np points are y coordinates
            * Np:3*Np points are z coordinates 
            * the remaining 3*Np are coordinates of orientations
    ii: time index at which the minDistance is required

    returns: min distance between particles
    """
    X = data['X']
    ii=350;    rp = X[ii, :]
    Np = int(np.size(rp)/6)
    ...
    dx = np.abs(rx - rx[:, np.newaxis])  # matrix of distance in x-direction
    dy = np.abs(ry - ry[:, np.newaxis])
    dz = np.abs(rz - rz[:, np.newaxis])
    dr = np.sqrt(dx*dx + dy*dy + dz*dz)
    
    dr1 = dr.reshape(Np*Np)              # flattened matrix of distances between particles
    dr1[::Np+1]=100000 # make a large self-distancing!
