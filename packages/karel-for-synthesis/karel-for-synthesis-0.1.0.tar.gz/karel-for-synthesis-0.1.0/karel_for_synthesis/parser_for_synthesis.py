...
from .errors import KarelSyntaxError, TimeoutError
...
class KarelForSynthesisParser(Parser):
    ...
    def token_list_to_tokenfunc(self, tokens):
        try:
            tokens = [
                self.string_to_token_map[token](i) for i, token in enumerate(tokens)
            ]
        ...
        if self.build_tree:
            ...
            def repeat():
                for i in range(limit, 0, -1):
                    self.karel.event_callback("repeat", span, cste.span, i, true_span)
                    stmt()
                self.karel.event_callback("repeat", span, cste.span, 0, false_span)
    ...
    def p_action(self, p):
        ...
        if self.build_tree:
            ...
            action = functools.partial(getattr(self.karel, action_name), metadata=span)
        p[0] = action
...
type_to_list_fn = {
    ...
    "if": lambda v: ("IF", "c(")
    + tree_to_tokens(v["cond"])
    + ("c)", "i(")
    + tree_to_tokens(v["body"])
    + ("i)",),
    "ifElse": lambda v: ("IFELSE", "c(")
    + tree_to_tokens(v["cond"])
    + ("c)", "i(")
    + tree_to_tokens(v["ifBody"])
    + ("i)", "ELSE", "e(")
    + tree_to_tokens(v["elseBody"])
    + ("e)",),
    "while": lambda v: ("WHILE", "c(")
    + tree_to_tokens(v["cond"])
    + ("c)", "w(")
    + tree_to_tokens(v["body"])
    + ("w)",),
    "repeat": lambda v: ("REPEAT",)
    + tree_to_tokens(v["times"])
    + ("r(",)
    + tree_to_tokens(v["body"])
    + ("r)",),
...
for k in (
    KarelForSynthesisParser.conditional_functions
