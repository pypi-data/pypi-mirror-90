...
def preprocessing(
    ...
    cell_subsets = []
    for i in range(adata.shape[0]//chunk_size+1):
        cell_subset, _ = sc.pp.filter_cells(adata[i*chunk_size:(i+1)*chunk_size], min_genes=min_genes, inplace=False)
        cell_subsets.append(cell_subset)
    indices = np.where(np.concatenate(cell_subsets))[0]
    adata = adata[indices]
    
    if log: log.info('Filtering genes')
    sc.pp.filter_genes(adata, min_cells=min_cells)
#     adata = adata[np.where(np.sum(adata.X>0, 0))]
#     gene_set, _ = sc.pp.filter_genes(adata, min_cells=min_cells, inplace=False)
#     log.info('subset gene')
#     adata = adata[:, np.where(gene_set)[0]]

    if log: log.info('Normalizing total to {}'.format(target_sum))
    X_ = []
    for i in range(adata.shape[0]//chunk_size+1):
        X = sc.pp.normalize_total(adata[i*chunk_size:(i+1)*chunk_size], target_sum=target_sum, exclude_highly_expressed=True, inplace=False)['X']
        X_.append(X)
#     sc.pp.normalize_total(adata, target_sum=target_sum, exclude_highly_expressed=True)
    adata.X = scipy.sparse.vstack(X_)
    del X_
    del X
    ...
    if log: log.info('Finding variable genes')
