...
class QuoteData(object):
    ...
    @assert_quote_init
    def execute_technical_analysis(self, data, indications, inplace=False, skip_sort=False) -> pd.DataFrame:
        """执行技术指标运算
        ...
        for v in indications:
            v = [v] if type(v) is str else list(v)
            c = v.pop(0)
            func = globals().get(c)
            if not func:
            ...
            if not hasattr(func, '__wrapped__'):
                ...
                continue
            f_code = getattr(func, '__wrapped__').__code__
            f_args = [x for x in f_code.co_varnames[:f_code.co_argcount]]
