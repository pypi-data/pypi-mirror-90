...
This namespace portion is providing a set of widgets for to allow the user of your app to change the her/his personal
app settings/preferences, like the theme, the font size, the language and the used colors.

For to use it in your app import this module, which can be done either in one of the modules of your app via::
...
.. note::
    ...
    sure that the translation texts of your main app get registered after the import of this portion. For that you
    can use the `on_app_build` event which gets fired after Kivy has imported the main KV file.


The user preferences are implemented as a :class:`~ae.kivy_app.FlowDropDown` via the widget `UserPreferencesOpenPopup`.

For to integrate it in your app you simply add the `UserPreferencesButton` widget to the main KV file of your app.
