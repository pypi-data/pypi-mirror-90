...
class MapControls(wx.Panel):
    def __init__(self, parent, id, canvas):
        ...
        self.build_ctrls()
        self.build_menu()
        self.text_prop_dialog = TextPropDialog(self.map_frame)
    ...
    def build_ctrls(self):
        ...
        self.Bind(wx.EVT_TOOL, self.map_frame.on_open, id=Tool.MAP_OPEN)
        self.Bind(wx.EVT_TOOL, self.map_frame.on_save, id=Tool.MAP_SAVE)
    ...
    def on_left_down(self,evt):
        ...
        if session.denied(ROLE_PLAYER):
            open_rpg.get_component("chat").InfoPost(
                "You must be either a player or GM to use this feature")
            return
