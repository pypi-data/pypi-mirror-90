class stock(object):
    ...
    def sharp(self, df, no_risk_rate=0.45/5200):
        ...
        for i in lists:
            x = df[i[0]]
            y = df[i[1]]
            a = (np.dot([xi - np.mean(x) for xi in x], [yi - np.mean(y)
                                                        for yi in y])/(len(x)-1))/((x.std())*(y.std()))
        for i in lists:
            i.append(a)
        weights = np.random.dirichlet(
            np.ones(len(list(df.columns))), size=1)[0]
        rand = {}
        ...
        df = DataFrame([[rate, risk, sharp, rand]], columns=[
                        'rate', 'risk', 'sharp', 'weights'])
    ...
    def Markowit(self, names, number=500, start_date='2018-11-01', end_date='2020-12-31', frequency='w', save=True):
        ...
        for i, j in zuhe.iterrows():
            ...
            for k in trange(number):
                ...
                all = concat(result)
                all.to_csv(
                    r'all/{}.csv'.format(str(j[0])[1:-1]), encoding='utf8')
        path = r'./all'
        filenames = []
        ...
        df = DataFrame(result, columns=[
                        'names', 'sharp', 'weights', 'rate', 'risk'])
