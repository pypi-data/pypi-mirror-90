...
def get_significant_tokens(source):
    ...
    If an exception is raised by Python's tokenize module,
    the list of tokens accumulated up to that point is returned.
    """
    tokens = []
    try:
        for tok in py_tokenize.generate_tokens(StringIO(source).readline):
...
def get_comments(source):
    """Gets a list of comment tokens from a source (str).
