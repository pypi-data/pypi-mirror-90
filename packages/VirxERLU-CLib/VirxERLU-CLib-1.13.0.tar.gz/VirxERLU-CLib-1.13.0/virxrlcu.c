...
// Constants (& some that aren't xD)

static const double simulation_dt = 1. / 20.;
static const double physics_dt = 1. / 120.;
static const double min_simulation_distance = 25.;

static const double min_shot_time = 0;
static const double max_shot_time = 6;
static const double max_shot_slope = 3;
double max_shot_0_slope = 10;
static const double ground_shot_min_slope = 1;
static const double jump_shot_min_slope = 1.25;
static const double double_jump_shot_min_slope = 1.5;
static const double aerial_shot_min_slope = 2.9;
...
Vector get_final_target(Vector *ball_offset, Vector *shot_vector, Vector *car_to_ball, Car *me, double *T)
{
    ...
    double adjustment = angle2D(car_to_offset_target, *shot_vector) * *T * 1000;
...
_Bool can_reach_target_forwards(double max_time, double jump_time, double *boost_accel, double *distance_remaining, double *car_speed, unsigned char *car_boost, double *max_speed_reduction)
{
    ...
    const double ms = max_speed - ceil(*max_speed_reduction);
    const double ms_ba_dt = ms - ba_dt;
    ...
    const double th_dt = aerial_throttle_accel * simulation_dt;
    const double ms_th_dt = ms - th_dt;
...
_Bool can_reach_target_backwards(double max_time, double jump_time, double *distance_remaining, double *car_speed)
{
    ...
    const double th_dt = aerial_throttle_accel * simulation_dt;
    const double ms_th_dt = max_speed - th_dt;
...
_Bool legacy_generic_is_viable(double *T, double jump_time, double *boost_accel, Vector *car_forward, double *car_speed, unsigned char *car_boost, Vector *direction, double *distance_remaining, double *max_speed_reduction)
{
    ...
    double forward_time = *T - (forward_angle * turn_rad / abs_car_speed);
    double backward_time = *T - (backward_angle * turn_rad / abs_car_speed);
...
_Bool generic_is_viable(double *T, double jump_time, double *boost_accel, Car *me, Vector *direction, double *distance_remaining, double *max_speed_reduction)
{
    ...
    char car_dir = sign((int)true_car_speed);
    double turn_rad = turn_radius(car_speed);

    double forward_time = *T - (forward_angle * turn_rad / car_speed);
    double backward_time = *T - (backward_angle * turn_rad / car_speed);
...
_Bool legacy_ground_shot_is_viable(double *T, HitBox *hitbox, double *boost_accel, double *car_z, double *distance_remaining, Vector *direction, Vector *car_forward, unsigned char *car_boost, double *car_speed, double *slope)
{
	...
	return legacy_generic_is_viable(T, 0, boost_accel, car_forward, car_speed, car_boost, direction, distance_remaining, &speed_reduction);
}
...
_Bool ground_shot_is_viable(double *T, double *boost_accel, Car *me, double *offset_target_z, Vector *direction, double *distance_remaining, double *slope)
{
    ...
    if (*offset_target_z >= (92.75 + (me->hitbox.height / 2)) || me->airborne || *slope < get_adjusted_min_slope(T, &slope_jump_time, &ground_shot_min_slope))
        return 0;

    return generic_is_viable(T, 0, boost_accel, me, direction, distance_remaining, &speed_reduction);
}
...
_Bool jump_shot_is_viable(double *T, double *boost_accel, Vector *gravity, Car *me, double *offset_target_z, Vector *direction, double *distance_remaining, double *slope)
{
    ...
    if (*offset_target_z < 92.75 + (me->hitbox.height / 2) || *offset_target_z > max_jump_height + (me->hitbox.height / 2) || me->airborne || *slope < get_adjusted_min_slope(T, &jump_info.time, &jump_shot_min_slope))
        return 0;
...
_Bool double_jump_shot_is_viable(double *T, double *boost_accel, Vector *gravity, Car *me, double *offset_target_z, Vector *direction, double *distance_remaining, double *slope)
{
    ...
    if (*offset_target_z < min_double_jump_height + (me->hitbox.height / 2) || *offset_target_z > max_double_jump_height + (me->hitbox.height / 2) || me->airborne || *slope < get_adjusted_min_slope(T, &jump_info.time, &double_jump_shot_min_slope))
        return 0;
...
Shot parse_slice_for_shot(_Bool do_ground_shot, _Bool do_jump_shot, _Bool do_double_jump_shot, _Bool do_aerial_shot, double *time_remaining, double *ball_radius, double *boost_accel, Vector *gravity, Ball *ball_slice, Car *me)
{
	...
	{
        // this is 2D stuff, as these shots are mostly 2D
        double distance_remaining = flat_dist(ball_offset, me->location) - (me->hitbox.length / 3);
        Vector offset_direction = normalize(sub(ball_offset, me->location));

		if (do_ground_shot)
		{
            r.found = ground_shot_is_viable(time_remaining, boost_accel, me, &ball_offset.z, &offset_direction, &distance_remaining, &max_shot_0_slope);
        	r.shot_type = GROUND_SHOT;
		}

		if (do_jump_shot && !r.found)
		{
            r.found = jump_shot_is_viable(time_remaining, boost_accel, gravity, me, &ball_offset.z, &offset_direction, &distance_remaining, &max_shot_0_slope);
        	r.shot_type = JUMP_SHOT;
		}
	...
	{
        // this is 3D stuff, as aerials are mostly 3D
	    r.found = aerial_shot_is_viable(time_remaining, boost_accel, gravity, me, ball_offset, &r.fast, 1);
    	r.shot_type = AERIAL_SHOT;
	}
