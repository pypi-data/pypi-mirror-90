...
class Configuration:
    ...
    def __init__(self):
        ...
        if not os.path.isfile(user_config):
            ...
            shutil.copyfile(path+"/default_config.ini.py", user_config)
        ...
        if self.config.has_section(configured_active_profile):
            if self.config.has_section(configured_active_profile):
                if (self.config.get(configured_active_profile, 'api_key') and self.config.get(configured_active_profile,
                    ...
                    click.echo('account/api_key is not set')

            else:
                click.echo('exception profile is not set')

        else:
            click.echo('profile is not selected')
