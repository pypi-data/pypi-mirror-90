...
class Merger:
    def __init__(
        ...
        self.primary = pd.DataFrame.from_records(
            sorted(self._map(normalize, primary), key=sorter)
        )

        self.secondary = pd.DataFrame.from_records(
            sorted(self._map(normalize, secondary), key=sorter)
        )
    ...
    @staticmethod
    def _map(mapping, dataset: List[dict]):
        """
        Map equivalent naming conventions
        """
        for count, attributes in tqdm.tqdm(enumerate(dataset)):
    ...
    @staticmethod
    def map(mapping, dataset: pd.DataFrame) -> pd.DataFrame:
        """
        Map equivalent naming conventions (pandas)
        """
