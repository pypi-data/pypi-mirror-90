    ...
    def __init__(self, url, insecure=INSECURE,
        ...
        for (addr, family) in addrinfo_set:
            ...
            try:
                ...
                if not insecure:
                    valid = validate_hostname(a_host, cert, iri)
                    if not valid:
                        self.error = "Name %s not in certificate" % a_host
                        continue
                ...
                if tofu is not None and tofu != "":
                    ...
                    pkeyfile_name = os.path.join(tofu, a_host + tport)
                    try:
                        pkeyfile = open(pkeyfile_name, "r")
                        old_key = pkeyfile.read()
                        if self.keystring != old_key:
                            self.error = "Former public key was %s, new one is %s. If the change is legitimate, delete %s " % \
                                              (old_key, self.keystring, pkeyfile_name)
                            continue
                    ...
                    pkeyfile.close()
                request = iri_to_uri(urllib.parse.urlunsplit(("gemini", a_host + tport, components.path , query, None))) + "\r\n" # We
