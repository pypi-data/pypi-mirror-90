...
tag_dict, tags = cfg.assign_all_framework_tags(charged_structure,indices)
charged_structure.set_tags(tags)
...
superstructure = charged_structure * (supercell[0],supercell[1],supercell[2])
...
with open('{0}ex3_CONFIG'.format(outdirectory), 'w') as f:
    f.write(str(config))
...
with open('{0}ex3_{1}.FIELD'.format(outdirectory,N2['name']), 'w') as f:
