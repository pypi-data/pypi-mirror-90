...
def run_policy(policy: str, region: str, dry_run: str):
    ...
    # Custom policy Zombie
        ...
        policy_path = os.path.join(os.path.dirname(__file__), f'/policy/{policy}.yml')
        if os.path.isfile(policy_path):
            os.system(f'custodian run {dry_run} -s {policy_output} {policy_path}')
        else:
            raise Exception(f'Missing Policy name: {policy_path}')
            logger.exception(f'Missing Policy name: {policy_path}')
