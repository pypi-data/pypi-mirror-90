...
def bulk(insee_region_names, date_=None, fetch_episodes=False):
    ...
    current_app.logger.info(f'Timer 1: {time() - start}')
    start = time()
    ...
    current_app.logger.info(f'Timer 2: {time() - start}')
    start = time()
    if fetch_episodes:
        for insee in insee_region_names.keys():
    ...
    current_app.logger.info(f'Timer 3: {time() - start}')
    start = time()
    to_return = {
        insee: {
            "forecast": make_resp(
    ...
    current_app.logger.info(f'Timer 4: {time() - start}')
    start = time()
    if fetch_episodes:
    ...
    current_app.logger.info(f'Timer 5: {time() - start}')
    return to_return
