...
def find_scenes(video_path, is_update=False, print_infos=False):
    ...
    scene_file_path = os.path.join(scene_dir, 'scene.csv')
    ...
    stats_file_path = os.path.join(scene_dir, 'stats.csv')
...
def moviepy_split_video(file, scene_list, out_dir=None, is_thumbnail=False, thumbnail_width=400, thumbnail_height=None):
    """
    按场景分割视频
    ...
    for i, scene in enumerate(scene_list):
        ...
        # print(scene[0].get_seconds(), scene[1].get_seconds(), i, scene)
        ...
        if is_thumbnail and subclip.w > thumbnail_width:
            ...
            subclip.write_videofile(os.path.join(out_dir, f'{start}.mp4'), codec='libx264', bitrate='1000k')
        else:
