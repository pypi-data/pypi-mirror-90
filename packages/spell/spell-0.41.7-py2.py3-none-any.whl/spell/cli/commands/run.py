...
@click.option(
    "--project", default=None, help="Name of project to add this run to", hidden=True,
)
@stop_condition_option
@click.pass_context
def run(
    ...
    **kwargs,
    ...
    if project:
        try:
            ...
            if click.confirm(
                f"A project named '{project}' could not be found. "
                "Would you like to create a new one?"
            ):
                with api_client_exception_handler():
                    proj = client.create_project({"name": project})
            else:
                raise
        run_req.project_id = proj.id
