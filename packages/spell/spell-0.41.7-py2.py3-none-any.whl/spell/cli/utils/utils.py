...
def get_project_by_name(client, name, is_archived=False):
    with api_client_exception_handler():
        all_projects = client.list_projects(is_archived)
    matching = [proj for proj in all_projects if proj.name.lower() == name.lower()]
    if len(matching) == 0:
        if is_archived is True:
            raise NotFoundException(
                f"Unkown project `{name}`. Run `spell project list --archived` to see all currently archived projects"
            )
        raise NotFoundException(
