      ...
      .on("hide_node.jstree", $.proxy(function (e, data) {
        ...
        newtree._reattachColumns(obj.id);
			...
			.on("enable_node.jstree", $.proxy(function (e, data) {
				var id = data.node.id;
				if (this._domManipulation) {
					/*
					 * We're being called from inside a _detach_columns..._reattach_columns
					 * block, so the columns aren't part of the DOM at present.
					 * Example: this was triggered by a `open_node.jstree` event callback,
					 * as jstreegrid wraps jstree.open_node in detach...reattach.
					 */
					for (var i = 0, len = this._gridSettings.gridcols.length; i < len; i++) {
						if (i === this._gridSettings.treecol) { continue; }
						findDataCell(this.uniq, id, this._gridSettings.gridcols[i],
												 $(this._domManipulation.columns[i]))
							.removeClass("jstree-disabled");
					}
				} else {
					/* The columns should be findable normally */
