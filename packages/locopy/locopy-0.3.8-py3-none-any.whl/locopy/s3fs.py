...
"""S3 Module
Module to wrap the s3fs api usage and provide functionality to manage
...
class S3(object):
    ...
    **kwargs
    ...
    def _set_session(self):
        try:
            self.s3 = S3FileSystem(**{'profile': self.profile})
            # self.session = Session(profile_name=self.profile)
            logger.info("Initialized S3FileSystem session.")
        except Exception as e:
            logger.error("Error initializing S3FileSystem Session, err: %s", e)
            raise S3Error("Error initializing S3FileSystem Session.")
        credentials = self.s3.session.get_credentials()
