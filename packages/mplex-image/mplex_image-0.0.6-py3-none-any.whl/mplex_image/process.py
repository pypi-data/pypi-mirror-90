...
def fill_cellpose_nas(df_img_all,df_mi_full,ls_marker_cyto,s_thresh='Ecad',qcdir='.'): 
    '''
    some nuclei don't have a cytoplasm, use perinuc5
    '''
    #tissues: plot mean Ecad across all tissues (negative scenes will drive down the mean
    ...
    ls_neg_cells = []
    ls_ecad_cells = []
    for s_slide_scene in sorted(set(df_mi_full.slide_scene)):
        print(s_slide_scene)
        ls_index = df_mi_full[df_mi_full.slide_scene==s_slide_scene].index
        ecad_thresh = df_img_all[(df_img_all.slide_scene==s_slide_scene) & (df_img_all.marker==s_thresh)].threshold_li[0]
        ls_neg_slide = (df_mi_full[~(df_mi_full.loc[:,f'{s_thresh}_cytoplasm'] > ecad_thresh)]).index.intersection(ls_index).tolist()
        ls_ecad_slide = (df_mi_full[(df_mi_full.loc[:,f'{s_thresh}_cytoplasm'] > ecad_thresh)]).index.intersection(ls_index).tolist()
        ls_neg_cells = ls_neg_cells + ls_neg_slide
        ls_ecad_cells = ls_ecad_cells + ls_ecad_slide
    print('')
    ...
    for s_marker in ls_marker_cyto:
        ...
        df_mi_full.loc[ls_neg_cells,f'{s_marker}_cytoplasm'] = df_mi_full.loc[ls_neg_cells,f'{s_marker}_perinuc5'] 
        print(f'with {s_marker}_perinuc5')
    df_mi_full[f'{s_thresh}_negative'] = df_mi_full.index.isin(ls_neg_cells)
    return(df_mi_full)
...
def filter_loc_cellpose(df_mi_filled, ls_marker_cyto, ls_custom,filter_na=True):
    ...
    ls_marker = sorted(set([item.split('_')[0] for item in df_mi_filled.columns[df_mi_filled.dtypes=='float64']]))
...
def filter_cellpose_df(s_sample,segdir,qcdir,s_thresh,ls_membrane,ls_marker_cyto,ls_custom,ls_filter,ls_shrunk,man_thresh = 900):
    ...
    i_max = df_img_all[df_img_all.marker.isin([item.split('_')[0] for item in ls_filter])].sort_values('round_ord').iloc[-1].round_ord
    ls_drop_marker = [item + '_' for item in sorted(set(df_img_all[(df_img_all.round_ord>i_max)].marker))]
    # drop rounds after last filter (makes sense)
    [df_mi_full.drop(df_mi_full.columns[df_mi_full.columns.str.contains(item)],axis=1,inplace=True) for item in ls_drop_marker]
    df_mi_filled = fill_cellpose_nas(df_img_all,df_mi_full,ls_marker_cyto,s_thresh=s_thresh,ls_celline=[],
    ...
    #df_bg = filter_cellpose_background(df_mi_filled, es_standard)
    ...
    #df_bg.to_csv(f'{segdir}/features_{s_sample}_BackgroundQuantiles.csv')
    ...
    df_mi_filled = fill_cellpose_nas(df_img_all,df_mi_full,ls_marker_cyto,s_thresh=s_thresh,qcdir=qcdir)
