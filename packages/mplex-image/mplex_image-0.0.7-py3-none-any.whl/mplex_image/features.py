...
def combine_labels(s_sample,segdir, subdir, ls_seg_markers, nuc_diam, cell_diam, df_mi_full,s_thresh):
    ...
    ls_scene = sorted(set([item.split('_cell')[0].replace('_scene','-Scene-') for item in se_neg.index]) & set(ls_scene))
    for s_scene in ls_scene:
...
def edge_mask(s_sample,segdir,subdir,i_pixel=154, dapi_thresh=350):
    '''
    ...
    for s_scene in sorted(set(df_img.scene)):
        ...
        mask_filled = morphology.remove_small_holes(mask, 50000)
