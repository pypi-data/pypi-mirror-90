...
class CloudSQL:
    ...
    def create_table(self, table, recreate=False):
        ...
        self.metadata.create_all(self.connection.engine, tables=[table])
        if recreate:
            self.connection.execute(text('TRUNCATE TABLE {}'.format(table.name)))
    ...
    def insert(self, table_name, key_field, schema):
        ...
        self.connection.execute(text(query.sql))
