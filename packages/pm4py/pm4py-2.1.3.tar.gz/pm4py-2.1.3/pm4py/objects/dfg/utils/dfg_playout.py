...
def apply(dfg, start_activities, end_activities, parameters=None):
    ...
    while partial_traces and len(
            final_traces) < max_no_variants and overall_probability <= min_weighted_probability:
        if interrupt_simulation_when_dfg_complete and interrupt_break_condition:
            break
        # we add step by step until reaching an end activity
        # and adding the trace to final_traces.
        ...
        for new_act in node_transition_probabilities[last_act]:
            if trace_counter[new_act] < max_no_occ_per_activity:
                ...
                if new_act is None:
                    ...
                    # is the new variant taking anything new to action?
