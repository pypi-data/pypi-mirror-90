        ...
        with tracer.start_as_current_span('WritableFuse.flush', self._span_context) as span:
            ...
            if file is None or not file.is_dirty or not self._datastore:
                log.debug('Skipping flush because. file: {}, is_dirty: {}, datastore: {}'.format(
                    file, file and file.is_dirty, self._datastore and self._datastore.name
                ))
                return 0
            file_lock = self._file_locks[path]
            with file_lock:
                prev_modified_time = file.last_flush_modified_time
                try:
                    file.reset_last_flush_modified_time()
                    ...
                    file.last_flush_modified_time = prev_modified_time
                    log.warning(e)
    ...
    def readdir(self, path, fh):
        ...
        if directory is not None:
            ...
            try:
                ...
                pass
            try:
                remote_entries = self._read_fuse.readdir(path, fh)
            except ExecutionError as e:
                if 'NotFound' not in e.error_code:
                    raise
                remote_entries = []
