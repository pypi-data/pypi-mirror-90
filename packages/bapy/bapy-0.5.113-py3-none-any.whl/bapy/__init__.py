    ...
    def exit(self):
        ...
        for child in self.current_process.children(recursive=True):
            try:
                ...
                if self.log:
                    self.log.debug('Killed current child sudo', f'{function=}', f'{self.current_pid=}',
                                   f'{child.pid=}', f'{exception=}')
                                ...
                                for p in process.children(recursive=True):
                                    ...
                                    if stat.get(text, None):
                                        if text in p.info['cmdline'][0] or text in p.info['cmdline'][1]:
                                            stat[text] += 1
        except (psutil.AccessDenied, psutil.NoSuchProcess, psutil.ProcessLookupError) as exception:
            if self.log:
