...
class Count:
    ...
    def console(self, msg, error: Bool = False, no: Bool = False):
        ...
        if self.verbose:
            ...
            cprint(f"[bold blue]{percentage}[white] "
...
class Kill:
    ...
    def exit(self):
        ...
        if self.log:
            self.log.debug(f'{function=}', f'{current_pid=}')
        parent = psutil.Process(current_pid)
        for child in parent.children(recursive=True):
            try:
                child.kill()
                if self.log:
                    self.log.debug('Killed current child', f'{function=}', f'{current_pid=}', f'{child.pid=}')
            except psutil.AccessDenied as exception:
                if self.log:
                    self.log.warning('Kill current child', f'{function=}', f'{current_pid=}', f'{child.pid=}',
                                     f'{exception=}')
                cmd(f'{self.cmd} {child.pid}')
                if self.log:
                    self.log.debug('Killed current child sudo', f'{function=}', f'{current_pid=}', f'{child.pid=}')

    def stop(self, command, parameter):
        function = inspect.currentframe().f_code.co_name
        current_pid = os.getpid()
        if self.log:
            self.log.debug(f'{function=}', f'{current_pid=}')
        ...
        for process in psutil.process_iter(attrs):
            if isinstance(process.info['cmdline'], list):
                if len(process.info['cmdline']) > 1:
                    if (command in process.info['cmdline'][0] or command in process.info['cmdline'][1]) \
                            and parameter in process.info['cmdline'][1:]:
                        if self.log:
                            self.log.debug(f'{function=}', f'{process.info=}')
                        if parameter in process.info['cmdline']:
                            if self.log:
                                self.log.debug(f'{function=}', f'{parameter=}', f'{process.info["cmdline"]=}')
                            for p in process.children(recursive=True):
                                try:
                                    ...
                                    if self.log:
                                        self.log.warning('Kill child', f'{function=}', f'{p.pid=}',
                                                         f'{exception=}')
                                    cmd(f'{self.cmd} {p.pid}')
                                    if self.log:
                                        self.log.debug('Killed child sudo', f'{function=}', f'{p.pid=}')
...
class GitHub:
    ...
    thread: Text = dataclasses.field(default=str(), init=False)
...
class Log(verboselogs.VerboseLogger):
    ...
    manager: Log = None
    package: Text = None
    format: Literal['short', 'large', 'date'] = 'large_thread'
...
class Logr(Logger):
    ...
    manager: Log = None
    package: Text = None
    format: Literal['short', 'large'] = 'large_thread'
    formats: Dict[Literal['short', 'large', 'date'], Text] = dict(
        short='%(message)s',
