...
class Kill:
    ...
    command: typing.Text = dataclasses.field(default=str(), init=False)
    ...
    parameter: typing.Text = dataclasses.field(default=str(), init=False)
    ...
    def exit(self):
        ...
        for child in self.current_process.children(recursive=True):
            try:
                ...
                if self.log:
                    self.log.warning('Kill current child', f'{function=}', f'{self.current_pid=}', f'{child.pid=}',
                                     f'{exception=}')
                cmd(f'{self.cmd} {child.pid}')
                if self.log:
                    self.log.debug('Killed current child sudo', f'{function=}', f'{self.current_pid=}', f'{child.pid=}')

    def stat(self, text: typing.Text = None, verbose: Bool = True):
        function = inspect.currentframe().f_code.co_name
        stat = {item: int() for item in ['children', ''] if item}
        stat['threads'] = self.current_process.threads()
        stat['open_files'] = self.current_process.open_files()
                    ...
                    if (self.command in process.info['cmdline'][0] or self.command in process.info['cmdline'][1]) \
                        ...
                        if self.parameter in process.info['cmdline']:
                            for p in process.children(recursive=True):
                                stat['children'] += 1
                                if stat.get(text, None):
                                    if text in p.info['cmdline'][0] or text in p.info['cmdline'][1]:
                                        stat[text] += 1
