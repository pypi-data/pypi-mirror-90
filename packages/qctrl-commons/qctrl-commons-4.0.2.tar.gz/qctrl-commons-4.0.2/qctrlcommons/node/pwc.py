...
class TensorPwc(CoreNode):
    ...
    durations : np.ndarray (1D, real)
        The durations :math:`\{\delta t_n\}` of the constant segments.
    values : np.ndarray or Tensor
        The values :math:`\{v_n\}` of the function on the constant segments.
        The first dimension must be the same length as `durations`.
    name : str, optional
    ...
    TensorPwc
        ...
        :math:`t_0=0` and :math:`t_n=t_{n-1}+\delta t_n`.
    """
    ...
    def create_node_data(cls, _operation, **kwargs):
        ...
        check_argument(
            values_shape[0] == durations_shape[0],
            "The first dimension of values and durations must be equal.",
...
class PwcSum(CoreNode):
    ...
    Parameters
    ----------
    terms : list[qctrlcommons.node.pwc.TensorPwc]
    ...
    Returns
    -------
    qctrlcommons.node.pwc.TensorPwc
...
class SymmetrizePwc(CoreNode):
    ...
    Parameters
    ----------
    pwc : qctrlcommons.node.pwc.TensorPwc
...
class TimeEvolutionOperatorsPwc(CoreNode):
    ...
    Parameters
    ----------
    hamiltonian : qctrlcommons.node.pwc.TensorPwc
