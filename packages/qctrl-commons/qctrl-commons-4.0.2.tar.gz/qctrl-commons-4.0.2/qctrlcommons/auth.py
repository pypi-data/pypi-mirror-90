...
class TokenAuth(ClientAuthBase):  # pylint: disable=too-few-public-methods
...
class BearerTokenAuth(ClientAuthBase):
    ...
    def _check_subscription(cls, access_token: str) -> None:
        ...
        # dont check subscription for superusers
        if payload.get("is_superuser", False):
            return

        # TODO need to check if products is even in payload
        # while cutover happens. Remove this later
