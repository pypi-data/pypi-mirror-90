...
LANGUAGES = load(
    open(os.path.join("resources", "languages.yml"), "r"),
    Loader=Loader
).items()
VENDOR_PATTERNS = load(
    open(os.path.join("resources", "vendor.yml"), "r"),
    Loader=Loader
)
DOC_PATTERNS = load(
    open(os.path.join("resources", "documentation.yml"), "r"),
    Loader=Loader
)["Files"]
USER_AGENTS = list(load(
    open(os.path.join("resources", "useragents.yml"), "r"),
    Loader=Loader
))
...
def fetch_file_paths(repo_name, owner):
    ...
    driver = webdriver.Chrome(
        executable_path=os.path.join("resources", "chromedriver"),
