module fruit_mpi
  ...
  integer, parameter :: FRUIT_MPI_INT = 4
  integer(FRUIT_MPI_INT), parameter :: FN_LENGTH = 50
...
contains
  ...
  subroutine     fruit_summary_mpi_(size, rank)
    ...
    integer(FRUIT_MPI_INT) :: num_msgs
    integer(FRUIT_MPI_INT) :: num_msgs_sum
    integer(FRUIT_MPI_INT), allocatable :: num_msgs_rank(:)

    integer(FRUIT_MPI_INT) :: ierr
    integer :: i
    integer :: imsg
    integer(FRUIT_MPI_INT) :: status(MPI_STATUS_SIZE)

    integer(FRUIT_MPI_INT), parameter :: MSG_LENGTH_HERE = 256
    ...
    call MPI_Allgather(&
    & num_msgs,      1_FRUIT_MPI_INT, MPI_INTEGER, &
    & num_msgs_rank, 1_FRUIT_MPI_INT, MPI_INTEGER, MPI_COMM_WORLD, ierr)
    ...
    if (rank == 0) then
      ...
      do i = 1, size - 1
        ...
        & int(i, FRUIT_MPI_INT), 7_FRUIT_MPI_INT, MPI_COMM_WORLD, status, ierr)
      enddo
    else
      call MPI_Send(&
      & msgs, &
      & num_msgs * MSG_LENGTH_HERE               , MPI_CHARACTER, &
      & 0_FRUIT_MPI_INT, 7_FRUIT_MPI_INT, MPI_COMM_WORLD, ierr)
    endif

    call MPI_REDUCE(&
    & fail_assert    , &
    & fail_assert_sum, 1_FRUIT_MPI_INT, MPI_INTEGER, MPI_SUM, 0_FRUIT_MPI_INT, MPI_COMM_WORLD, ierr)

    call MPI_REDUCE(&
    & succ_assert    , &
    & succ_assert_sum, 1_FRUIT_MPI_INT, MPI_INTEGER, MPI_SUM, 0_FRUIT_MPI_INT, MPI_COMM_WORLD, ierr)

    call MPI_REDUCE(&
    & fail_case    , &
    & fail_case_sum,   1_FRUIT_MPI_INT, MPI_INTEGER, MPI_SUM, 0_FRUIT_MPI_INT, MPI_COMM_WORLD, ierr)

    call MPI_REDUCE(&
    & succ_case    , &
    & succ_case_sum,   1_FRUIT_MPI_INT, MPI_INTEGER, MPI_SUM, 0_FRUIT_MPI_INT, MPI_COMM_WORLD, ierr)
  ...
  subroutine     fruit_summary_mpi_xml_(size, rank)
    ...
    &     FN_LENGTH, MPI_CHARACTER,     0_FRUIT_MPI_INT, 8_FRUIT_MPI_INT, MPI_COMM_WORLD, ierr)
    endif
    if (rank == 0) then
      ...
      do i = 1+1, size
        ...
        & FN_LENGTH, MPI_CHARACTER, i - 1_FRUIT_MPI_INT, 8_FRUIT_MPI_INT, MPI_COMM_WORLD, status, ierr)
      enddo
    endif
