...
#include "core/preprocessor.inc"
...
#endif
  implicit none
  integer(lli) :: n_trails, trial, verbosity, n_rho, n_in, n_out, n_members, n_nets, n_outputs
  type (qlknn_options) :: opts
  real(qlknn_dp), dimension(:,:), allocatable :: input
  real(qlknn_dp) :: start, finish
  real(qlknn_dp), dimension(:,:), allocatable :: qlknn_out, qlknn_eb
  real(qlknn_dp), dimension(:,:,:), allocatable :: dqlknn_out_dinput
  !logical(qlknn_bool), dimension(:,:), allocatable :: qlknn_validity
  logical, dimension(:,:), allocatable :: qlknn_validity
  character(len=4096) :: nn_path = '', input_path = '', output_path = ''
  character(len=12) :: cmd_name = 'qlknn_jetexp'
  ...
  call cla_register('-n', '--nn-path', 'Path to NN namelist folder', cla_char, 'data/qlknn-jetexp-namelists')
  ...
  call cla_register('-c', '--committee-members', 'Set number of committee members', cla_int, '1')
  ...
  call cla_get('--committee-members', n_members)
  ...
  allocate(qlknn_eb(n_rho, n_out))
  allocate(qlknn_validity(n_rho, n_out))
  allocate(dqlknn_out_dinput(n_out, n_rho, n_in))

  call load_jetexp_nets_from_disk(nn_path, n_members, verbosity)
  ...
  do trial = 1,n_trails
    if (.not. calculate_jacobian) then
      ...
      call evaluate_jetexp_net(input, nets, n_members, qlknn_out, qlknn_eb, verbosity, opts, dqlknn_out_dinput, qlknn_validity=qlknn_validity)
    endif
  end do
