...
#include "core/preprocessor.inc"
  ...
  if (verbosity >= 0) then
    write(*,"(A,A,A,I2)") "Running nettype '", trim(nettype), "', casenum=", casenum
  endif

  if (trim(nettype) == "hyper") then
    call regression_hyper_multi(casenum, input, err_out, derr_out, verbosity - 1)
  endif

  n_out = size(derr_out, 1)
  n_rho = size(derr_out, 2)
  n_in = size(derr_out, 3)
