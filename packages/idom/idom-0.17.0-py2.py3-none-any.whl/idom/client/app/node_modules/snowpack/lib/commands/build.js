...
class FileBuilder {
    ...
    async buildFile() {
        ...
        const builtFileOutput = await build_pipeline_1.buildFile(this.filepath, {
            ...
            const resolvedCode = await rewrite_imports_1.transformFileImports(file, (spec) => {
                ...
                if (isProxyImport) {
                    if (isAbsoluteUrlPath) {
                        this.filesToProxy.push(path_1.default.resolve(this.config.devOptions.out, resolvedImportPath));
                    }
                    else {
                ...
                if (isAbsoluteUrlPath) {
                    resolvedImportUrl = util_1.relativeURL(path_1.default.dirname(outLoc), path_1.default.resolve(this.config.devOptions.out, resolvedImportPath));
                }
...
async function command(commandOptions) {
    ...
    const mountedDirectories = Object.entries(config.mount).map(([fromDisk, toUrl]) => {
        return [
            path_1.default.resolve(cwd, fromDisk),
            path_1.default.resolve(buildDirectoryLoc, util_1.removeLeadingSlash(toUrl)),
        ];
    });
    ...
    for (const runPlugin of config.plugins) {
        if (runPlugin.run) {
            const runJob = runPlugin
                ...
                log: (msg, data = {}) => {
                    if (msg === 'WORKER_MSG') {
                        logger_1.logger.info(`[${runPlugin.name}] ${data.msg.trim()}`);
                    }
                },
            })
                .catch((err) => {
    ...
    await fs_1.promises.writeFile(path_1.default.join(internalFilesBuildLoc, 'env.js'), build_import_proxy_1.generateEnvModule('production'));
    ...
    for (const [fromDisk, dirDest] of mountedDirectories) {
        ...
        for (const rawLocOnDisk of allFiles) {
            const locOnDisk = path_1.default.resolve(rawLocOnDisk); // this is necessary since glob.sync() returns paths with / on windows.  path.resolve() will switch them to the native path separator.
    ...
    if (hmrEngine) {
        logger_1.logger.info(`[HMR] WebSocket URL available at ${colors.cyan(`ws://localhost:${config.devOptions.hmrPort}`)}`);
    }
