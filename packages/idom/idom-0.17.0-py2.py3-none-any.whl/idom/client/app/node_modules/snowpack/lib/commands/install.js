...
async function getInstallTargets(config, scannedFiles) {
    ...
    if (scannedFiles) {
        ...
        installTargets.push(...(await scan_imports_js_1.scanImports(cwd, config)));
    }
...
async function command(commandOptions) {
    ...
    logger_1.logger.debug('Install command successfully ran');
    if (finalResult.newLockfile) {
        await util_js_1.writeLockfile(path_1.default.join(cwd, 'snowpack.lock.json'), finalResult.newLockfile);
...
async function run({ config, lockfile, installTargets, }) {
