...
function parseScript(script) {
    ...
    if (outputMatch) {
        ...
        cleanOutput = Array.from(new Set(cleanInput.map((ext) => src_file_extension_mapping_1.default[ext] || ext)));
    }
    return {
        scriptType,
        input: cleanInput,
...
function normalizeMount(config) {
    ...
    for (const [mountDir, mountUrl] of Object.entries(mountedDirs)) {
        const fromDisk = path_1.default.posix.normalize(mountDir + '/');
...
function normalizeAlias(config, createMountAlias) {
    ...
    if (createMountAlias) {
        for (const mountDir of Object.keys(config.mount)) {
            if (mountDir !== '.') {
                cleanAlias[util_1.addTrailingSlash(mountDir)] = util_1.addTrailingSlash(`./${mountDir}`);
            }
        }
    }
    for (const [target, replacement] of Object.entries(config.alias)) {
        if (replacement === '.' ||
            ...
            cleanAlias[util_1.addTrailingSlash(target)] = util_1.addTrailingSlash(path_1.default.resolve(cwd, replacement));
        }
    }
    return cleanAlias;
}
...
function normalizeConfig(config) {
    ...
    config.devOptions.out = path_1.default.resolve(cwd, config.devOptions.out);
