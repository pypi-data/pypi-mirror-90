...
def run_daily_data(run_date: dt.date = dt.date.today(),
                   ...
                   sl_min: float = .02,
                   sl_max: float = .1,
                   macd_trend_details: bool = False) -> pd.DataFrame:
    ...
    final_df_non_zero.sort_values(['ranking', 'ticker', 'pattern'], ascending=[False, True, True], inplace=True)
