...
def daily_recommendation_twitter_template(symbol: str,
    ...
    :param name: str - Full name of company or any other information to be added
    :param end_date: dt.date - Date T
    ...
    if sl is None:
        sl = max(min(min(df.Low.iloc[-2:]), (1-sl_min)*entry), (1-sl_max)*entry)
    if rr is None:
        rr = 2
