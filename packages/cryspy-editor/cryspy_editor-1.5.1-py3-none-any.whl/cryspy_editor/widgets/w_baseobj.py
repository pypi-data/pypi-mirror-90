...
class WBaseObj(QtWidgets.QTabWidget):
    ...
    def set_object(self, obj):
        ...
        for ind_item in range(self.count()-3, -1, -1):
        ...
        if report_html != "":
            ...
            # self.addWidget(w_plain_text) 
            self.insertTab(0, w_plain_text, "Report") 

        # Figure tab
        try:
            l_fig_ax = reversed([fig_ax for fig_ax in obj.plots() if fig_ax is not None])
        except Exception as e:
            l_fig_ax = []
        ...
        if isinstance(obj, (LoopN, ItemN)):
            ...
            self.insertTab(0, w_plain_text, "RCIF format") 

        if self.count() == 0:
            q_label = QtWidgets.QLabel("No graphs and reports.")
            q_label.setSizePolicy(
                QtWidgets.QSizePolicy(QtWidgets.QSizePolicy.Expanding,
                                      QtWidgets.QSizePolicy.Expanding))
            self.insertTab(0, q_label, "")
