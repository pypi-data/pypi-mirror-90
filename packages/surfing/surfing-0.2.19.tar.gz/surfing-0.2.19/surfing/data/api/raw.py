...
class RawDataApi(metaclass=Singleton):
    ...
    def get_em_conv_bond_info(self):
        with RawDatabaseConnector().managed_session() as db_session:
            try:
                query = db_session.query(
                    EmConvBondInfo,
    ...
    def get_em_stock_price_recent(self, codes: Tuple[str] = ()):
        with RawDatabaseConnector().managed_session() as db_session:
            try:
                dt = db_session.query(
    ...
    def get_em_stock_ipo_info(self):
        with RawDatabaseConnector().managed_session() as db_session:
            ...
            try:
                ...
                df['股票代码不带市场'] = df.股票代码.map(lambda x : x.split('.')[0])
                df['申购代码'] = df.申购代码带市场.map(lambda x : x.split('.')[0])
    ...
    def get_em_conv_bond_ipo_info(self):
        with RawDatabaseConnector().managed_session() as db_session:
            ...
            try:
                ...
                df['申购代码带市场'] = df.申购代码 + '.' +df.转债代码.map(lambda x: x.split('.')[1])
