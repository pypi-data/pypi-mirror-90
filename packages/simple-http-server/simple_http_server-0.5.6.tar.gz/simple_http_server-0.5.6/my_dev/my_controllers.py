...
def show_upload():
    root = os.path.dirname(os.path.abspath(__file__))
    return StaticFile(f"{root}/my_test_index.html", "text/html; charset=utf-8")
...
def my_path_val_ctr(val=PathValue("pval"), path_val=PathValue(), param=Parameter(name="abc")):
...
request_map("/obj/say_hello", controller_function=my_ctrl_obj.my_ctrl_mth)
