...
from quarchpy.disk_test.driveTestCore import executeAndCheckCommand, comms
...
def setupSimpleHotplug(myDevice, delayTime, stepCount):
    ...
    for steps in range(1, 6):
        ...
        if "numeric value" in str(result).lower():
            comms.create_request_log(time.time(), "error", "Quarch Module incapable of this delay, swapping to 1270mS",
...
def setupSourceBounce(myDevice, sourceNumber, bouncePattern):
    ...
    if bounceParams[0] == "Simple":
        ...
        if not executeAndCheckCommand(myDevice, "source:" + str(sourceNumber) + ":delay " + str(bounceParams[1]).replace(" mS", ""):
            commandSuccess = False
        # Set the bounce length
        if not executeAndCheckCommand(myDevice,
                                      "source:" + str(sourceNumber) + ":bounce:length " + str(bounceParams[2]).replace(" mS", "")):
            commandSuccess = False
        # Set the bounce period
        if not executeAndCheckCommand(myDevice,
                                      "source:" + str(sourceNumber) + ":bounce:period " + str(bounceParams[3]).replace(" uS", "")):
            commandSuccess = False
        # Set the bounce duty cycle (strip the %, which is not needed for the command)
        if not executeAndCheckCommand(myDevice, "source:" + str(sourceNumber) + ":bounce:duty " + str(bounceParams[4]).replace(" mS", "").strip(
                '%')):
            commandSuccess = False
    else:
...
def setupSignalsToSource(myDevice, signalList, sourceNumber):
    ...
    for nextSignal in signalList:
        if not executeAndCheckCommand(myDevice, "signal:" + nextSignal + ":source " + str(sourceNumber)):
            commandSuccess = False
...
def clearSourceBounce(myDevice, sourceNumber):
    ...
    if not executeAndCheckCommand(myDevice, "source:" + str(sourceNumber) + ":bounce:clear"):
        commandSuccess = False
...
def unhIolHotPlugSimpleSweep(quarchName, driveName, startTime, endTime, stepCount, repeats=1, onTime=10, offTime=5):
    ...
    # Log start of test
    comms.create_request_log(time.time(), "testDescription",
    ...
    for nextTime in testTimes:
        ...
        if driveTestConfig.testCallbacks["TEST_GETDISKSTATUS"](driveObject, 1):
            comms.create_request_log(time.time(), "testResult",
                                 "HOT_PLUG: Drive detected as expected after " + str(onTime) + "seconds",
                                     os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name,
                                     {"testResult": True})
        else:
            comms.create_request_log(time.time(), "testResult",
                                 "HOT_PLUG: Drive NOT detected as expected after " + str(onTime) + "seconds",
                                     os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name,
                                     {"testResult": False})
            if dtsGlobals.runUntilFirstFailure:
        ...
        for loop in range(0, repeats):
            ...
            if driveTestConfig.testCallbacks["TEST_GETDISKSTATUS"](driveObject, 0):
                ...
                comms.create_request_log(time.time(), "testResult",
                                     "HOT_PLUG: Drive NOT removed after " + str(offTime) + "seconds",
                                         os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name,
                                         {"testResult": False})
                if dtsGlobals.runUntilFirstFailure:
                    comms.send_stop_test()
            ...
            if driveTestConfig.testCallbacks["TEST_GETDISKSTATUS"](driveObject, 1):
                ...
                comms.create_request_log(time.time(), "testResult",
                                     "HOT_PLUG: Drive NOT detected after " + str(onTime) + "seconds",
                                         os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name,
                                         {"testResult": False})
                if dtsGlobals.runUntilFirstFailure:
                    comms.send_stop_test()
        ...
        if errorSubCounter == 0:
            comms.create_request_log(time.time(), "testResult",
                                 "Hotplug sweep test passed all repetitions at: " + str(nextTime),
                                     os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name,
                                     {"testResult": True})
        else:
            comms.create_request_log(time.time(), "testResult",
                                 "Hotplug sweep test failed: " + str(errorSubCounter) + " repetitions at: " + str(
                                     nextTime), os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name,
                                     {"testResult": False})

    # Track errors from the entire sweep
    if errorCounter == 0:
        comms.create_request_log(time.time(), "testResult", "Hotplug sweep test passed all tests",
                                 os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, {"testResult": True})
    else:
        comms.create_request_log(time.time(), "testResult",
                             "Hotplug sweep test failed: " + str(errorCounter) + " repetitions in total",
                                 os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name,
                                 {"testResult": False})
...
def simpleHotPlugTest(uniqueID, quarchName, driveName, insertTime, repeats=1, onTime=10, offTime=5):
    ...
    comms.create_request_log(time.time(), "testDescription", "Starting Hotplug test at: " + str(
        insertTime) + "mS on device: " + driveName + ", over " + str(repeats) + " repetitions",
    ...
    comms.create_request_log(time.time(), "testDescription", "Setting up module for hotplug test",
                             os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, uId=uniqueID + ".0")
    # Module to default state
    executeAndCheckCommand(quarchDevice, "conf:def:state")
    # Setup the hot-plug timing
    ...
    for loop in range(0, int(repeats)):
        ...
        comms.create_request_log(time.time(), "Test_Description", "Hotplug Cycle " + str(int(loop + 1)),
                                 os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, uId=uniqueID)
    ...
    comms.create_request_log(time.time(), "Results_Description", "Results of hotplug test",
                             os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, uId=uniqueID)

    # Track errors from this time step
    if errorSubCounter == 0:
        comms.create_request_log(time.time(), "testResult", "Hotplug test passed all repetitions",
                                 os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, {"testResult": True},
                                 uId=uniqueID + ".1")
    else:
        comms.create_request_log(time.time(), "testResult",
    ...
    comms.create_request_log(time.time(), "testSummary", summaryString,
...
def simpleHotPlugBounceTest(uniqueID, quarchName, driveName, insertTime, bouncePattern, bounceSignals, repeats=1,
                            onTime=15, offTime=10, bounceOnPlug=True, bounceOnPull=False):
    startTestTime = time.time()
    # Create a counter to increment id's
    startUID = uniqueID
    errorSubCounter = 0
    ...
    comms.create_request_log(time.time(), "testDescription",
                         "Starting " + str(insertTime) + " mS staged Hotplug with " + str(
                             bounceParams[1]) + " bounce on source: 4, over " + str(repeats) + " repetitions",
                             os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, uId=uniqueID)
    ...
    comms.create_request_log(time.time(), "testDescription", "Setting up module for Bounce test",
                             os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, uId=uniqueID + ".0")
    # Module to default state
    executeAndCheckCommand(quarchDevice, "conf:def:state")
    # Setup the hot-plug timing
    ...
    for loop in range(0, int(repeats)):
        ...
        uniqueID = startUID + "." + str(int(loop + 1))
        comms.create_request_log(time.time(), "testDescription", "Bounce test Cycle " + str(int(loop + 1)),
                                 os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, uId=uniqueID)
    ...
    uniqueID = startUID + "." + str(int(repeats) + 1)
    comms.create_request_log(time.time(), "testDescription", "Results of hotplug test",
                             os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, uId=uniqueID)

    # Track errors from this time step
    if errorSubCounter == 0:
        comms.create_request_log(time.time(), "testResult", "Bounce test passed all repetitions",
                                 os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, {"testResult": True},
                                 uId=uniqueID + ".1")
    else:
        comms.create_request_log(time.time(), "testResult",
                             "Bounce sweep test failed: " + str(errorSubCounter) + " sub-test points",
                                 os.path.basename(__file__) + " - " + sys._getframe().f_code.co_name, {"testResult": False},
                                 uId=uniqueID + ".1")
