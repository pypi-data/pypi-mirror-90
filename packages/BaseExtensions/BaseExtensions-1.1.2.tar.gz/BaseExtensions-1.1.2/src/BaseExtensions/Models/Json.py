...
__all__ = [
        'BaseModel', 'BaseListModel', 'BaseDictModel', 'BaseSetModel'
        ]
...
class BaseModel(object):
    ...
    def Assert(t: type, d: object):
        if not isinstance(d, t): BaseModel.throw(d, t)

    @staticmethod
    @overload
    def throw(d: object, t: Type): ...
    @staticmethod
    @overload
    def throw(d: object, *types: Type): ...
    @staticmethod
    def throw(d: object, *types: Type):
    ...
    def AssertKeys(d: dict, *args):
        try:
            ...
            raise KeyError(f"Required keys {args} are missing.     existing keys: {d.keys()}")
