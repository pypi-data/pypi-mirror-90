...
class BaseOutputHandler(object):
    ...
    def add_records(self, records, **kwargs):
        ...
        return True, self.send(data, **kwargs)
    ...
    def initialize(self, PROJECT_DIR):
        ...
        while True:
            try:
                ...
                # initialising state
                self.task_params = [] if self.is_another_instance_running else self.build_task_params()
                break
                ...
                log.error(f'''Initialize: {e}''', extra={"status": SourceHealthStatus.StatusError,
            ...
            if duration > max_back_off_secs:
                log.error(f'''Initialized failed: MAX_BACKOFF_MINUTES limit reached {base}''')
                break
            else:
        ...
        if not self.is_another_instance_running:
            try:
                ...
                if self.collection_config.get("IS_SHUFFLE_TASKS", True):
                    self.log.debug(f'''shuffling {len(self.task_params)} tasks''')
                    shuffle(self.task_params)
                all_futures = {}
                current_num_workers = min(self.num_workers, len(self.task_params))
                self.log.debug(f"spawning {current_num_workers} workers")
                ...
                for future in futures.as_completed(all_futures):
                    ...
                    try:
                        ...
                        # error handling for api threads
                        # Here exc info is true because these are invalid config errors and are non recoverable
                        self.log.error(f'''Thread: {api_type} {e}''', extra={"status": SourceHealthStatus.StatusError,
                                                                             "errorType": ErrorType.ERROR_CONFIG},
                                       exc_info=True)
                        ...
                        if self.is_first_fetch:
                            self.log.info("Authentication successful", extra={"status": SourceHealthStatus.StatusAuthenticated})
                            self.is_first_fetch = False
                        self.log.info(f"Thread: {api_type} completed successfully")

            except Exception as e:

                self.log.error(f'''{e}''', extra={"status": SourceHealthStatus.StatusError,
            ...
            if self.execution_state != ExecutionState.STOP:
                if has_any_error:
                    ...
                    self.log.info("All Threads completed successfully", extra={"status": SourceHealthStatus.StatusCollecting})
            self.stop_running()
        else:
        ...
        if self.collection_config["ENVIRONMENT"] == "ctoc":
            ...
            while self.execution_state != ExecutionState.STOP:
                ...
                if self.execution_state == ExecutionState.BACKOFF:
                    ...
                    sleep_time = base * polling_interval_secs
                    if sleep_time > max_back_off_secs:
                        self.execution_state = ExecutionState.STOP
                        self.log.info(
                            f'''Max backoff limit reached execution_state: {self.execution_state} backoff: {base} last_execution_time: {execution_time} sleep_time: {sleep_time - execution_time}''')
                        break
                    else:
                        base = base << 1
                ...
                self.log.info(f'''execution_state: {self.execution_state} backoff: {base} last_execution_time: {execution_time} sleep_time: {sleep_time - execution_time}''')
