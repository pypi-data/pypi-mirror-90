...
from sumoappclient.sumoclient.httputils import SessionPool, ClientMixin
...
class CToCHandler(HTTPHandler):
    ...
    def add_records(self, records, *args, **kwargs):
        ...
        current_data_size = self.utf8len(get_body(data, jsondump=True))
    ...
    def send(self, data, extra_headers=None, jsondump=True, **kwargs):
        ...
        for idx, batch in enumerate(self.bytesize_chunking(data, self.collection_config.get("MAX_PAYLOAD_BYTESIZE", self.MaxBundleSize), jsondump, extra_padding=self.CNCLogPaddingSize), start=1):
            body = json.dumps({"data": batch})
            self.log.debug(f'''Sending batch {idx} len: {len(body)}''')

            ctoc_data_endpoint = self.sumo_config['CTOC_BASE_URL'] + "/data"
            fetch_success, respjson = ClientMixin.make_request(ctoc_data_endpoint, method="post", session=sess, data=body, TIMEOUT=self.collection_config['TIMEOUT'], headers=headers, logger=self.log)
            if not fetch_success:
                self.log.error(f'''DataAPIError: {respjson}''',
                               extra={"status": SourceHealthStatus.StatusError, "errorType": ErrorType.ERROR_FRAMEWORK})
                return False
        return True
