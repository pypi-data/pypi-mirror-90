# AUTOGENERATED! DO NOT EDIT! File to edit: 00_simple_interpolation.ipynb (unless otherwise specified).
...
# Cell
def interpolate_gaps(df_to_patch, method = 'wiener', fixed_freq = None, plot = False):
    """
    Searches for gaps and interpolates with a Brownian Bridge,
    which preserves volatility of the whole trend.

    To adjust interpolation to, e.g., exact minutes,
    pass fixed_freq = 'min':
            interpolate_gaps( df, fixed_freq = 'min')
    This argument is inherited from Pandas; see valid options in:
    ...
    for gap_index in gap_indexes:
        ...
        if gap_indexes.index(gap_index) % 50 == 0:
            ...
            remain = ((elapsed_min / (perc+0.001))*(100-perc))/60
            print(f"Processed "+"{:.2f}".format(perc) + "% of gaps in "+"{:.2f}".format(elapsed_min)\
                  +" mins, around "+"{:.2f}".format(remain)+" h remaining")
...
#             # Estimated points to interpolate
#             if datetime_present:
#                 print(delta)
...
#             print(f"Estimated points to fill: {estimated_points_to_fill}, with interval {interval}")
...
# Internal Cell
def bb(x0, y0, x1, y1, params_deduced, variance, interpolated, fixed_freq, recursion_level = 0):
    """
    ...
    """
    # Recursion level exposed
    global recursion_level_exposed
    recursion_level_exposed = recursion_level
    ...
    bb(x0, y0, xm, ym, params_deduced, variance/scaleFactor, interpolated, fixed_freq, recursion_level = recursion_level + 1)
...
def compare_models(models):
    ...
    for m in models:
        ...
        # Forecast
        print(f"Extending {model_name} dataframe...")
