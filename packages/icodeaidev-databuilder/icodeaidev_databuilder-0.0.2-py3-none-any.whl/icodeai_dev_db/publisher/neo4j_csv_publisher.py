...
from neo4j.exceptions import TransientError, CypherError
from pyhocon import ConfigFactory, ConfigTree
from icodeai_dev_db.publisher.base_publisher import Publisher
from icodeai_dev_db.publisher.neo4j_preprocessor import NoopRelationPreprocessor
...
class Neo4jCsvPublisher(Publisher):
    ...
    def init(self, conf: ConfigTree) -> None:
        ...
        self.deadlock_node_labels = set(
            conf.get_list(NEO4J_DEADLOCK_NODE_LABELS, default=[])
        )
        ...
        LOGGER.info(
            "Publishing Node csv files %s, and Relation CSV files %s",
            self._node_files,
    ...
    def publish_impl(self) -> None:  # noqa: C901
        ...
        try:
            ...
            LOGGER.info(
                "Successfully published. Elapsed: %i seconds", time.time() - start
            )
    ...
    def _create_indices(self, node_file: str) -> None:
        ...
        with open(node_file, "r", encoding="utf8") as node_csv:
            for node_record in pandas.read_csv(node_csv, na_filter=False).to_dict(
                orient="records"
            ):
    ...
    def _publish_relation(self, relation_file: str, tx: Transaction) -> Transaction:
        ...
        if self._relation_preprocessor.is_perform_preprocess():
            ...
            with open(relation_file, "r", encoding="utf8") as relation_csv:
                for rel_record in pandas.read_csv(
                    relation_csv, na_filter=False
        ...
        with open(relation_file, "r", encoding="utf8") as relation_csv:
            for rel_record in pandas.read_csv(relation_csv, na_filter=False).to_dict(
                orient="records"
            ):
                ...
                while exception_exists and retries_for_exception > 0:
                    try:
                        ...
                        tx = self._execute_statement(
                            stmt, tx, params, expect_result=self._confirm_rel_created
                        )
                        exception_exists = False
                    except TransientError as e:
                        if (
                            rel_record[RELATION_START_LABEL]
                            in self.deadlock_node_labels
                            or rel_record[RELATION_END_LABEL]
                            in self.deadlock_node_labels
                        ):
    ...
    def _create_props_body(
        self, record_dict: dict, excludes: Set, identifier: str
    ) -> str:
        """
    ...
    def _try_create_index(self, label: str) -> None:
        ...
        LOGGER.info(
            f"Trying to create index for label %s if not exist: %s", label, stmt
        )
