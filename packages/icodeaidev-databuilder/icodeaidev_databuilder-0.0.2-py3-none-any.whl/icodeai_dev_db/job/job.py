...
from icodeai_dev_db.publisher.base_publisher import NoopPublisher, Publisher
...
class DefaultJob(Job):
    ...
    def __init__(
        self, conf: ConfigTree, task: Task, publisher: Publisher = NoopPublisher()
    ) -> None:
        self.task = task
        self.conf = conf
    ...
    def launch(self) -> None:
        ...
        try:
            ...
            self.publisher.init(
                Scoped.get_scoped_conf(self.conf, self.publisher.get_scope())
            )
