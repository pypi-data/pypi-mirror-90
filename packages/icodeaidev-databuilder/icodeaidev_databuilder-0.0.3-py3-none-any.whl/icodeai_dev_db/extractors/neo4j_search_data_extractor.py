...
from icodeai_dev_db.extractors.neo4j_extractor import Neo4jExtractor
from icodeai_dev_db.publisher.neo4j_csv_publisher import JOB_PUBLISH_TAG
...
class Neo4jSearchDataExtractor(Extractor):
    ...
    def init(self, conf: ConfigTree) -> None:
        ...
        """
        self.conf = conf
        self.entity = conf.get_string(
            Neo4jSearchDataExtractor.ENTITY_TYPE, default="table"
        ).lower()
        ...
        self.neo4j_extractor.init(
            Scoped.get_scoped_conf(self.conf, self.neo4j_extractor.get_scope())
        )
