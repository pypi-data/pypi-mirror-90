...
from icodeai_dev_db.extractors.base_extractor import Extractor
from icodeai_dev_db.models.badge import Badge, BadgeMetadata
from icodeai_dev_db.models.table import ColumnMetadata, TableMetadata
...
class CsvTableBadgeExtractor(Extractor):
    ...
    def init(self, conf: ConfigTree) -> None:
        self.conf = conf
        self.table_file_location = conf.get_string(
            CsvTableBadgeExtractor.TABLE_FILE_LOCATION
        )
        self.badge_file_location = conf.get_string(
            CsvTableBadgeExtractor.BADGE_FILE_LOCATION
        )
        self._load_csv()
    ...
    def _load_csv(self) -> None:
        ...
        for badge_dict in self.badges:
            ...
            badge = Badge(name=badge_dict["name"], category=badge_dict["category"])
        ...
        for table_dict in tables:
            ...
            badge_metadata = BadgeMetadata(
                start_label=TableMetadata.TABLE_NODE_LABEL, start_key=id, badges=badges
            )
...
class CsvTableColumnExtractor(Extractor):
    ...
    def init(self, conf: ConfigTree) -> None:
        ...
        self.conf = conf
        self.table_file_location = conf.get_string(
            CsvTableColumnExtractor.TABLE_FILE_LOCATION
        )
        self.column_file_location = conf.get_string(
