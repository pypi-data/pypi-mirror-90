...
from icodeai_dev_db.job.base_job import Job
from icodeai_dev_db.loader.base_loader import Loader
from icodeai_dev_db.models.graphs.serializable import GraphSerializable
from icodeai_dev_db.serializers.neo4j import serialize_node, serialize_relationship
...
class FsNeo4jCSVLoader(Loader):
    ...
    _DEFAULT_CONFIG = ConfigFactory.from_dict(
        {SHOULD_DELETE_CREATED_DIR: True, FORCE_CREATE_DIR: False}
    )
    ...
    def load(self, csv_serializable: GraphSerializable) -> None:
        ...
        while node:
            ...
            node_writer = self._get_writer(
                node_dict, self._node_file_mapping, key, self._node_dir, file_suffix
            )
        ...
        while relation:
            relation_dict = serialize_relationship(relation)
            key2 = (
                relation.start_label,
                relation.end_label,
                relation.type,
    ...
    def _get_writer(
        ...
        writer = csv.DictWriter(
            file_out, fieldnames=csv_record_dict.keys(), quoting=csv.QUOTE_NONNUMERIC
        )
