...
from icodeai_dev_db.publisher.elasticsearch_constants import (
...
class ElasticsearchPublisher(Publisher):
    ...
    def init(self, conf: ConfigTree) -> None:
        ...
        self.file_mode = self.conf.get_string(
            ElasticsearchPublisher.FILE_MODE_CONFIG_KEY, "w"
        )

        self.elasticsearch_type = self.conf.get_string(
            ElasticsearchPublisher.ELASTICSEARCH_DOC_TYPE_CONFIG_KEY
        )
        self.elasticsearch_client = self.conf.get(
            ElasticsearchPublisher.ELASTICSEARCH_CLIENT_CONFIG_KEY
        )
        self.elasticsearch_new_index = self.conf.get(
            ElasticsearchPublisher.ELASTICSEARCH_NEW_INDEX_CONFIG_KEY
        )
        self.elasticsearch_alias = self.conf.get(
        ...
        self.elasticsearch_batch_size = self.conf.get(
            ElasticsearchPublisher.ELASTICSEARCH_PUBLISHER_BATCH_SIZE, 10000
        )
    ...
    def _fetch_old_index(self) -> List[str]:
        ...
        """
        try:
            indices = self.elasticsearch_client.indices.get_alias(
                self.elasticsearch_alias
            ).keys()
            return indices
    ...
    def publish_impl(self) -> None:
        ...
        self.elasticsearch_client.indices.create(
            index=self.elasticsearch_new_index, body=self.elasticsearch_mapping
        )
        for action in actions:
            index_row = dict(
                index=dict(
                    _index=self.elasticsearch_new_index, _type=self.elasticsearch_type
                )
            )
        ...
        # delete old indices
        delete_actions = [
            {"remove_index": {"index": index}} for index in elasticsearch_old_indices
        ]
