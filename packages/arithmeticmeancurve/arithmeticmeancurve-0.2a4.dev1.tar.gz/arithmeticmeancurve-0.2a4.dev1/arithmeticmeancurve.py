...
def _dataframes_are_valid_for_calculation(
    different_curves: List[DataFrame],
) -> ValidationResult:
    """
...
def _ndarrays_are_valid_for_calculation(
    *different_curves,
) -> ValidationResult:
    """
...
def _input_curves_are_valid_for_calculation(
    ...
    .. doctest::
        ...
        ...     max_line_width=70
        ... )
        ValidationMessage(correct=False, error_message="Column-count of the provided
        curves doesn't match.", error_type=<class 'ValueError'>)
        >>> doctest_print(
        ...     _input_curves_are_valid_for_calculation(
        ...         [
        ...             DataFrame(numpy.arange(12).reshape(4, 3)),
...
def _merge_two_curves(
    ...
    .. doctest::
        ...
        >>> from doctestprinter import doctest_print
        >>> doctest_print(_merge_two_curves(left_curve, right_curve))
                 left     right
        x
...
class FrozenStdExtrapolation(Extrapolates):
    ...
    >>> doctest_print(end_cap.round(3))
             y_0     y_1     y_2    y_3  y_4
    x
    0.920  8.100   9.650   9.390  11.29  NaN
    0.960  8.276   9.825   9.595  11.50  NaN
...
def _find_positions_in_between(
    ...
    Examples:
        ...
        >>> str(_find_positions_in_between(Series(), 2.0))
        'None'
    ...
    deltas_to_search_value = source_series - search_value
    ...
    left_reduced_selection = source_series.copy().iloc[:left_position]
    positions_of_left_selection = _find_positions_in_between(
        source_series=left_reduced_selection, search_value=search_value
    )
    if positions_of_left_selection is not None:
        return positions_of_left_selection

    right_reduced_selection = source_series.copy().iloc[right_position:]
    positions_of_left_selection = _find_positions_in_between(
        source_series=right_reduced_selection, search_value=search_value
    )
    if positions_of_left_selection is not None:
        offset_of_search = right_position
        sub_left_position, sub_right_position = positions_of_left_selection
        return (
            sub_left_position + offset_of_search,
            sub_right_position + offset_of_search,
        )
...
def _calculate_cutting_index_for_value(
    ...
    direct_value_within_series = cutting_value in selection_section.values
    if direct_value_within_series:
        cutting_index = series_to_cut[series_to_cut == cutting_value].index.values
        first_occurred_index = cutting_index[0]
        return first_occurred_index

    left_position, right_position = _find_positions_in_between(
        source_series=series_to_cut,
        search_value=cutting_value,
    )
    ...
    target_index = numpy.interp([cutting_value], y_as_x_values, x_as_y_values)[0]
    return target_index


def _get_slope(x0, y0, x1, y1):
    """
...
def _get_target_distances(
    ...
    .. doctest::
        ...
        ...     index=Index([0.960002, 0.969662, 0.972496, 0.973147], name="x")
        ... )
        >>> _get_target_distances(
        ...     end_mean_curve=horizontallinear0_sample_curve,
        ...     target_point=horizontallinear0_target
        ... )
        (0.006365999999999872, 0.0006509999999999572)
...
class _ExtrapolationBreakCondition(ABC):
    @abstractmethod
    def is_meet(self, extrapolated_values: Series) -> True:
        pass
...
class _TargetValuesAreReachedCondition(_ExtrapolationBreakCondition):
    ...
    .. doctest::
        ...
        ... )
        >>> first_quadrant_sample.mean(axis=1)
        0.1    2.0
        0.2    4.0
        0.3    6.0
        0.4    8.0
    ...
    .. doctest::
        ...
        ...     index = [0.1, 0.2, 0.3, 0.4]
        ... )
        >>> fourth_quadrant_sample.mean(axis=1)
        0.1   -2.0
        0.2   -4.0
        0.3   -6.0
        0.4   -8.0
    ...
    .. doctest::
        ...
        ...     index = [-0.1, -0.2, -0.3, -0.4]
        ... )
        >>> second_quadrant_sample.mean(axis=1)
        -0.1    2.0
        -0.2    4.0
        -0.3    6.0
        -0.4    8.0
    ...
    .. doctest::
        ...
        ...     index = [-0.1, -0.2, -0.3, -0.4]
        ... )
        >>> third_quadrant_sample.mean(axis=1)
        -0.1   -2.0
        -0.2   -4.0
        -0.3   -6.0
        -0.4   -8.0
...
def _preprocess_end_cap_for_extrapolation(
    ...
    Args:
        a_mean_curve(AMeanCurve):
            The (arithmetic) mean curve, which contains the family of curves to be
            extrapolated.
...
def _calculate_mean_and_std_curve(
    ...
    index_for_target_y = _calculate_cutting_index_for_value(
        ...
        cutting_value=target_end_y_value,
    )
...
class ArithmeticMeanCurve(ExtrapolatingMeanCurve, SectioningCurves, AMeanCurve):
    def __init__(
...
class _IterationRecord(object):
    ...
    ... )
    >>> doctest_print(sample_record)
                       y0   y1   y2
    sections     x
    start        0.1  1.0  1.5  NaN
    extrapolated 0.1  NaN  NaN  2.0
    iteration    0.1  1.0  1.5  1.9
                 0.1  1.0  1.5  2.0
    >>> doctest_print(sample_record.start_values)
          y0   y1  y2
    x
    0.1  1.0  1.5 NaN
    >>> doctest_print(sample_record.iteration_history)
          y0   y1   y2
    x
    0.1  1.0  1.5  1.9
    0.1  1.0  1.5  2.0
    >>> doctest_print(sample_record.extrapolation_result)
          y0   y1   y2
    x
    0.1  1.0  1.5  2.0
    >>> doctest_print(sample_record.extrapolated_values)
         y0  y1   y2
    x
    0.1 NaN NaN  2.0
    >>> reindex_sample = sample_record.reindex(iteration_number=2, common_divider=10)
    >>> doctest_print(reindex_sample)
                       y0   y1   y2
    sections     x
    start        0.1  1.0  1.5  NaN
    extrapolated 0.1  NaN  NaN  2.0
    ...
    ... )
    >>> doctest_print(reindex_sample_history)
          y0   y1   y2
    2.1  1.0  1.5  1.9
    2.2  1.0  1.5  2.0
    """
    ...
    def create_multi_index(
        ...
        Examples:
            ...
            MultiIndex([(       'start', 0.1),
                       ...
                       names=['sections', 'x'])
        """
        indexes = [("start", index_value), ("extrapolated", index_value),] + list(
            zip(
    ...
    def reindex(self, iteration_number: int, common_divider: float) -> DataFrame:
...
class _IterationRecords(object):
    ...
    >>> doctest_print(iteration_records.get_start_values())
                y_0      y_1     y_2      y_3  y_4
    0.92000 8.10000  9.65000 9.39007 11.29036  NaN
    0.96000 8.27609  9.82500 9.59512 11.50000  NaN
    0.99900 8.44777  9.99563 9.79505      NaN  NaN
    1.00000 8.45217 10.00000 9.79901      NaN  NaN
    1.02400 8.55783      NaN 9.89397      NaN  NaN
    >>> doctest_print(iteration_records.get_extrapolation_result())
                y_0      y_1     y_2      y_3      y_4
    0.92000 8.10000  9.65000 9.39007 11.29036 11.59981
    0.96000 8.27609  9.82500 9.59512 11.50000 11.79175
    0.99900 8.44777  9.99563 9.79505 11.65017 11.96463
    1.00000 8.45217 10.00000 9.79901 11.65434 11.96879
    1.02400 8.55783 10.14646 9.89397 11.76951 12.08396
    ...
    def __iter__(self) -> Generator[_IterationRecord, None, None]:
        return iter(self._records)

    def __next__(self) -> Generator[_IterationRecord, None, None]:
...
class ExtrapolationIterationTester(object):
    ...
    def record_iterations(
        ...
        .. doctest::
            ...
            >>> import pandas
            >>> pandas.options.display.float_format = '{:.5f}'.format
            >>> doctest_print(iteration_records)
                                     y_0     y_1     y_2      y_3      y_4
            sections
            start        0.92000 8.10000 9.65000 9.39007 11.29036      NaN
            extrapolated 0.92000     NaN     NaN     NaN      NaN 11.59981
            iteration    1.10000 8.10000 9.65000 9.39007 11.29036 11.50000
                         1.20000 8.10000 9.65000 9.39007 11.29036 11.58049
                         1.30000 8.10000 9.65000 9.39007 11.29036 11.59659
                         1.40000 8.10000 9.65000 9.39007 11.29036 11.59981
                                     y_0     y_1     y_2      y_3      y_4
            sections
            start        0.96000 8.27609 9.82500 9.59512 11.50000      NaN
            extrapolated 0.96000     NaN     NaN     NaN      NaN 11.79175
            iteration    2.10000 8.27609 9.82500 9.59512 11.50000 11.59981
                         2.20000 8.27609 9.82500 9.59512 11.50000 11.75361
                         2.30000 8.27609 9.82500 9.59512 11.50000 11.78437
                         2.40000 8.27609 9.82500 9.59512 11.50000 11.79052
                         2.50000 8.27609 9.82500 9.59512 11.50000 11.79175
                                     y_0     y_1     y_2      y_3      y_4
            sections
            start        0.99900 8.44777 9.99563 9.79505      NaN      NaN
            extrapolated 0.99900     NaN     NaN     NaN 11.65017 11.96463
            iteration    3.10000 8.44777 9.99563 9.79505 11.47730 11.79175
                         3.20000 8.44777 9.99563 9.79505 11.58145 11.89590
                         3.30000 8.44777 9.99563 9.79505 11.62311 11.93756
                         3.40000 8.44777 9.99563 9.79505 11.63978 11.95423
                         3.50000 8.44777 9.99563 9.79505 11.64644 11.96089
                         3.60000 8.44777 9.99563 9.79505 11.64911 11.96356
                         3.70000 8.44777 9.99563 9.79505 11.65017 11.96463
                                     y_0      y_1     y_2      y_3      y_4
            sections
            start        1.00000 8.45217 10.00000 9.79901      NaN      NaN
            extrapolated 1.00000     NaN      NaN     NaN 11.65434 11.96879
            iteration    4.10000 8.45217 10.00000 9.79901 11.65017 11.96463
                         4.20000 8.45217 10.00000 9.79901 11.65315 11.96760
                         4.30000 8.45217 10.00000 9.79901 11.65434 11.96879
                                     y_0      y_1     y_2      y_3      y_4
            sections
            start        1.02400 8.55783      NaN 9.89397      NaN      NaN
            extrapolated 1.02400     NaN 10.14646     NaN 11.76951 12.08396
            iteration    5.10000 8.55783 10.03129 9.89397 11.65434 11.96879
                         5.20000 8.55783 10.07815 9.89397 11.70119 12.01565
                         5.30000 8.55783 10.10626 9.89397 11.72931 12.04376
                         5.40000 8.55783 10.12313 9.89397 11.74617 12.06063
                         5.50000 8.55783 10.13325 9.89397 11.75630 12.07075
                         5.60000 8.55783 10.13932 9.89397 11.76237 12.07682
                         5.70000 8.55783 10.14296 9.89397 11.76601 12.08046
                         5.80000 8.55783 10.14515 9.89397 11.76820 12.08265
                         5.90000 8.55783 10.14646 9.89397 11.76951 12.08396
            >>> pandas.options.display.float_format = None
...
class MeanCurvePlotter(object):
    ...
    @staticmethod
    def plot_default(
        axis: pyplot.axis, a_mean_curve: ArithmeticMeanCurve, title: str = None
    ):
        """
        Plots into the end region of the family of curves and their mean curve.
        ...
        for index, (label, curve) in enumerate(family_of_curves.iteritems()):
            ...
            axis.plot(curve, marker="o", markersize=3, label=current_label)
        axis.plot(
            a_mean_curve.mean_curve, "--ko", markersize=5, label="arithmetic mean curve"
        )
        axis.plot(end_x, end_y, "rP")
