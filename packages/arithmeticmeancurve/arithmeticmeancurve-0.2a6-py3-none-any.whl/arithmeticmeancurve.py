...
def _merge_two_curves(
    ...
    .. doctest::
        ...
        0.15  6.5  7.5
        0.20  8.5  9.5
        >>> merged_frame = _merge_two_curves(left_sample, right_sample)
    ...
    if not left.index.is_unique:
        left_index = left.index.to_series()
        left_index.index = numpy.arange(len(left_index))
        left_index = left_index.drop_duplicates(keep="first")
        left_without_dup = left.iloc[left_index.index]
        warnings.warn(
            "Duplicated indexes of a curve where detected and removed. "
            "First occurrences were kept. Preprocessed curves before "
            "mean curve calculation."
        )
    else:
        left_without_dup = left

    if not right.index.is_unique:
        right_index = right.index.to_series()
        right_index.index = numpy.arange(len(right_index))
        right_index = right_index.drop_duplicates(keep="first")
        right_without_dup = right.iloc[right_index.index]
        warnings.warn(
            "Duplicated indexes of a curve where detected and removed. "
            "First occurrences were kept. Preprocessed curves before "
            "mean curve calculation."
...
def _calculate_nominal_circular_arc_points(
    ...
    """
    angle_steps = (end_angle_in_rad - start_angle_in_rad) / number_of_points
    full_circle_angles = numpy.arange(start_angle_in_rad, end_angle_in_rad, angle_steps)
...
class ExtrapolationIterationTester(object):
    ...
    def record_iterations(
        ...
        .. doctest::
            ...
            >>> import pandas
            >>> pandas.options.display.float_format = '{:.5f}'.format
            >>> doctest_print(recorded_iterations)
                                     y_0     y_1     y_2      y_3      y_4
            sections
