...
main1 = "<div><span class='fontteal'>neoviz is an open-source visualization tool that helps in quick exploratory data analysis and lets user to report findings as an application\
\
</span></div>"
...
<p>vizdxp is an open-source web app designed via streamlit weaved with plotly and seaborn libraries. <span class='fontblue'><b>Its incredibly Simple - Just drag & drop any csv and explore the data visually.</b></span> By default the application will figure out better visualization based on user selections. \
...
def main():
    ...
    st.sidebar.markdown('''# <span class='fontblue'>neo</span><span class='fontteal'>insyt</span>''', unsafe_allow_html=True)
    st.sidebar.text('')
    ...
    if page == 'Getting Started':
        ...
        demo_video = st.checkbox("Sample demo video")
        if demo_video:
            dv = video()
            st.video(dv)
            st.markdown("Credits: Dataset used in the demo above is from Chai Time Data Science show [podcasts](https://chaitimedatascience.com/)",unsafe_allow_html=True)
        st.sidebar.markdown("")
