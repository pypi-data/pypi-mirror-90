...
class Auth(DovecotAuth):
    ...
    [auth]
    type = radicale_kalabash_auth_token

    radicale_kalabash_auth_token_check_url = https://my.kalabash.com/api/v1/user-calendars/check_token/
    radicale_kalabash_auth_token_token = MYTOKENHERE
    ...
    def get_external_login(self, environ):
        ...
        try:
            check_token_url = self.configuration.get(
                'auth', 'radicale_kalabash_auth_token_check_url')
            api_token = self.configuration.get(
                'auth', 'radicale_kalabash_auth_token_token')
        except KeyError:
